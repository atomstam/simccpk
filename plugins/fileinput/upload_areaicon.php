...
if(!empty($_SESSION['admin_login']) || !empty($_SESSION['area_login']) || !empty($_SESSION['smp_login']) || !empty($_SESSION['clus_login']) ){
// Define a destination
$targetFolder = WEB_PATH_IMG_AREA; // Relative to the root
$TIMESTAMP = time();
...
if ($widths > _I_AREA_W || $heights > _I_AREA_H) {
...
//				echo str_replace($_SERVER['DOCUMENT_ROOT'],'',$targetFile);
				...
				$image->output_resized(str_replace('//','/',$targetPath).$TIMESTAMP.'.gif'."", "GIF");
				echo json_encode($TIMESTAMP.'.gif');
