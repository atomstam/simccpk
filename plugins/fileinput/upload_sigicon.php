...
$filename = $_FILES['avatar-3']['name'];
$ext = pathinfo($filename, PATHINFO_EXTENSION);
if(!in_array($ext,$allowed) ) {
//if (empty($_FILES['avatar-3']) || (strchr($_FILES['avatar-3']['name'],".")!=".JPG") ||(strchr($_FILES['avatar-3']['name'],".")!=".jpg") || (strchr($_FILES['avatar-3']['name'],".")!=".GIF") || (strchr($_FILES['avatar-3']['name'],".")!=".gift") || (strchr($_FILES['avatar-3']['name'],".")!=".PNG") || (strchr($_FILES['avatar-3']['name'],".")!=".png") ) {
...
//$images = $_FILES['avatar-3'];
//$nombre_img2=$images['name'];
//echo json_encode(['success'=>'SI LLEGA y es el fileinput con nombre de archivo:'. $nombre_img2]);
$tempFile = $_FILES['avatar-3']['tmp_name'];
$targetPath = $targetFolder;
$targetFile = $targetFolder . $_FILES['avatar-3']['name'];

$date_array = explode(".",$_FILES['avatar-3']['name']);
$File1=$date_array[0];
$File2=$date_array[1];

$size = getimagesize($_FILES['avatar-3']['tmp_name']);
$widths = $size[0];
$heights = $size[1];

if ($widths > _I_LOGO_SIG_W || $heights > _I_LOGO_SIG_H) {
if ((strchr($_FILES['avatar-3']['name'],".")==".JPG") ||(strchr($_FILES['avatar-3']['name'],".")==".jpg")){
...
//				echo str_replace($_SERVER['DOCUMENT_ROOT'],'',$targetFile);
				...
				} else if ((strchr($_FILES['avatar-3']['name'],".")==".GIF") || (strchr($_FILES['avatar-3']['name'],".")==".gift")){
				move_uploaded_file($tempFile,$targetFile);
//				echo str_replace($_SERVER['DOCUMENT_ROOT'],'',$targetFile);
				$original_image = $targetFile;
				$width = _I_LOGO_SIG_W ;
				...
				} else if ((strchr($_FILES['avatar-3']['name'],".")==".PNG") || (strchr($_FILES['avatar-3']['name'],".")==".png")){
				move_uploaded_file($tempFile,$targetFile);
	//			echo str_replace($_SERVER['DOCUMENT_ROOT'],'',$targetFile);
				$original_image = $targetFile;
				$width = _I_LOGO_SIG_W ;
	...
	//			echo $TIMESTAMP."_".$_FILES['avatar-3']['name'];
...
//	echo json_encode(['success'=>'Success resize']);
	...
	//$targetFolder . $_FILES['avatar-3']['name'];
