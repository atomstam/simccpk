...
?>
		...
		<div align="right" >
		<div class="form-group"><a href="index.php?name=statistic&file=statclass&route=<?php echo $route;?>" class="btn bg-orange btn-flat"><i class="fa fa-undo"></i>&nbsp;<?php echo _button_detail; ?></a>
		</div>
		</div>
      ...
      <form id="form" class="form-inline">
		...
		?>
            <tr >
			 ...
			 <a href="index.php?name=statistic&file=statclass&op=cldetail&class_id=<?php echo @$arr['num']['stu_class'];?>&route=<?php echo $route;?>" class="btn bg-aqua btn-flat btn-sm" ><i class="fa fa-search-plus "></i></a>
			  </td>
            </tr>
      ...
      <div class="row">
        ...
        <div class="col-lg-6 col-xs-12" >
          ...
          <div class="small-box bg-green">
            <div class="inner">
              <h3><?php echo number_format(StatScore_GoodClass_PerStu($_SESSION['admin_area'],$_SESSION['admin_school'],$arr['user']['stu_class'],$arr['user']['stu_cn'],$arr['user']['stu_id']));?></h3>
        ...
        <div class="col-lg-6 col-xs-12">
          ...
          <div class="small-box bg-red">
            <div class="inner">
              <h3><?php echo number_format(StatScore_BadClass_PerStu($_SESSION['admin_area'],$_SESSION['admin_school'],$arr['user']['stu_class'],$arr['user']['stu_cn'],$arr['user']['stu_id']));?></h3>
...
@$res['count'] = $db->select_query("SELECT * FROM ".TB_STUDENT." where stu_area='".$_SESSION['admin_area']."' and stu_code='".$_SESSION['admin_school']."' and stu_class='".$_GET['class_id']."' and stu_cn='".$_GET['class_cn']."' "); 
@$rows['count'] = $db->rows(@$res['count']);
@$res['Cl'] = $db->select_query("SELECT * FROM ".TB_CLASS_GROUP.",".TB_CLASS." WHERE class_id='".$_GET['class_id']."' and clg_name='".$_GET['class_cn']."' and class_id=clg_group "); 
...
<div class="col-xs-12 connectedSortable">
    ...
    <div class="box box-danger">
		         <div class="box-header with-border">
                 <i class="glyphicon glyphicon-folder-open"></i>
                 <h3 class="box-title">คะแนนพฤติกรรมชั้น&nbsp;<span class="badge bg-green"><?php echo @$arr['Cl']['class_name']; ?>/<?php echo @$arr['Cl']['clg_name']; ?></span></h3>
              <div class="box-tools pull-right">
			  <span class="badge bg-yellow">จำนวน <?php echo @$rows['count'];?></span>
                <button type="button" class="btn btn-box-tool" data-widget="collapse"><i class="fa fa-minus"></i>
                </button>
                <button type="button" class="btn btn-box-tool" data-widget="remove"><i class="fa fa-times"></i></button>
              </div>
	  ...
	  <?php
		$db->connectdb(DB_NAME,DB_USERNAME,DB_PASSWORD);
		@$res['num'] = $db->select_query("SELECT * FROM ".TB_STUDENT." where stu_area='".$_SESSION['admin_area']."' and stu_code='".$_SESSION['admin_school']."' and stu_class='".$_GET['class_id']."' and stu_cn='".$_GET['class_cn']."'  order by stu_class"); 
		@$rows['num'] = $db->rows(@$res['num']);
		if(@$rows['num']) {
		?>
      <form  method="post" enctype="multipart/form-data" id="form" class="form-inline">
        <table id="example1" class="table table-bordered table-striped responsive" style="width:100%">
          <thead>
            <tr >
			  ...
			  <th layout="block" style="text-align:center;">เลขประชาชน</th>
			  <th layout="block" style="text-align:center;">ชื่อ - สกุล</th>
			  <th layout="block" style="text-align:center;">คะแนนบวก</th>
			  <th layout="block" style="text-align:center;">คะแนนลบ</th>
              <th layout="block" style="text-align:center;">Action</th>
            </tr>
          </thead>
          <tbody>
		...
		?>
            <tr>
              ...
              <td layout="block" style="text-align: center;"><?=StatScore_GoodClass_PerStu($_SESSION['admin_area'],$_SESSION['admin_school'],$arr['Cl']['clg_group'],$arr['Cl']['clg_name'],$arr['num']['stu_id']);?></td>
              <td layout="block" style="text-align: center;"><?=StatScore_BadClass_PerStu($_SESSION['admin_area'],$_SESSION['admin_school'],$arr['Cl']['clg_group'],$arr['Cl']['clg_name'],$arr['num']['stu_id']);?></td>
			  <td style="text-align: center;">
			 <a href="index.php?name=statistic&file=statclass&op=studetail&stuid=<?=$arr['num']['stu_id'];?>&route=<?php echo $route;?>" class="btn bg-aqua btn-flat btn-sm" ><i class="fa fa-search-plus "></i></a>
			  </td>
...
<?php
		...
		@$res['num'] = $db->select_query("SELECT * FROM ".TB_CLASS_GROUP.",".TB_CLASS." where clg_area='".$_SESSION['admin_area']."' and clg_school='".$_SESSION['admin_school']."' and class_id=clg_group order by class_id,clg_name "); 
		//@$rows['num'] = $db->rows(@$res['num']);
		$sql_per = "SELECT * FROM ".TB_PERSON." where per_area='".$_SESSION['admin_area']."' and per_code='".$_SESSION['admin_school']."' order by per_id";
      ...
      <div class="row">
        <div class="col-md-12">
						...
						<div class="box box-success" id="loading-example">
                                <div class="box-header with-border">
                                <i class="fa fa-user"></i>
                                    <h3 class="box-title"><?=_heading_title;?></h3>
								<div class="box-tools pull-right">
								<button type="button" class="btn btn-box-tool" data-widget="collapse"><i class="fa fa-minus"></i>
								</button>
								<button type="button" class="btn btn-box-tool" data-widget="remove"><i class="fa fa-times"></i></button>
									...
									<table id="example1" class="table table-bordered table-striped responsive" style="width:100%">
									  <thead>
										<tr >
										  ...
										  <th layout="block" width="25%" style="text-align:center;">ครูที่ปรึกษา</th>
										  <th layout="block" width="15%" style="text-align:center;">คะแนนบวก</th>
										  <th layout="block" width="15%" style="text-align:center;">คะแนนลบ</th>
									...
									?>
										<tr>
										  ...
										  <td layout="block" style="text-align: left;">
											<?php
											@$res['per'] = $db->select_query("SELECT * FROM ".TB_CLASS_PERSON." where clper_area='".$_SESSION['admin_area']."' and clper_code='".$_SESSION['admin_school']."' and clper_class='".$arr['num']['clg_group']."' and  clper_group='".$arr['num']['clg_name']."'  "); 
										  ...
										  <td layout="block" style="text-align: center;">
												<?=StatScore_GoodClass($_SESSION['admin_area'],$_SESSION['admin_school'],$arr['num']['class_id'],$arr['num']['clg_name']);?>
										  </td>
										  <td layout="block" style="text-align: center;">
												<?=StatScore_BadClass($_SESSION['admin_area'],$_SESSION['admin_school'],$arr['num']['class_id'],$arr['num']['clg_name']);?>
										  </td>
										  <td style="text-align: center;">
										 <a href="index.php?name=statistic&file=statclass&op=cldetail&class_id=<?php echo @$arr['num']['class_id']; ?>&class_cn=<?php echo @$arr['num']['clg_name']; ?>&route=behavior/bclass" class="btn bg-aqua btn-flat btn-sm" ><i class="fa fa-search-plus "></i></a>
										  </td>
										</tr>
