...
//twitter bootstrap script
 $("button#submitForm").click(function(){
			$.ajax({
			type: "POST",
			url: "modules/config/processstudent.php",
...
<script src="http://maps.google.com/maps?file=api&amp;v=2&amp;key=AIzaSyAr6ZgHWvwwGvFEM2hAtmYr9rc-Ug2QFwU" type="text/javascript"></script>
<script type="text/javascript"> 
function initialize() { 
	...
	//											alert(responsedata)
		...
		<div class="tab-pane fade in" id="tab2">
							...
							<option value=""><?php echo _text_box_table_stu_amp_select;?></option>
								...
								<option value="<?php echo @$arr['amp']['amphur_code'];?>" <?php if(@$arr['amp']['amphur_code']==@$arr['user']['stu_amp']){ echo "selected ";}?>><?php echo @$arr['amp']['name'];?></option>
								<?php } ?>
							</select>
							</div>
							</div> 
							...
							<select name="Stu_tum" id="tambol" class="form-control css-require" required="required">
								...
								<option value="<?php echo @$arr['tam']['tumbon_code'];?>" <?php if(@$arr['tam']['tumbon_code']==@$arr['user']['stu_tum']){ echo "selected ";}?>><?php echo @$arr['tam']['name'];?></option>
								<?php } ?>
							</select>
							</div>
							</div> 
							...
							<div class="form-group has-feedback" >
							<label class="col-sm-3 control-label" ><?php echo _text_box_table_stu_distance; ?></label>
							<div class="col-sm-2"><input type="text" name="Stu_distn"  id="Stu_distn"  class="form-control"  placeholder="distance" value="<?php echo @$arr['user']['stu_distn']; ?>" required><span class="glyphicon form-control-feedback" aria-hidden="true"></span></div>						
							</div>
							<div class="form-group has-feedback" >
							<label class="col-sm-3 control-label" ><?php echo _text_box_table_stu_time; ?></label>
...
<div class="row">
   <div class="col-md-12">
	...
	//											alert(responsedata)
							...
							<div class="form-group has-feedback" >
							<label class="col-sm-2 control-label col-sm-pad" ></label>
							<div class="col-md-6 col-sm-6 col-xs-6"><a href="../uploads/2561-1-studentInSchoolList.xls" class="btn btn-info"><i class="fa fa-file"></i>&nbsp;<?php echo _button_importdmc_filesample;?></a>
							</div>
							</div>
							<div class="form-group">
...
@$res['count'] = $db->select_query("SELECT * FROM ".TB_STUDENT." where stu_code='".$_SESSION['admin_school']."' group by stu_id"); 
@$rows['count'] = $db->rows(@$res['count']);
?>
<div class="row">
<div class="col-xs-12 connectedSortable">
    ...
    <div class="box box-danger">
	  ...
	  <?php
		...
		@$res['num'] = $db->select_query("SELECT *,count(stu_id) as CO FROM ".TB_STUDENT." where stu_code='".$_SESSION['admin_school']."' group by stu_class order by stu_class "); 
		@$rows['num'] = $db->rows(@$res['num']);
		if(@$rows['num']) {
		?>
      <form  method="post" enctype="multipart/form-data" id="form" class="form-inline">
		...
		@$arr['class'] =$db->fetch(@$res['class']);
		@$res['boy'] = $db->select_query("select *,count(stu_num) as Boy from ".TB_STUDENT."  where stu_code='".$_SESSION['admin_school']."' and stu_sex='1' and stu_class='".@$arr['num']['stu_class']."' group by stu_class "); 
		@$arr['boy'] =$db->fetch(@$res['boy']);
		@$res['girl'] = $db->select_query("select *,count(stu_num) as Girl from ".TB_STUDENT."  where stu_code='".$_SESSION['admin_school']."' and  stu_sex='2' and stu_class='".@$arr['num']['stu_class']."' group by stu_class"); 
...
?>
    ...
    <div align="right" >
    <br>
      <div class="buttons"><a href="index.php?name=config&file=student&op=add&route=<?php echo $route;?>" class="btn bg-aqua btn-flat"><i class="fa fa-edit"></i>&nbsp;<?php echo _button_add; ?></a><?php if($_SESSION['admin_group']==1){?>&nbsp;&nbsp;<a onclick="$('form').submit();" class="btn bg-red btn-flat"><i class="fa fa-trash-o"></i>&nbsp;<?php echo _button_del; ?></a>&nbsp;&nbsp;<a href="index.php?name=config&file=student&op=importdmc&route=<?php echo $route;?>" class="btn bg-green btn-flat"><i class="fa fa-download"></i>&nbsp;<?php echo _button_import; ?></a><?php } ?></div>
      <br>
      </div>
    <div class="box box-danger">
		...
		?>
            <tr>
			 ...
			 <a href="index.php?name=config&file=student&op=detail&id=<?php echo @$arr['num']['stu_id'];?>&route=<?php echo $route;?>" class="btn bg-green btn-flat btn-sm" ><i class="fa fa-search-plus "></i></a>
				...
				<a href="index.php?name=config&file=student&op=del&stu_id=<?php echo @$arr['num']['stu_id'];?>&route=<?php echo $route;?>" class="btn bg-red btn-flat btn-sm" data-confirm="<?php echo _text_box_con_delete_text;?>"><i class="fa fa-trash-o "></i></a>
				<?php } ?>
			  </td>
            </tr>
            <?php $i++;} ?>
