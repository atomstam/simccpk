...
if($op=='del'){
		...
		$del .=$db->del(TB_ENT," go_area='".$_SESSION['admin_area']."' and go_code='".$_SESSION['admin_school']."' and go_id='".$_GET['go_id']."' ");
		$del .=$db->del(TB_ENTTAIL," got_area='".$_SESSION['admin_area']."' and got_code='".$_SESSION['admin_school']."' and got_tail='".$_GET['go_id']."' ");
		if($del){
...
if($op=='delall'){
		...
		while(list($key, $value) = each ($_POST['selected'])){
			...
			$del .=$db->del(TB_ENT," go_area='".$_SESSION['admin_area']."' and go_code='".$_SESSION['admin_school']."' and go_id='".$value."' ");
			$del .=$db->del(TB_ENTTAIL," got_area='".$_SESSION['admin_area']."' and got_code='".$_SESSION['admin_school']."' and got_tail='".$value."' ");
//			$db->closedb ();
		}
		if($del){
		$success=_text_report_del_ok;
...
<?php
//<form action="index.php?name=config&file=ent&op=add&action=add&route=" method="post" //enctype="multipart/form-data" id="form" role="form" data-toggle="validator" class="form-horizontal bootstrap-validator-form" >
?>
...
//twitter bootstrap script
 $("button#submitForm").click(function(){
			$.ajax({
			type: "POST",
			url: "modules/config/processent.php",
...
//                 $("#thanks").html(msg.message),
				 ...
				 $("#error").hide();
				 setTimeout(function() {
  				 window.location='index.php?name=config&file=ent&route=<?php echo $route;?>';
				}, 1000);
			} else {
//                $("#error").html(msg.message),
	...
	//		$("#form-content").modal('hide'); 
		...
		<div align="right" ><div class="form-group"><button class="btn bg-green btn-flat" type="submit" id="submitForm" name="submitForm"><i class="fa fa-save"></i>&nbsp;<?php echo _button_save; ?></button>&nbsp;&nbsp;<a href="index.php?name=config&file=ent&route=<?php echo $route;?>" class="btn bg-red btn-flat"><i class="fa fa-undo"></i>&nbsp;<?php echo _button_reset; ?></a>
		</div></div>
				<form method="post" enctype="multipart/form-data" id="formAdd" role="formAdd" data-toggle="validator" class="form-horizontal bootstrap-validator-form" >
					    <div class="box box-success" id="loading-example">
							...
							<label class="col-sm-3 control-label" ><?php echo _text_box_table_ent_name; ?></label>
...
@$res['class'] = $db->select_query("SELECT * FROM ".TB_ENT." WHERE go_area='".$_SESSION['admin_area']."' and go_code='".$_SESSION['admin_school']."' and go_id='".$_GET['go_id']."'"); 
		...
		<div align="right" ><div class="form-group"><button class="btn bg-green btn-flat" type="submit" id="submitForm" name="submitForm"><i class="fa fa-save"></i>&nbsp;<?php echo _button_save; ?></button>&nbsp;&nbsp;<a href="index.php?name=config&file=ent&route=<?php echo $route;?>" class="btn bg-red btn-flat"><i class="fa fa-undo"></i>&nbsp;<?php echo _button_reset; ?></a>
				...
				<form method="post" enctype="multipart/form-data" id="formEdit" role="formEdit" data-toggle="validator" class="form-horizontal bootstrap-validator-form" >
					    <div class="box box-info" id="loading-example">
							...
							<div class="form-group has-feedback" >
							<label class="col-sm-3 control-label" ><?php echo _text_box_table_ent_name; ?></label>
							<div class="col-sm-6"><input type="text" name="Class_name"  class="form-control"  value="<?php echo @$arr['class']['go_name'];?>" placeholder="class1" required><span class="glyphicon form-control-feedback" aria-hidden="true"></span></div>
							</div>
							<div class="form-group">
							<div class="col-sm-4" ><input type="hidden" name="OP"  value="Edit"><input name="CLID" type="hidden" value="<?php echo @$arr['class']['go_id'];?>">
							<br>
							</div>
							</div>
...
<div class="col-xs-12 connectedSortable">
    ...
    <div align="right" >
    <br>
      <div class="buttons"><a href="index.php?name=config&file=ent&op=add&route=<?php echo $route;?>" class="btn bg-aqua btn-flat"><i class="fa fa-edit"></i>&nbsp;<?php echo _button_add; ?></a><?php if($_SESSION['admin_group']==1){ ?>&nbsp;<a onclick="$('form').submit();" class="btn bg-red btn-flat"><i class="fa fa-trash-o"></i>&nbsp;<?php echo _button_del; ?></a><?php } ?></div>
      <br>
      </div>
    <div class="box box-danger">
	  ...
	  <?php
		...
		@$res['num'] = $db->select_query("SELECT * FROM ".TB_ENT." where go_area='".$_SESSION['admin_area']."' and go_code='".$_SESSION['admin_school']."' order by go_id"); 
		@$rows['num'] = $db->rows(@$res['num']);
		if(@$rows['num']) {
		?>
      <form action="index.php?name=config&file=ent&op=delall&route=<?php echo $route;?>" method="post" enctype="multipart/form-data" id="form" class="form-inline">
		...
		@$res['stu'] = $db->select_query("SELECT * FROM ".TB_ENTTAIL." WHERE got_area='".$_SESSION['admin_area']."' and got_code='".$_SESSION['admin_school']."' and got_id='".@$arr['num']['go_id']."' "); 
		@$rowsSTU=$db->rows(@$res['stu']);
		?>
            <tr>
              <td style="text-align: center;"><?php if($_SESSION['admin_group']==1){ ?><input type="checkbox" name="selected[]" value="<?php echo @$arr['num']['go_id']; ?>" class="selector flat"/><?php } else { echo $i;} ?></td>
              <td layout="block" style="text-align: left;"><?php echo @$arr['num']['go_name'];?></td>
              <td layout="block" style="text-align: right;"><?php echo @$rowsSTU;?></td>
			  <td style="text-align: center;">
<?php
/*
				
			 <a href="#" data-toggle="modal" data-target="#myModal" class="btn bg-green btn-flat btn-sm" id="<?php echo @$arr['num']['go_id']; ?>"><i class="fa fa-search-plus "></i></a>
*/
?>
				<a href="index.php?name=config&file=ent&op=edit&go_id=<?php echo @$arr['num']['go_id']; ?>&route=<?php echo $route;?>" class="btn bg-aqua btn-flat btn-sm"><i class="fa fa-edit "></i></a>
				<?php if($_SESSION['admin_group']==1){ ?>
				<a href="index.php?name=config&file=ent&op=del&go_id=<?php echo @$arr['num']['go_id'];?>&route=<?php echo $route;?>" class="btn bg-red btn-flat btn-sm" data-confirm="<?php echo _text_box_con_delete_text;?>"><i class="fa fa-trash-o "></i></a>
				<?php } ?>
			  </td>
            </tr>
...
$(document).ready(function() { 
    $('.bg-green').click(function(e){//Modal Event
		...
		data : 'go_id='+ id, //Here pass id via 
