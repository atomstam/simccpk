...
if($op=='del'){
		...
		$del .=$db->del(TB_BEST," bt_id='".$_GET['bt_id']."' ");
		if($del){
...
if($op=='delall'){
		...
		while(list($key, $value) = each ($_POST['selected'])){
			...
			$del .=$db->del(TB_BEST," bt_id='".$value."' ");
//			$db->closedb ();
		}
		if($del){
		$success=_text_report_del_ok;
...
<?php
//<form action="index.php?name=config&file=best&op=add&action=add&route=" method="post" //enctype="multipart/form-data" id="form" role="form" data-toggle="validator" class="form-horizontal bootstrap-validator-form" >
?>
...
//twitter bootstrap script
 $("button#submitForm").click(function(){
			$.ajax({
			type: "POST",
			url: "modules/config/processbest.php",
...
//                 $("#thanks").html(msg.message),
				 ...
				 $("#error").hide();
				 setTimeout(function() {
  				 window.location='index.php?name=config&file=best&route=<?php echo $route;?>';
				}, 1000);
			} else {
//                $("#error").html(msg.message),
	...
	//		$("#form-content").modal('hide'); 
		...
		<div align="right" ><div class="form-group"><button class="btn bg-green btn-flat" type="submit" id="submitForm" name="submitForm"><i class="fa fa-save"></i>&nbsp;<?php echo _button_save; ?></button>&nbsp;&nbsp;<a href="index.php?name=config&file=best&route=<?php echo $route;?>" class="btn bg-red btn-flat"><i class="fa fa-undo"></i>&nbsp;<?php echo _button_reset; ?></a>
		</div></div>
				<form method="post" enctype="multipart/form-data" id="formAdd" role="formAdd" data-toggle="validator" class="form-horizontal bootstrap-validator-form" >
					    <div class="box box-success" id="loading-example">
							...
							<label class="col-sm-3 control-label" ><?php echo _text_box_table_best_id; ?></label>
							<div class="col-sm-2"><input type="text" name="bt_id"  class="form-control"  placeholder="m1" required><span class="glyphicon form-control-feedback" aria-hidden="true"></span></div>
							</div>-->
							<div class="form-group has-feedback" >
							<label class="col-sm-3 control-label" ><?php echo _text_box_table_best_name; ?></label>
...
@$res['class'] = $db->select_query("SELECT * FROM ".TB_BEST." WHERE bt_id='".$_GET['bt_id']."'"); 
		...
		<div align="right" ><div class="form-group"><button class="btn bg-green btn-flat" type="submit" id="submitForm" name="submitForm"><i class="fa fa-save"></i>&nbsp;<?php echo _button_save; ?></button>&nbsp;&nbsp;<a href="index.php?name=config&file=best&route=<?php echo $route;?>" class="btn bg-red btn-flat"><i class="fa fa-undo"></i>&nbsp;<?php echo _button_reset; ?></a>
				...
				<form method="post" enctype="multipart/form-data" id="formEdit" role="formEdit" data-toggle="validator" class="form-horizontal bootstrap-validator-form" >
					    <div class="box box-info" id="loading-example">
							...
							<!--<div class="form-group has-feedback" >
							<label class="col-sm-3 control-label" ><?php echo _text_box_table_best_id; ?></label>
							<div class="col-sm-2"><input type="text" name="bt_id"  class="form-control"  value="<?php echo @$arr['class']['bt_id'];?>" placeholder="m1" readonly><span class="glyphicon form-control-feedback" aria-hidden="true"></span></div>
							</div>-->
							<div class="form-group has-feedback" >
							<label class="col-sm-3 control-label" ><?php echo _text_box_table_best_name; ?></label>
							<div class="col-sm-6"><input type="text" name="Class_name"  class="form-control"  value="<?php echo @$arr['class']['bt_name'];?>" placeholder="class1" required><span class="glyphicon form-control-feedback" aria-hidden="true"></span></div>
							</div>
							<div class="form-group">
							<div class="col-sm-4" ><input type="hidden" name="OP"  value="Edit"><input name="CLID" type="hidden" value="<?php echo @$arr['class']['bt_id'];?>">
							<br>
							</div>
							</div>
...
<!--    <div align="right" >
    <br>
      <div class="buttons"><a href="index.php?name=config&file=best&op=add&route=<?php echo $route;?>" class="btn bg-aqua btn-flat"><i class="fa fa-edit"></i>&nbsp;<?php echo _button_add; ?></a>&nbsp;&nbsp;<a onclick="$('form').submit();" class="btn bg-red btn-flat"><i class="fa fa-trash-o"></i>&nbsp;<?php echo _button_del; ?></a></div>
      <br>
      </div> -->
    <div class="box box-danger">
	  ...
	  <?php
		...
		if(@$rows['num']) {
		?>
      <form action="index.php?name=config&file=best&op=delall&route=<?php echo $route;?>" method="post" enctype="multipart/form-data" id="form" class="form-inline">
        <table id="example1" class="table table-bordered table-striped responsive" style="width:100%">
		 ...
		 <th width="1" style="text-align: center;">#</th>
              <th layout="block" style="text-align:center;" ><?php echo _text_box_table_best_id; ?></th>
		...
		@$res['stu'] = $db->select_query("SELECT * FROM ".TB_BESTTAIL." WHERE btail_area='".$_SESSION['admin_area']."' and btail_code='".$_SESSION['admin_school']."' and btail_id='".@$arr['num']['bt_id']."' "); 
		@$rowsSTU=$db->rows(@$res['stu']);
		?>
            <tr>
 <!--             <td style="text-align: center;"><input type="checkbox" name="selected[]" value="<?php echo @$arr['num']['bt_id']; ?>" class="selector flat"/></td> -->
 <td style="text-align: center;"><?php echo $i;?></td>
              <td layout="block" style="text-align: center;"><?php echo @$arr['num']['bt_id']; ?></td>
...
/*
			 ...
			 <a href="#" data-toggle="modal" data-target="#myModal" class="btn bg-green btn-flat btn-sm" id="<?php echo @$arr['num']['bt_id']; ?>"><i class="fa fa-search-plus "></i></a>

				<a href="index.php?name=config&file=best&op=edit&bt_id=<?php echo @$arr['num']['bt_id']; ?>&route=<?php echo $route;?>" class="btn bg-aqua btn-flat btn-sm"><i class="fa fa-edit "></i></a>
				<a href="index.php?name=config&file=best&op=del&bt_id=<?php echo @$arr['num']['bt_id'];?>&route=<?php echo $route;?>" class="btn bg-red btn-flat btn-sm" data-confirm="<?php echo _text_box_con_delete_text;?>"><i class="fa fa-trash-o "></i></a>
*/
?>
			  </td> -->
            </tr>
...
$(document).ready(function() { 
    $('.bg-green').click(function(e){//Modal Event
		...
		data : 'bt_id='+ id, //Here pass id via 
