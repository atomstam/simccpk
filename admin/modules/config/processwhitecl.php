...
if($_POST['OP']=='Add'){
	if( $_POST['wh_id'] !='' && $_POST['wh_point'] !='' && $_POST['wh_name'] !='' ){
//		$Avatar=$_FILES['avatar-1']['name'];
		$db->connectdb(DB_NAME,DB_USERNAME,DB_PASSWORD);
		$add .=$db->add_db(TB_WHITECLASS,array(
			"wh_id"=>"".$_POST['wh_id']."",
			"wh_name"=>"".$_POST['wh_name']."",
			"wh_point"=>"".$_POST['wh_point']."",
		));
	} else {
		$add .='';
	}
...
if($_POST['OP']=='Edit'){
	if( $_POST['wh_id'] !='' && $_POST['wh_point'] !='' && $_POST['wh_name'] !='' ){//		$Avatar=$_FILES['avatar-1']['name'];
		$db->connectdb(DB_NAME,DB_USERNAME,DB_PASSWORD);
		$edit .=$db->update_db(TB_WHITECLASS,array(
			"wh_name"=>"".$_POST['wh_name']."",
			"wh_point"=>"".$_POST['wh_point']."",
		)," wh_id=".$_POST['CLID']." ");
	} else {
