...
//			$Birthday=$YY."-".$MM."-".$DD;
			...
			if($filesop[4]=='ม.1'){
				$M='m1';
			} else if($filesop[4]=='ม.2'){
				$M='m2';
			} else if($filesop[4]=='ม.3'){
				$M='m3';
			} else if($filesop[4]=='ม.4'){
				$M='m4';
			} else if($filesop[4]=='ม.5'){
				$M='m5';
			} else if($filesop[4]=='ม.6'){
				$M='m6';
			} 
			$Father=$filesop[27].$filesop[28]." ".$filesop[29];
			$Marther=$filesop[33].$filesop[34]." ".$filesop[35];
			$Other=$filesop[40].$filesop[41]." ".$filesop[42];

			@$res['prov'] = $db->select_query("SELECT * FROM ".TB_PROVINCE." where name like '%".$filesop[51]."%' "); 
			@$arr['prov'] = $db->fetch(@$res['prov']);
			$Prov=@$arr['prov']['id'];
			if(!empty($Prov)){
				@$res['amp'] = $db->select_query("SELECT * FROM ".TB_AMPHUR." where provinceID='".$Prov."' and name like '%".$filesop[50]."%' "); 
				@$arr['amp'] = $db->fetch(@$res['amp']);
				$Amp=@$arr['amp']['id'];
				if(!empty($Amp)){
					@$res['tam'] = $db->select_query("SELECT * FROM ".TB_TUMBON." where amphurID='".$Amp."' and name like '%".$filesop[49]."%' "); 
					@$arr['tam'] = $db->fetch(@$res['tam']);
					$Tam=@$arr['tam']['id'];
				}
			}
			if($filesop[77]=='เด็กยากจน'){
				$Best=1;
			} else {
				$Best=0;
			}
		...
		if($i>1) {
			@$res['stu'] = $db->select_query("SELECT * FROM ".TB_STUDENT." where stu_code='".$_SESSION['admin_school']."' and stu_id='".$filesop[6]."' "); 
			...
			$import .=$db->add_db(TB_STUDENT,array(
				...
				"stu_birth"=>"".$Birthday."",
				"stu_father"=>"".$Father."",
				"stu_fphone"=>"".$filesop[31]."",
//				"stu_femail"=>"".$_POST['Stu_femail']."",
//				"stu_fstatus"=>"".$_POST['Stu_fstatus']."",
				"stu_marther"=>"".$Marther."",
				"stu_mphone"=>"".$filesop[37]."",
//				"stu_memail"=>"".$_POST['Stu_memail']."",
//				"stu_mstatus"=>"".$_POST['Stu_mstatus']."",
				"stu_status"=>"".$filesop[25]."",
				"stu_orther"=>"".$Other."",
				"stu_ophone"=>"".$filesop[44]."",
//				"stu_oemail"=>"".$_POST['Stu_oemail']."",
				"stu_ostatus"=>"".$filesop[38]."",
				"stu_add"=>"".$filesop[55]."",
				"stu_group"=>"".$filesop[56]."",
//				"stu_ban"=>"".$_POST['Stu_ban']."",
				"stu_tum"=>"".$Tam."",
				"stu_amp"=>"".$Amp."",
				"stu_prov"=>"".$Prov."",
				"stu_post"=>"".$filesop[52]."",
				"stu_best"=>"".$Best."",
				"stu_blood"=>"".$filesop[16]."",
				"stu_fpid"=>"".$filesop[26]."",
				"stu_mpid"=>"".$filesop[32]."",
				"stu_opid"=>"".$filesop[37]."",
				"stu_weight"=>"".$filesop[63]."",
				"stu_hight"=>"".$filesop[64]."",
				"stu_distance"=>"".$filesop[73]."",
				"stu_time"=>"".$filesop[75]."",
				"stu_travel"=>"".$filesop[76]."",
//				"stu_pic"=>"".$_POST['Icon']."",
//				"stu_lat"=>"".$_POST['lat_value']."",
//				"stu_long"=>"".$_POST['lon_value']."" 
			));
...
//				"stu_lat"=>"".$_POST['lat_value']."",
//				"stu_long"=>"".$_POST['lon_value']."" 
			)," stu_area='".$_SESSION['admin_area']."' and stu_code='".$_SESSION['admin_school']."' and stu_id=".$filesop[6]." ");
			}
		}
		$i++;
		}
...
//			$Birthday=$YY."-".$MM."-".$DD;
			...
			if($data->sheets[$i][cells][$j][7]=='ช'){
				$Sex=1;
			} else {
				$Sex=2;
			}

			if($data->sheets[$i][cells][$j][4]=='ม.1'){
				$M='m1';
			} else if($data->sheets[$i][cells][$j][4]=='ม.2'){
				$M='m2';
			} else if($data->sheets[$i][cells][$j][4]=='ม.3'){
				$M='m3';
			} else if($data->sheets[$i][cells][$j][4]=='ม.4'){
				$M='m4';
			} else if($data->sheets[$i][cells][$j][4]=='ม.5'){
				$M='m5';
			} else if($data->sheets[$i][cells][$j][4]=='ม.6'){
				$M='m6';
			} 
			...
			if($data->sheets[$i][cells][$j][77]=='เด็กยากจน'){
				$Best=1;
			} else {
				$Best=0;
			}
