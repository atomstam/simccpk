...
if($op=='cldetail' and $action=='' ){
		...
		@$res['numchkclass'] = $db->select_query("SELECT * FROM ".TB_CHK_CHMCLASS." WHERE chkm_area='".$_SESSION['admin_area']."' and chkm_code='".$_SESSION['admin_school']."' and DATE_FORMAT(chkm_date, '%Y-%m') like '%".$_GET['mo_id']."%'  group by chkm_class,chkm_cn"); 
...
<div class="col-xs-12 connectedSortable">
		...
		<div class="box box-danger">
                 ...
                 <h3 class="box-title">การเข้าเรียน&nbsp;<span class="badge bg-green"><?php echo $month[$get_month]." ".$year; ?></span></h3>
      ...
      <form  method="post" enctype="multipart/form-data" id="form" class="form-inline">
		...
		$year = $CC[0]+543;
		$date=$CC[2];
		@$res['chkclass'] = $db->select_query("SELECT * FROM ".TB_CHK_CHMCLASS." WHERE chkm_area='".$_SESSION['admin_area']."' and chkm_code='".$_SESSION['admin_school']."' and DATE_FORMAT(chkm_date, '%Y-%m-%d') like '%".$arr['num']['c_date']."%' group by chkm_class,chkm_cn"); 
		...
		?>
            <tr>
			 ...
			 <a href="index.php?name=check&file=mclass&op=datedetail&moid=<?php echo @$arr['num']['c_date'];?>&gr=<?php echo $_GET['gr'];?>&route=<?php echo $route;?>" class="btn bg-aqua btn-flat btn-sm" ><i class="fa fa-search-plus "></i></a>
			  </td>
...
<?php
		...
		@$res['numchkclass'] = $db->select_query("SELECT * FROM ".TB_CHK_CHMCLASS." WHERE chkm_area='".$_SESSION['admin_area']."' and chkm_code='".$_SESSION['admin_school']."' and DATE_FORMAT(chkm_date, '%Y-%m-%d') like '%".$_GET['moid']."%' and gr='".$_GET['gr']."' group by chkm_class,chkm_cn"); 
		@$NumRowChkclass = $db->rows(@$res['numchkclass']);
...
<div class="col-xs-12 connectedSortable">
    ...
    <div class="box box-danger">
		         <div class="box-header with-border">
                 <i class="glyphicon glyphicon-folder-open"></i>
                 <h3 class="box-title">ตรวจสอบการบันทึกการเข้าเรียน&nbsp;<span class="badge bg-green"><?php echo DateThai($_GET['moid']); ?></span></h3>
              <div class="box-tools pull-right">
		...
		?>
            <tr>
              ...
              <td layout="block" style="text-align: center;">
				<?php
				//	SELECT * FROM `web_chkm_chclass` WHERE chkm_area='101726' and chkm_code='44012028' and chkm_class='m2' and chkm_cn='1' and chkm_gr='1' and DATE_FORMAT(chkm_date, '%Y-%m-%d') like '%2022-06-06%' 
				$res['chk'] = $db->select_query("SELECT * FROM ".TB_CHK_CHMCLASS." WHERE chkm_area='".$_SESSION['admin_area']."' and chkm_code='".$_SESSION['admin_school']."' and chkm_class='".$arr['num']['class_id']."' and chkm_cn='".$arr['num']['clg_name']."' and DATE_FORMAT(chkm_date, '%Y-%m-%d') like '%".$_GET['moid']."%' "); 
				@$arr['chk'] = $db->fetch(@$res['chk']);
				//echo $arr['chk']['chkm_id'];
				if($arr['chk']['chkm_id']){
			  ...
			  <td style="text-align: center;">
			 <a href="index.php?name=academic&file=mclass&mo_id=<?php echo $_GET['moid'];?>&route=academic/bclass" class="btn bg-aqua btn-flat btn-sm" ><i class="fa fa-search-plus "></i></a>
			  </td>
            </tr>
...
?>
					    ...
					    <div class="box box-success" id="loading-example">
                                ...
                                <i class="fa fa-user"></i>
                                    <h3 class="box-title">ตรวจสอบการบันทึกการเข้าเรียน</h3>
								<div class="box-tools pull-right">
								<button type="button" class="btn btn-box-tool" data-widget="collapse"><i class="fa fa-minus"></i>
								</button>
								<button type="button" class="btn btn-box-tool" data-widget="remove"><i class="fa fa-times"></i></button>
				...
				<div class="form-group">
				  ...
				  <form  method="post" enctype="multipart/form-data" id="form" class="form-inline">
					...
					$get_month = $DD[1];
					$year = $DD[0]+543;
					@$res['chkclass'] = $db->select_query("SELECT *,count(chkm_id) as CO  FROM ".TB_CHK_CHMCLASS." WHERE chkm_area='".$_SESSION['admin_area']."' and chkm_code='".$_SESSION['admin_school']."' and DATE_FORMAT(chkm_date, '%Y-%m') like '%".$DD1."%' group by DATE_FORMAT(chkm_date, '%Y-%m') like '%".$DD1."%'"); 
					...
					?>
						<tr>
						  ...
						  <td layout="block" style="text-align: center;">
						 <a href="index.php?name=check&file=mclass&op=cldetail&mo_id=<?php echo $DD1;?>&gr=1&route=<?php echo $route;?>" class="btn bg-aqua btn-flat btn-sm" ><i class="fa fa-search-plus "></i></a>
						  </td>
						</tr>
