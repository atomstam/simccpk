...
if($op=='del'){
		...
		$del .=$db->del(TB_PUTTAIL," pt_area='".$_SESSION['admin_area']."' and pt_code='".$_SESSION['admin_school']."' and pt_id='".$_GET['pt_id']."' ");
		if($del){
...
if($op=='cldel'){
		...
		$del .=$db->del(TB_PUTTAIL," pt_area='".$_SESSION['admin_area']."' and pt_code='".$_SESSION['admin_school']."'  and pt_id='".$_GET['pt_id']."' and pt_stu='".$_GET['stu_id']."' ");
		if($del){
		$success=_text_report_del_ok;
		} else {
		$error_warning=_text_report_del_fail;
...
if($op=='delall'){
		...
		while(list($key, $value) = each ($_POST['selected'])){
			...
			$del .=$db->del(TB_PUTTAIL," pt_area='".$_SESSION['admin_area']."' and pt_code='".$_SESSION['admin_school']."'  and pt_id='".$value."' ");
//			$db->closedb ();
		}
		if($del){
		$success=_text_report_del_ok;
...
if($op=='cldelall'){
		...
		while(list($key, $value) = each ($_POST['selected'])){
			...
			$del .=$db->del(TB_PUTTAIL," pt_area='".$_SESSION['admin_area']."' and pt_code='".$_SESSION['admin_school']."'  and pt_stu='".$value."' and pt_id='".$_GET['pt_id']."' ");
//			$db->closedb ();
		}
		if($del){
		$success=_text_report_del_ok;
		...
		<div align="right" ><div class="form-group"><button class="btn bg-green btn-flat" type="submit" id="submitForm" name="submitForm"><i class="fa fa-save"></i>&nbsp;<?php echo _button_save; ?></button>&nbsp;&nbsp;<a href="index.php?name=behavior&file=put&route=<?php echo $route;?>" class="btn bg-red btn-flat"><i class="fa fa-undo"></i>&nbsp;<?php echo _button_reset; ?></a>
				...
				<form method="post" enctype="multipart/form-data" id="formAdd" role="formAdd" data-toggle="validator" class="form-horizontal bootstrap-validator-form" >
					    <div class="box box-success" id="loading-example">
							...
							@$res['bt'] = $db->select_query("SELECT * FROM ".TB_PUT." where pu_area='".$_SESSION['admin_area']."' and pu_code='".$_SESSION['admin_school']."' ORDER BY pu_id ");
...
<?php
		...
		@$res['best'] = $db->select_query("SELECT * FROM ".TB_PUTTAIL." where pt_area='".$_SESSION['admin_area']."' and pt_code='".$_SESSION['admin_school']."' and pt_stu='".$_GET['stu_id']."'"); 
...
@$res['bad'] = $db->select_query("SELECT *,count(pt_stu) as STU FROM ".TB_PUTTAIL." where pt_area='".$_SESSION['admin_area']."' and pt_code='".$_SESSION['admin_school']."' and  pt_pu='".$_GET['pt_id']."' "); 
 @$arr['bad']= $db->fetch(@$res['bad']);
@$res['tail'] = $db->select_query("SELECT * FROM ".TB_PUT." where pu_area='".$_SESSION['admin_area']."' and pu_code='".$_SESSION['admin_school']."'  and  pu_id='".$_GET['pt_id']."' "); 
...
?>
     ...
     <button type="button" class="close" data-dismiss="alert">X</button>
      ...
      <form action="index.php?name=behavior&file=put&op=cldelall&pt_id=<?php echo $_GET['pt_id'];?>&route=<?php echo $route;?>" method="post" enctype="multipart/form-data" id="form" class="form-inline">
		...
		@$res['tail'] = $db->select_query("SELECT * FROM ".TB_PUTTAIL." where pt_area='".$_SESSION['admin_area']."' and pt_code='".$_SESSION['admin_school']."' and pt_id='".@$arr['num']['pt_id']."' "); 
		@$arr['tail'] =$db->fetch(@$res['tail']);
