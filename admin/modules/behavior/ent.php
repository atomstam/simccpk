...
if($op=='del'){
		...
		$del .=$db->del(TB_ENTTAIL," got_area='".$_SESSION['admin_area']."' and got_code='".$_SESSION['admin_school']."'  and got_tail='".$_GET['got_id']."' ");
		if($del){
...
if($op=='cldel'){
		...
		$del .=$db->del(TB_ENTTAIL," got_area='".$_SESSION['admin_area']."' and got_code='".$_SESSION['admin_school']."'  and got_id='".$_GET['got_id']."' and got_stu='".$_GET['stu_id']."' ");
		if($del){
		$success=_text_report_del_ok;
		} else {
		$error_warning=_text_report_del_fail;
...
if($op=='delall'){
		...
		while(list($key, $value) = each ($_POST['selected'])){
			...
			$del .=$db->del(TB_ENTTAIL," got_area='".$_SESSION['admin_area']."' and got_code='".$_SESSION['admin_school']."'  and got_id='".$value."' ");
//			$db->closedb ();
		}
		if($del){
		$success=_text_report_del_ok;
...
if($op=='cldelall'){
		...
		while(list($key, $value) = each ($_POST['selected'])){
			...
			$del .=$db->del(TB_ENTTAIL," got_area='".$_SESSION['admin_area']."' and got_code='".$_SESSION['admin_school']."'  and got_stu='".$value."' and got_tail='".$_GET['got_id']."' ");
//			$db->closedb ();
		}
		if($del){
		$success=_text_report_del_ok;
...
if($op=='studel'){
		...
		$del .=$db->del(TB_ENTTAIL," got_area='".$_SESSION['admin_area']."' and got_code='".$_SESSION['admin_school']."'  and got_id='".$_GET['got_id']."' ");
		if($del){
		$success=_text_report_del_ok;
		} else {
		$error_warning=_text_report_del_fail;
		...
		<div align="right" ><div class="form-group"><button class="btn bg-green btn-flat" type="submit" id="submitForm" name="submitForm"><i class="fa fa-save"></i>&nbsp;<?php echo _button_save; ?></button>&nbsp;&nbsp;<a href="index.php?name=behavior&file=ent&route=<?php echo $route;?>" class="btn bg-red btn-flat"><i class="fa fa-undo"></i>&nbsp;<?php echo _button_reset; ?></a>
				...
				<form method="post" enctype="multipart/form-data" id="formAdd" role="formAdd" data-toggle="validator" class="form-horizontal bootstrap-validator-form" >
					    <div class="box box-success" id="loading-example">
							...
							@$res['bt'] = $db->select_query("SELECT * FROM ".TB_ENT." where go_area='".$_SESSION['admin_area']."' and go_code='".$_SESSION['admin_school']."' ORDER BY go_id ");
...
<?php
		...
		@$res['best'] = $db->select_query("SELECT * FROM ".TB_ENTTAIL." where got_area='".$_SESSION['admin_area']."' and got_code='".$_SESSION['admin_school']."' and got_stu='".$_GET['stu_id']."'"); 
...
<?php
		...
		@$res['stu'] = $db->select_query("SELECT * FROM ".TB_STUDENT." where stu_area='".$_SESSION['admin_area']."' and stu_code='".$_SESSION['admin_school']."' and stu_id='".$_GET['stu_id']."' "); 
		@$arr['stu'] =$db->fetch(@$res['stu']);
		@$res['best'] = $db->select_query("SELECT * FROM ".TB_ENTTAIL." where got_area='".$_SESSION['admin_area']."' and got_code='".$_SESSION['admin_school']."' and got_id='".$_GET['got_id']."'"); 
		@$arr['best']= $db->fetch(@$res['best']);
?>
      <div class="alert alert-success" name="thanks" id="thanks" style="display: none">
      <button type="button" class="close" data-dismiss="alert">X</button>
...
@$res['bad'] = $db->select_query("SELECT *,count(got_stu) as STU FROM ".TB_ENTTAIL." where got_area='".$_SESSION['admin_area']."' and got_code='".$_SESSION['admin_school']."' and got_tail='".$_GET['got_id']."' group by got_tail"); 
 @$arr['bad']= $db->fetch(@$res['bad']);
@$res['tail'] = $db->select_query("SELECT * FROM ".TB_ENT." where go_area='".$_SESSION['admin_area']."' and go_code='".$_SESSION['admin_school']."' and go_id='".$_GET['got_id']."' "); 
...
?>
     ...
     <button type="button" class="close" data-dismiss="alert">X</button>
	  ...
	  <?php
		...
		@$res['num'] = $db->select_query("SELECT * FROM ".TB_ENTTAIL." as a, ".TB_STUDENT." as b where a.got_area='".$_SESSION['admin_area']."' and a.got_code='".$_SESSION['admin_school']."' and a.got_tail='".$_GET['got_id']."'  and a.got_stu=b.stu_id order by b.stu_class,b.stu_id "); 
...
<div class="col-xs-12 connectedSortable">
    ...
    <div align="right" >
    <br>
      <div class="buttons"><a href="index.php?name=behavior&file=ent&op=add&route=<?php echo $route;?>" class="btn bg-aqua btn-flat"><i class="fa fa-edit"></i>&nbsp;<?php echo _button_add; ?></a>&nbsp;&nbsp;<a onclick="$('form').submit();" class="btn bg-red btn-flat"><i class="fa fa-trash-o"></i>&nbsp;<?php echo _button_del; ?></a></div>
      <br>
      </div>
    <div class="box box-danger">
	  ...
	  <?php
		...
		if(@$rows['num']) {
		?>
      <form action="index.php?name=behavior&file=ent&op=delall&route=<?php echo $route;?>" method="post" enctype="multipart/form-data" id="form" class="form-inline">
        <table id="example1" class="table table-bordered table-striped responsive" style="width:100%">
          <thead>
            <tr >
              <th width="1" style="text-align: center;"><input type="checkbox" id="check" class="selector flat all"></th>
...
//		@$arr['level'] =$db->fetch(@$res['level']);
		...
		?>
            <tr>
			  ...
			  <td style="text-align: center;">
			 <a href="index.php?name=behavior&file=ent&op=cldetail&got_id=<?php echo @$arr['num']['got_tail'];?>&route=<?php echo $route;?>" class="btn bg-green btn-flat btn-sm" ><i class="fa fa-search-plus "></i></a>
				<a href="index.php?name=behavior&file=ent&op=del&got_id=<?php echo @$arr['num']['got_tail'];?>&route=<?php echo $route;?>" class="btn bg-red btn-flat btn-sm" data-confirm="<?php echo _text_box_con_delete_text;?>"><i class="fa fa-trash-o "></i></a>
			  </td>
            </tr>
