...
if($op=='del'){
		...
		$del .=$db->del(TB_WHITECLTAIL," whcl_area='".$_SESSION['admin_area']."' and whcl_code='".$_SESSION['admin_school']."' and whcl_id='".$_GET['whcl_id']."' ");
		if($del){
...
if($op=='cldel'){
		...
		$del .=$db->del(TB_WHITECLTAIL," whcl_area='".$_SESSION['admin_area']."' and whcl_code='".$_SESSION['admin_school']."' and whcl_id='".$_GET['whcl_id']."' and whcl_stu='".$_GET['stu_id']."' ");
		if($del){
		$success=_text_report_del_ok;
		} else {
		$error_warning=_text_report_del_fail;
...
if($op=='delall'){
		...
		while(list($key, $value) = each ($_POST['selected'])){
			...
			$del .=$db->del(TB_WHITECLTAIL," whcl_area='".$_SESSION['admin_area']."' and whcl_code='".$_SESSION['admin_school']."' and whcl_id='".$value."' ");
//			$db->closedb ();
		}
		if($del){
		$success=_text_report_del_ok;
...
if($op=='cldelall'){
		...
		while(list($key, $value) = each ($_POST['selected'])){
			...
			$del .=$db->del(TB_WHITECLTAIL," whcl_area='".$_SESSION['admin_area']."' and whcl_code='".$_SESSION['admin_school']."' and whcl_stu='".$value."' and whcl_id='".$_GET['whcl_id']."' ");
//			$db->closedb ();
		}
		if($del){
		$success=_text_report_del_ok;
...
<?php
//<form action="index.php?name=import&file=whiteclass&op=add&action=add&route=" method="post" //enctype="multipart/form-data" id="form" role="form" data-toggle="validator" class="form-horizontal bootstrap-validator-form" >
?>
...
//twitter bootstrap script
 $("button#submitForm").click(function(){
			$.ajax({
			type: "POST",
			url: "modules/import/processwhite.php",
...
//                 $("#thanks").html(msg.message),
				 ...
				 $("#error").hide();
				 setTimeout(function() {
  				 window.location='index.php?name=import&file=whiteclass&route=<?php echo $route;?>';
				}, 1000);
			} else {
...
<?php
		...
		@$res['best'] = $db->select_query("SELECT * FROM ".TB_WHITECLTAIL." WHERE whcl_area='".$_SESSION['admin_area']."' and whcl_code='".$_SESSION['admin_school']."' and whcl_stu='".$_GET['stu_id']."'"); 
		@$arr['best']= $db->fetch(@$res['best']);
?>
		<div align="right" >
		<div class="form-group"><a href="index.php?name=import&file=whiteclass&op=add&route=<?php echo $route;?>" class="btn bg-aqua btn-flat"><i class="fa fa-edit"></i>&nbsp;<?php echo _button_add; ?></a>&nbsp;&nbsp;<a href="index.php?name=import&file=whiteclass&route=<?php echo $route;?>" class="btn bg-orange btn-flat"><i class="fa fa-undo"></i>&nbsp;<?php echo _button_detail; ?></a>
		</div>
		</div>
...
//                $("#error").html(msg.message),
	...
	//		$("#form-content").modal('hide'); 
		...
		<div align="right" >
		<div class="form-group"><button class="btn bg-green btn-flat" type="submit" id="submitForm" name="submitForm"><i class="fa fa-save"></i>&nbsp;<?php echo _button_save; ?></button>&nbsp;&nbsp;<a href="index.php?name=import&file=whiteclass&route=<?php echo $route;?>" class="btn bg-orange btn-flat"><i class="fa fa-undo"></i>&nbsp;<?php echo _button_detail; ?></a>
		</div>
		</div>
...
@$res['bad'] = $db->select_query("SELECT *,count(whcl_stu) as STU FROM ".TB_WHITECLTAIL." WHERE whcl_class='".$_GET['whcl_id']."' and whcl_gr='".$_GET['whcl_gr']."'  and whcl_area='".$_SESSION['admin_area']."' and whcl_code='".$_SESSION['admin_school']."' group by whcl_class"); 
 @$arr['bad']= $db->fetch(@$res['bad']);
@$res['cl'] = $db->select_query("SELECT * FROM ".TB_CLASS_GROUP." ,".TB_CLASS." WHERE clg_group='".$_GET['whcl_id']."' and clg_name='".$_GET['whcl_gr']."' and class_id=clg_group "); 
...
?>
     ...
     <button type="button" class="close" data-dismiss="alert">X</button>
      <span><?php echo _text_report_add_fail; ?></span>
		...
		<div align="right" >
		<div class="form-group"><a href="index.php?name=import&file=whiteclass&op=add&route=<?php echo $route;?>" class="btn bg-aqua btn-flat"><i class="fa fa-edit"></i>&nbsp;<?php echo _button_add; ?></a><?php if($_SESSION['admin_group']==1){ ?>&nbsp;<a onclick="$('form').submit();" class="btn bg-red btn-flat"><i class="fa fa-trash-o"></i>&nbsp;<?php echo _button_del; ?></a><?php } ?>&nbsp;<a href="index.php?name=import&file=whiteclass&route=<?php echo $route;?>" class="btn bg-orange btn-flat"><i class="fa fa-undo"></i>&nbsp;<?php echo _button_detail; ?></a>
		</div>
		</div>
					    <div class="box box-info" id="loading-example">
                                <div class="box-header with-border">
	  ...
	  <?php
		...
		@$res['num'] = $db->select_query("SELECT *,count(whcl_stu) as CO FROM ".TB_WHITECLTAIL." as a, ".TB_STUDENT." as b where whcl_class='".$_GET['whcl_id']."' and whcl_gr='".$_GET['whcl_gr']."' and whcl_area='".$_SESSION['admin_area']."' and whcl_code='".$_SESSION['admin_school']."' and whcl_stu=stu_id group by whcl_stu order by CO desc,stu_class,stu_id "); 
		@$rows['num'] = $db->rows(@$res['num']);
		if(@$rows['num']) {
		?>
      <form action="index.php?name=import&file=whiteclass&op=cldelall&whcl_id=<?php echo $_GET['whcl_id'];?>&route=<?php echo $route;?>" method="post" enctype="multipart/form-data" id="form" class="form-inline">
...
//		@$PerC=(100*(@$arr['num']['CO']))/(@$arr['bad']['STU']);
		...
		?>
            <tr>
			 ...
			 <a href="index.php?name=import&file=whiteclass&op=studetail&whcl_id=<?php echo @$arr['num']['whcl_id'];?>&stu_id=<?php echo @$arr['num']['stu_id'];?>&route=<?php echo $route;?>" class="btn bg-green btn-flat btn-sm" ><i class="fa fa-search-plus "></i></a>
			 <a href="index.php?name=import&file=whiteclass&op=stuedit&whcl_id=<?php echo @$arr['num']['whcl_id']; ?>&stu_id=<?php echo @$arr['num']['stu_id'];?>&route=<?php echo $route;?>" class="btn bg-aqua btn-flat btn-sm"><i class="fa fa-edit "></i></a>
			 <?php if($_SESSION['admin_group']==1){ ?>
				<a href="index.php?name=import&file=whiteclass&op=cldel&whcl_id=<?php echo @$arr['num']['whcl_id'];?>&stu_id=<?php echo @$arr['num']['stu_id'];?>&route=<?php echo $route;?>" class="btn bg-red btn-flat btn-sm" data-confirm="<?php echo _text_box_con_delete_text;?>"><i class="fa fa-trash-o "></i></a>
				<?php } ?>
			  </td>
            </tr>
...
@$res['count'] = $db->select_query("SELECT * FROM ".TB_WHITECLTAIL." where whcl_area='".$_SESSION['admin_area']."' and whcl_code='".$_SESSION['admin_school']."' group by whcl_stu"); 
...
<div class="col-xs-12 connectedSortable">
    ...
    <div align="right" >
    <br>
      <div class="buttons"><a href="index.php?name=import&file=whiteclass&op=add&route=<?php echo $route;?>" class="btn bg-aqua btn-flat"><i class="fa fa-edit"></i>&nbsp;<?php echo _button_add; ?></a><?php if($_SESSION['admin_group']==1){ ?>&nbsp;<a onclick="$('form').submit();" class="btn bg-red btn-flat"><i class="fa fa-trash-o"></i>&nbsp;<?php echo _button_del; ?></a><?php } ?></div>
      <br>
      </div>
    <div class="box box-danger">
	  ...
	  <?php
		...
		@$res['num'] = $db->select_query("SELECT * FROM ".TB_CLASS_GROUP." order by clg_id "); 
		@$rows['num'] = $db->rows(@$res['num']);
		if(@$rows['num']) {
		?>
      <form action="index.php?name=import&file=whiteclass&op=delall&route=<?php echo $route;?>" method="post" enctype="multipart/form-data" id="form" class="form-inline">
        <table id="example1" class="table table-bordered table-striped responsive" style="width:100%">
          <thead>
            <tr >
              <th width="1" style="text-align: center;"><?php if($_SESSION['admin_group']==1){ ?><input type="checkbox" id="check" class="selector flat all"><?php } else {?>#<?php } ?></th>
              <th layout="block" style="text-align:center;" ><?php echo _text_box_table_whiteclass_name; ?></th>
              <th layout="block" style="text-align:center;" ><?php echo _text_box_table_whiteclass_name_room; ?></th>
			  <th layout="block" style="text-align:center;"><?php echo _text_box_table_whiteclass_bar;?></th>
              <th layout="block" style="text-align:center;">Action</th>
            </tr>
          </thead>
          <tbody>
		...
		while (@$arr['num'] = $db->fetch(@$res['num'])){
		@$res['tail'] = $db->select_query("SELECT * FROM ".TB_WHITECLASS." WHERE wh_id='".@$arr['num']['whcl_id']."' "); 
		@$arr['tail'] =$db->fetch(@$res['tail']);
//		@$res['level'] = $db->select_query("SELECT * FROM ".TB_WHITECLASSLEVEL." WHERE blevel_id='".@$arr['tail']['badtail_level']."' "); 
//		@$arr['level'] =$db->fetch(@$res['level']);
		...
		?>
            <tr>
              ...
              <td layout="block" style="text-align: center;"><?php echo progress_bar_whitecl(@$arr['cl']['class_id'],@$arr['num']['clg_name']);?></td>
			  <td style="text-align: center;">
			 <a href="index.php?name=import&file=whiteclass&op=cldetail&whcl_id=<?php echo @$arr['num']['clg_group'];?>&whcl_gr=<?php echo @$arr['num']['clg_name'];?>&route=<?php echo $route;?>" class="btn bg-green btn-flat btn-sm" ><i class="fa fa-search-plus "></i></a>
			 <?php if($_SESSION['admin_group']==1){ ?>
				<a href="index.php?name=import&file=whiteclass&op=del&whcl_id=<?php echo @$arr['num']['clg_group'];?>&whcl_gr=<?php echo @$arr['num']['clg_name'];?>&route=<?php echo $route;?>" class="btn bg-red btn-flat btn-sm" data-confirm="<?php echo _text_box_con_delete_text;?>"><i class="fa fa-trash-o "></i></a>
				<?php } ?>
			  </td>
            </tr>
