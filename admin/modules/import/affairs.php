...
if($op=='del'){
		...
		$del .=$db->del(TB_AFFTAIL," afft_area='".$_SESSION['admin_area']."' and afft_code='".$_SESSION['admin_school']."' and afft_aff='".$_GET['afft_id']."' ");
		if($del){
...
if($op=='delcl'){
		...
		@$res['cl'] = $db->select_query("SELECT * FROM ".TB_CLASS_GROUP.",".TB_STUDENT." where clg_area='".$_SESSION['admin_area']."' and clg_school='".$_SESSION['admin_school']."' and clg_group='".$_GET['cl_id']."' and clg_name='".$_GET['clg_gr']."' and stu_class=clg_group and stu_cn=clg_name order by clg_id "); 
		while(@$arr['cl'] = $db->fetch(@$res['cl'])){
		$del .=$db->del(TB_AFFTAIL," afft_area='".$_SESSION['admin_area']."' and afft_code='".$_SESSION['admin_school']."' and afft_stu='".@$arr['cl']['stu_id']."' ");
		}
		if($del){
		$success=_text_report_del_ok;
		} else {
...
if($op=='cldel'){
		...
		$del .=$db->del(TB_AFFTAIL," afft_area='".$_SESSION['admin_area']."' and afft_code='".$_SESSION['admin_school']."' and afft_id='".$_GET['afft_id']."' and afft_stu='".$_GET['stu_id']."' ");
		if($del){
		$success=_text_report_del_ok;
		} else {
		$error_warning=_text_report_del_fail;
...
if($op=='cldelall'){
		...
		while(list($key, $value) = each ($_POST['selected'])){
			...
			$del .=$db->del(TB_AFFTAIL," afft_area='".$_SESSION['admin_area']."' and afft_code='".$_SESSION['admin_school']."' and afft_stu='".$value."' and afft_id='".$_GET['afft_id']."' ");
//			$db->closedb ();
		}
		if($del){
		$success=_text_report_del_ok;
...
if($op=='studel'){
		...
		$del .=$db->del(TB_AFFTAIL," afft_area='".$_SESSION['admin_area']."' and afft_code='".$_SESSION['admin_school']."' and afft_id='".$_GET['afft_id']."' ");
		if($del){
		$success=_text_report_del_ok;
		} else {
		$error_warning=_text_report_del_fail;
...
<?php
//<form action="index.php?name=import&file=affairs&op=add&action=add&route=" method="post" //enctype="multipart/form-data" id="form" role="form" data-toggle="validator" class="form-horizontal bootstrap-validator-form" >
?>
...
//twitter bootstrap script
 $("button#submitForm").click(function(){
			$.ajax({
			type: "POST",
			url: "modules/import/processaffairs.php",
...
//                 $("#thanks").html(msg.message),
				 ...
				 $("#error").hide();
				 setTimeout(function() {
  				 window.location='index.php?name=import&file=affairs&route=<?php echo $route;?>';
				}, 1000);
			} else {
//                $("#error").html(msg.message),
	...
	//		$("#form-content").modal('hide'); 
		...
		<div align="right" ><div class="form-group"><button class="btn bg-green btn-flat" type="submit" id="submitForm" name="submitForm"><i class="fa fa-save"></i>&nbsp;<?php echo _button_save; ?></button>&nbsp;&nbsp;<a href="index.php?name=import&file=affairs&route=<?php echo $route;?>" class="btn bg-red btn-flat"><i class="fa fa-undo"></i>&nbsp;<?php echo _button_reset; ?></a>
		</div></div>
...
$(function(){
 $("select#classlist4").change(function(){
  var datalist4 = $.ajax({ // รับค่าจาก ajax เก็บไว้ที่ตัวแปร datalist2
     url: "modules/import/classlist5.php", // ไฟล์สำหรับการกำหนดเงื่อนไข
     data :{
         "class_id" : $(this).val(),
...
//window.alert( $('#Date_id').val() );
  ...
  <!--  <div class="col-xs-12" align="right" >
	<div class="form-group"><button class="btn bg-green btn-flat" type="submit" id="submitForm" name="submitForm"><i class="fa fa-save"></i>&nbsp;<?php echo _button_save; ?></button>&nbsp;&nbsp;<a href="index.php?name=import&file=affairs&route=<?php echo $route;?>" class="btn bg-red btn-flat"><i class="fa fa-undo"></i>&nbsp;<?php echo _button_reset; ?></a>
      </div>
	 </div>-->
							...
							@$res['bt'] = $db->select_query("SELECT * FROM ".TB_AFFAIRS." ORDER BY aff_id ");
...
<?php
		...
		@$res['best'] = $db->select_query("SELECT * FROM ".TB_AFFTAIL." WHERE afft_stu='".$_GET['stu_id']."'"); 
		@$arr['best']= $db->fetch(@$res['best']);
?>
		<div align="right" >
		<div class="form-group"><a href="index.php?name=import&file=affairs&op=add&route=<?php echo $route;?>" class="btn bg-aqua btn-flat"><i class="fa fa-edit"></i>&nbsp;<?php echo _button_add; ?></a>&nbsp;&nbsp;<a href="index.php?name=import&file=affairs&route=<?php echo $route;?>" class="btn bg-orange btn-flat"><i class="fa fa-undo"></i>&nbsp;<?php echo _button_detail; ?></a>
		</div>
		</div>
...
//                $("#error").html(msg.message),
	...
	//		$("#form-content").modal('hide'); 
		...
		<div align="right" >
		<div class="form-group"><button class="btn bg-green btn-flat" type="submit" id="submitForm" name="submitForm"><i class="fa fa-save"></i>&nbsp;<?php echo _button_save; ?></button>&nbsp;&nbsp;<a href="index.php?name=import&file=affairs&route=<?php echo $route;?>" class="btn bg-orange btn-flat"><i class="fa fa-undo"></i>&nbsp;<?php echo _button_detail; ?></a>
		</div>
		</div>
				<form method="post" enctype="multipart/form-data" id="formEdit" role="formEdit" data-toggle="validator" class="form-horizontal bootstrap-validator-form" >
					    <div class="box box-info" id="loading-example">
							...
							<?php $DateTimeStart=date('Y-m-d');?>
							<div class="input-group date" id="dp1" data-date="<?php echo $DateTimeStart;?>" data-date-format="yyyy-mm-dd">
							<input type='text' class="form-control" name="Pu_Dtime" class="form-control css-require" value="<?php echo @$arr['best']['afft_date']; ?>">
							<span class="input-group-addon"><span class="add-on"><i class="glyphicon glyphicon-calendar"></i></span></span>
									</div>
									<!-- /.input group -->
								</div>
...
@$res['tails'] = $db->select_query("SELECT * FROM ".TB_AFFAIRS." WHERE aff_id='".$_GET['afft_id']."' "); 
...
<div class="col-xs-12 connectedSortable">
    ...
    <div align="right" >
    <br>
      <div class="buttons"><a href="index.php?name=import&file=affairs&op=add&route=<?php echo $route;?>" class="btn bg-aqua btn-flat"><i class="fa fa-edit"></i>&nbsp;<?php echo _button_add; ?></a><?php if($_SESSION['admin_group']==1){ ?>&nbsp;<a onclick="$('form').submit();" class="btn bg-red btn-flat"><i class="fa fa-trash-o"></i>&nbsp;<?php echo _button_del; ?></a><?php } ?></div>
      <br>
      </div>
    <div class="box box-danger">
	  ...
	  <?php
		...
		if(@$rows['num']) {
		?>
      <form action="index.php?name=import&file=affairs&op=delall&route=<?php echo $route;?>" method="post" enctype="multipart/form-data" id="form" class="form-inline">
		...
		@$res['stu'] = $db->select_query("SELECT *,count(stu_id) as STU FROM ".TB_STUDENT." , ".TB_AFFTAIL." where stu_class='".@$arr['num']['clg_group']."' and afft_stu=stu_id and stu_cn='".@$arr['num']['clg_name']."' "); 
...
//		@$PerC=(100*(@$arr['num']['CO']))/(@$rows['count']);
		?>
            <tr>
              <td style="text-align: center;"><?php if($_SESSION['admin_group']==1){ ?><input type="checkbox" name="selected[]" value="<?php echo @$arr['num']['whcl_id']; ?>" class="selector flat"/><?php } else { echo $i;} ?></td>
			 ...
			 <a href="index.php?name=import&file=affairs&op=cldetail&afft_id=<?php echo $_GET['afft_id'];?>&cl_id=<?php echo @$arr['num']['clg_group'];?>&clg_gr=<?php echo @$arr['num']['clg_name'];?>&route=<?php echo $route;?>" class="btn bg-green btn-flat btn-sm" ><i class="fa fa-search-plus "></i></a>
			 <?php if($_SESSION['admin_group']==1){ ?>
				<a href="index.php?name=import&file=affairs&op=delcl&afft_id=<?php echo $_GET['afft_id'];?>&cl_id=<?php echo @$arr['num']['clg_group'];?>&clg_gr=<?php echo @$arr['num']['clg_name'];?>&route=<?php echo $route;?>" class="btn bg-red btn-flat btn-sm" data-confirm="<?php echo _text_box_con_delete_text;?>"><i class="fa fa-trash-o "></i></a>
				<?php } ?>
			  </td>
            </tr>
...
@$res['tail'] = $db->select_query("SELECT * FROM ".TB_AFFAIRS." WHERE aff_id='".$_GET['afft_id']."' "); 
@$arr['tail'] =$db->fetch(@$res['tail']);
?>
     ...
     <button type="button" class="close" data-dismiss="alert">X</button>
      <span><?php echo _text_report_add_fail; ?></span>
		...
		<div align="right" >
		<div class="form-group"><a href="index.php?name=import&file=affairs&op=add&route=<?php echo $route;?>" class="btn bg-aqua btn-flat"><i class="fa fa-edit"></i>&nbsp;<?php echo _button_add; ?></a>&nbsp;&nbsp;<a onclick="$('form').submit();" class="btn bg-red btn-flat"><i class="fa fa-trash-o"></i>&nbsp;<?php echo _button_del; ?></a>&nbsp;<a href="index.php?name=import&file=affairs&route=<?php echo $route;?>" class="btn bg-orange btn-flat"><i class="fa fa-undo"></i>&nbsp;<?php echo _button_detail; ?></a>
		</div>
		</div>
					    <div class="box box-info" id="loading-example">
                                <div class="box-header with-border">
	  ...
	  <?php
		...
		if(@$rows['num']) {
		?>
      <form action="index.php?name=import&file=affairs&op=cldelall&afft_id=<?php echo $_GET['afft_id'];?>&route=<?php echo $route;?>" method="post" enctype="multipart/form-data" id="form" class="form-inline">
        <table id="example1" class="table table-bordered table-striped responsive" style="width:100%">
          <thead>
            <tr >
		...
		@$res['tail'] = $db->select_query("SELECT * FROM ".TB_AFFTAIL." WHERE afft_id='".@$arr['num']['afft_id']."' "); 
		@$arr['tail'] =$db->fetch(@$res['tail']);
...
//		@$arr['level'] =$db->fetch(@$res['level']);
		...
		?>
            <tr>
			  ...
			  <td style="text-align: center;">
			 <a href="index.php?name=import&file=affairs&op=studetail&afft_id=<?php echo @$arr['num']['afft_id'];?>&stu_id=<?php echo @$arr['num']['stu_id'];?>&route=<?php echo $route;?>" class="btn bg-green btn-flat btn-sm" ><i class="fa fa-search-plus "></i></a>
			 <a href="index.php?name=import&file=affairs&op=stuedit&afft_id=<?php echo @$arr['num']['afft_id']; ?>&stu_id=<?php echo @$arr['num']['stu_id'];?>&route=<?php echo $route;?>" class="btn bg-aqua btn-flat btn-sm"><i class="fa fa-edit "></i></a>
				<a href="index.php?name=import&file=affairs&op=cldel&afft_id=<?php echo @$arr['num']['afft_id'];?>&stu_id=<?php echo @$arr['num']['stu_id'];?>&route=<?php echo $route;?>" class="btn bg-red btn-flat btn-sm" data-confirm="<?php echo _text_box_con_delete_text;?>"><i class="fa fa-trash-o "></i></a>
			  </td>
            </tr>
...
<div class="col-xs-12 connectedSortable">
    ...
    <div align="right" >
    <br>
      <div class="buttons"><a href="index.php?name=import&file=affairs&op=add&route=<?php echo $route;?>" class="btn bg-aqua btn-flat"><i class="fa fa-edit"></i>&nbsp;<?php echo _button_add; ?></a>&nbsp;&nbsp;<a onclick="$('form').submit();" class="btn bg-red btn-flat"><i class="fa fa-trash-o"></i>&nbsp;<?php echo _button_del; ?></a></div>
      <br>
      </div>
    <div class="box box-danger">
		         <div class="box-header with-border">
...
//		@$arr['level'] =$db->fetch(@$res['level']);
		...
		?>
            <tr>
			  ...
			  <td style="text-align: center;">
			 <a href="index.php?name=import&file=affairs&op=scldetail&afft_id=<?php echo @$arr['num']['aff_id'];?>&route=<?php echo $route;?>" class="btn bg-green btn-flat btn-sm" ><i class="fa fa-search-plus "></i></a>
				<a href="index.php?name=import&file=affairs&op=del&afft_id=<?php echo @$arr['num']['aff_id'];?>&route=<?php echo $route;?>" class="btn bg-red btn-flat btn-sm" data-confirm="<?php echo _text_box_con_delete_text;?>"><i class="fa fa-trash-o "></i></a>
			  </td>
            </tr>
