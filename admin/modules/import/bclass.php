...
if($op=='AddTab1'){
	...
	for ($i=0; $i < $RANK; $i++) {
		...
		$sq4=$db->select_query("SELECT * FROM ".TB_BAD." WHERE bad_area='".$_SESSION['admin_area']."' and bad_code='".$_SESSION['admin_school']."' and bad_stu='".$_POST['StuID'][$i]."' and b_date='".$_POST['DateID']."' ");
		@$result4=$db->fetch($sq4);

		if(@$result4['bad_name'] !='ไม่มาโรงเรียน' && @$result4['bad_name'] !='ลาโรงเรียน' ){
		if($_POST['Bad_Status'][$i]=='1'){

			$sql=$db->select_query("SELECT * FROM ".TB_BADTAIL." WHERE badtail_area='".$_SESSION['admin_area']."' and badtail_code='".$_SESSION['admin_school']."' or badtail_area='' and badtail_code='' and badtail_name='ไม่มาเคารพธงชาติ' ");
			@$result=$db->fetch($sql);
		...
		if(@$arr['chclass']['c_id']){
			...
			$sql=$db->select_query("SELECT * FROM ".TB_BADTAIL." WHERE badtail_area='".$_SESSION['admin_area']."' and badtail_code='".$_SESSION['admin_school']."' or badtail_area='' and badtail_code=''  and badtail_name='มาสาย' ");
			@$result=$db->fetch($sql);
			$btailid=@$result['badtail_id'];
			$add .=$db->add_db(TB_BAD,array(
				"bad_area"=>"".$_SESSION['admin_area']."",
	...
	for ($i=0; $i < $RANK; $i++) {
		...
		if(@$arr['chclass']['c_id']){
			...
			$add .=$db->add_db(TB_BAD,array(
				...
				"bad_mouth"=>"".$m."",
				"bad_year"=>"".$y."",
				"bad_dam"=>"ฝ".$_SESSION['admin_login']."",
				"bad_t"=>"1",
				"b_date"=>"".$_POST['DateID']."",
				"b_Mtime"=>"".$Mtime."",
				"bad_sess"=>"admin"
    ...
    <div class="btn-pref btn-group btn-group-justified btn-group-lg" role="group" aria-label="...">
        ...
        <div class="btn-group" role="group">
            <button type="button" id="favorites" class="btn btn-default" href="#tab2" data-toggle="tab"><span class="glyphicon glyphicon-th" aria-hidden="true"></span>
                <div class="hidden-xs"><?php echo _heading_title_M_tab2;?></div>
            </button>
        </div>
        <div class="btn-group" role="group">
            <button type="button" id="following" class="btn btn-default" href="#tab3" data-toggle="tab"><span class="glyphicon glyphicon-th-list" aria-hidden="true"></span>
                <div class="hidden-xs"><?php echo _heading_title_M_tab3;?></div>
            </button>
        </div>
    </div>
...
$(function(){
 $("select#classlist").change(function(){
  var datalist1 = $.ajax({ // รับค่าจาก ajax เก็บไว้ที่ตัวแปร datalist2
     url: "modules/import/classlist.php", // ไฟล์สำหรับการกำหนดเงื่อนไข
     data :{
         "class_id" : $(this).val(),
         "date_id" : $('#Date_id').val(),
     },
...
//window.alert( $('#Date_id').val() );
  // ชื่อตัวแปร และ element ต่างๆ สามารถเปลี่ยนไปตามการกำหนด
				...
				<form method="post" action="index.php?name=import&file=bclass&op=AddTab1&route=<?php echo $route;?>" enctype="multipart/form-data" id="formAdd" role="formAdd" data-toggle="validator" class="form-horizontal bootstrap-validator-form" >
		<div class="col-xs-12" align="center" >
		<h4 class="box-title"><?php echo _heading_title_M_tab1; ?></h4>
		</div>
    <div class="col-xs-12" align="right" >
	<div class="form-group"><button class="btn bg-green btn-flat" type="submit" id="submitForm" name="submitForm"><i class="fa fa-save"></i>&nbsp;<?php echo _button_save; ?></button>&nbsp;&nbsp;<a href="index.php?name=import&file=bclass&route=<?php echo $route;?>" class="btn bg-red btn-flat"><i class="fa fa-undo"></i>&nbsp;<?php echo _button_reset; ?></a>
      </div>
	 </div>
							<div class="form-group">
							<div class="col-sm-12" >
							...
							<div class="col-sm-4" >
							<select  class="form-control css-require" id="classlist" name="Stu_class" required="required">
...
$(function(){
 $("select#classlist2").change(function(){
  var datalist2 = $.ajax({ // รับค่าจาก ajax เก็บไว้ที่ตัวแปร datalist2
     url: "modules/import/classlist2.php", // ไฟล์สำหรับการกำหนดเงื่อนไข
     data :{
         "class_id" : $(this).val(),
...
//window.alert( $('#Date_id').val() );
  // ชื่อตัวแปร และ element ต่างๆ สามารถเปลี่ยนไปตามการกำหนด
				...
				<form method="post" action="index.php?name=import&file=bclass&op=AddTab2&route=<?php echo $route;?>" enctype="multipart/form-data" id="formAdd" role="formAdd" data-toggle="validator" class="form-horizontal bootstrap-validator-form" >
		<div class="col-xs-12" align="center" >
		<div class="alert ">
		<h4 class="box-title"><?php echo _heading_title_M_tab2; ?></h4>
		</div>
		</div>
    <div class="col-xs-12" align="right" >
	<div class="form-group"><button class="btn bg-green btn-flat" type="submit" id="submitForm" name="submitForm"><i class="fa fa-save"></i>&nbsp;<?php echo _button_save; ?></button>&nbsp;&nbsp;<a href="index.php?name=import&file=bclass&route=<?php echo $route;?>" class="btn bg-red btn-flat"><i class="fa fa-undo"></i>&nbsp;<?php echo _button_reset; ?></a>
							...
							<label class="col-sm-3 control-label" ><?php echo _text_box_table_stu_class; ?></label>
							<div class="col-sm-4" >
							<select  class="form-control css-require" id="classlist2" name="Stu_class2" required="required">
							<option value="" selected disabled><?php echo _text_box_table_stu_class_select;?></option>
							<?php
...
$(function(){
 $("select#classlist3").change(function(){
  var datalist3 = $.ajax({ // รับค่าจาก ajax เก็บไว้ที่ตัวแปร datalist2
     url: "modules/import/classlist3.php", // ไฟล์สำหรับการกำหนดเงื่อนไข
     data :{
         "class_id" : $(this).val(),
...
//window.alert( $('#Date_id').val() );
  // ชื่อตัวแปร และ element ต่างๆ สามารถเปลี่ยนไปตามการกำหนด
				...
				<form method="post" action="index.php?name=import&file=bclass&op=AddTab3&route=<?php echo $route;?>" enctype="multipart/form-data" id="formAdd" role="formAdd" data-toggle="validator" class="form-horizontal bootstrap-validator-form" >
		<div class="col-xs-12" align="center" >
		<div class="alert ">
		<h4 class="box-title"><?php echo _heading_title_M_tab3; ?></h4>
		</div>
		</div>
