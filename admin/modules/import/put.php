...
if($op=='del'){
		...
		$del .=$db->del(TB_PUTTAIL," pt_id='".$_GET['pt_id']."' ");
		if($del){
...
if($op=='cldel'){
		...
		$del .=$db->del(TB_PUTTAIL," pt_id='".$_GET['pt_id']."' and pt_stu='".$_GET['stu_id']."' ");
		if($del){
		$success=_text_report_del_ok;
		} else {
		$error_warning=_text_report_del_fail;
...
if($op=='delall'){
		...
		while(list($key, $value) = each ($_POST['selected'])){
			...
			$del .=$db->del(TB_PUTTAIL," pt_id='".$value."' ");
//			$db->closedb ();
		}
		if($del){
		$success=_text_report_del_ok;
...
if($op=='cldelall'){
		...
		while(list($key, $value) = each ($_POST['selected'])){
			...
			$del .=$db->del(TB_PUTTAIL," pt_stu='".$value."' and pt_id='".$_GET['pt_id']."' ");
//			$db->closedb ();
		}
		if($del){
		$success=_text_report_del_ok;
...
<?php
//<form action="index.php?name=import&file=put&op=add&action=add&route=" method="post" //enctype="multipart/form-data" id="form" role="form" data-toggle="validator" class="form-horizontal bootstrap-validator-form" >
?>
...
//twitter bootstrap script
 $("button#submitForm").click(function(){
			$.ajax({
			type: "POST",
			url: "modules/import/processput.php",
...
//                 $("#thanks").html(msg.message),
				 ...
				 $("#error").hide();
				 setTimeout(function() {
  				 window.location='index.php?name=import&file=put&route=<?php echo $route;?>';
				}, 1000);
			} else {
//                $("#error").html(msg.message),
	...
	//		$("#form-content").modal('hide'); 
		...
		<div align="right" ><div class="form-group"><button class="btn bg-green btn-flat" type="submit" id="submitForm" name="submitForm"><i class="fa fa-save"></i>&nbsp;<?php echo _button_save; ?></button>&nbsp;&nbsp;<a href="index.php?name=import&file=put&route=<?php echo $route;?>" class="btn bg-red btn-flat"><i class="fa fa-undo"></i>&nbsp;<?php echo _button_reset; ?></a>
		</div></div>
				<form method="post" enctype="multipart/form-data" id="formAdd" role="formAdd" data-toggle="validator" class="form-horizontal bootstrap-validator-form" >
					    <div class="box box-success" id="loading-example">
							...
							@$res['bt'] = $db->select_query("SELECT * FROM ".TB_PUT." ORDER BY pu_id ");
...
<?php
		...
		@$res['best'] = $db->select_query("SELECT * FROM ".TB_PUTTAIL." WHERE pt_stu='".$_GET['stu_id']."'"); 
		@$arr['best']= $db->fetch(@$res['best']);
?>
		<div align="right" >
		<div class="form-group"><a href="index.php?name=import&file=put&op=add&route=<?php echo $route;?>" class="btn bg-aqua btn-flat"><i class="fa fa-edit"></i>&nbsp;<?php echo _button_add; ?></a>&nbsp;&nbsp;<a href="index.php?name=import&file=put&route=<?php echo $route;?>" class="btn bg-orange btn-flat"><i class="fa fa-undo"></i>&nbsp;<?php echo _button_detail; ?></a>
		</div>
		</div>
...
//                $("#error").html(msg.message),
	...
	//		$("#form-content").modal('hide'); 
		...
		<div align="right" >
		<div class="form-group"><button class="btn bg-green btn-flat" type="submit" id="submitForm" name="submitForm"><i class="fa fa-save"></i>&nbsp;<?php echo _button_save; ?></button>&nbsp;&nbsp;<a href="index.php?name=import&file=put&route=<?php echo $route;?>" class="btn bg-orange btn-flat"><i class="fa fa-undo"></i>&nbsp;<?php echo _button_detail; ?></a>
		</div>
		</div>
...
@$res['bad'] = $db->select_query("SELECT *,count(pt_stu) as STU FROM ".TB_PUTTAIL." WHERE pt_pu='".$_GET['pt_id']."' "); 
 @$arr['bad']= $db->fetch(@$res['bad']);
@$res['tail'] = $db->select_query("SELECT * FROM ".TB_PUT." WHERE pu_id='".$_GET['pt_id']."' "); 
...
?>
     ...
     <button type="button" class="close" data-dismiss="alert">X</button>
      <span><?php echo _text_report_add_fail; ?></span>
		...
		<div align="right" >
		<div class="form-group"><a href="index.php?name=import&file=put&op=add&route=<?php echo $route;?>" class="btn bg-aqua btn-flat"><i class="fa fa-edit"></i>&nbsp;<?php echo _button_add; ?></a>&nbsp;&nbsp;<a onclick="$('form').submit();" class="btn bg-red btn-flat"><i class="fa fa-trash-o"></i>&nbsp;<?php echo _button_del; ?></a>&nbsp;<a href="index.php?name=import&file=put&route=<?php echo $route;?>" class="btn bg-orange btn-flat"><i class="fa fa-undo"></i>&nbsp;<?php echo _button_detail; ?></a>
		</div>
		</div>
					    <div class="box box-info" id="loading-example">
                                <div class="box-header with-border">
	  ...
	  <?php
		...
		if(@$rows['num']) {
		?>
      <form action="index.php?name=import&file=put&op=cldelall&pt_id=<?php echo $_GET['pt_id'];?>&route=<?php echo $route;?>" method="post" enctype="multipart/form-data" id="form" class="form-inline">
		...
		@$res['tail'] = $db->select_query("SELECT * FROM ".TB_PUTTAIL." WHERE pt_id='".@$arr['num']['pt_id']."' "); 
		@$arr['tail'] =$db->fetch(@$res['tail']);
...
//		@$arr['level'] =$db->fetch(@$res['level']);
		...
		?>
            <tr>
			 ...
			 <a href="index.php?name=import&file=put&op=studetail&pt_id=<?php echo @$arr['num']['pt_id'];?>&stu_id=<?php echo @$arr['num']['stu_id'];?>&route=<?php echo $route;?>" class="btn bg-green btn-flat btn-sm" ><i class="fa fa-search-plus "></i></a>
			 <a href="index.php?name=import&file=put&op=stuedit&pt_id=<?php echo @$arr['num']['pt_id']; ?>&stu_id=<?php echo @$arr['num']['stu_id'];?>&route=<?php echo $route;?>" class="btn bg-aqua btn-flat btn-sm"><i class="fa fa-edit "></i></a>
				<a href="index.php?name=import&file=put&op=cldel&pt_id=<?php echo @$arr['num']['pt_id'];?>&stu_id=<?php echo @$arr['num']['stu_id'];?>&route=<?php echo $route;?>" class="btn bg-red btn-flat btn-sm" data-confirm="<?php echo _text_box_con_delete_text;?>"><i class="fa fa-trash-o "></i></a>
			  </td>
            </tr>
...
<div class="col-xs-12 connectedSortable">
    ...
    <div align="right" >
    <br>
      <div class="buttons"><a href="index.php?name=import&file=put&op=add&route=<?php echo $route;?>" class="btn bg-aqua btn-flat"><i class="fa fa-edit"></i>&nbsp;<?php echo _button_add; ?></a>&nbsp;&nbsp;<a onclick="$('form').submit();" class="btn bg-red btn-flat"><i class="fa fa-trash-o"></i>&nbsp;<?php echo _button_del; ?></a></div>
      <br>
      </div>
    <div class="box box-danger">
	  ...
	  <?php
		...
		if(@$rows['num']) {
		?>
      <form action="index.php?name=import&file=put&op=delall&route=<?php echo $route;?>" method="post" enctype="multipart/form-data" id="form" class="form-inline">
        <table id="example1" class="table table-bordered table-striped responsive" style="width:100%">
          <thead>
            <tr >
              <th width="1" style="text-align: center;"><input type="checkbox" id="check" class="selector flat all"></th>
...
//		@$arr['level'] =$db->fetch(@$res['level']);
		...
		?>
            <tr>
			  ...
			  <td style="text-align: center;">
			 <a href="index.php?name=import&file=put&op=cldetail&pt_id=<?php echo @$arr['num']['pt_pu'];?>&route=<?php echo $route;?>" class="btn bg-green btn-flat btn-sm" ><i class="fa fa-search-plus "></i></a>
				<a href="index.php?name=import&file=put&op=del&pt_id=<?php echo @$arr['num']['pt_pu'];?>&route=<?php echo $route;?>" class="btn bg-red btn-flat btn-sm" data-confirm="<?php echo _text_box_con_delete_text;?>"><i class="fa fa-trash-o "></i></a>
			  </td>
            </tr>
