...
if($op=='del'){
		...
		$del .=$db->del(TB_GOOD," good_area='".$_SESSION['admin_area']."' and good_code='".$_SESSION['admin_school']."' and good_tail='".$_GET['good_id']."' ");
		if($del){
...
if($op=='cldel'){
		...
		$del .=$db->del(TB_GOOD," good_area='".$_SESSION['admin_area']."' and good_code='".$_SESSION['admin_school']."' and good_tail='".$_GET['good_id']."' and good_stu='".$_GET['stu_id']."' ");
		if($del){
		$success=_text_report_del_ok;
		} else {
		$error_warning=_text_report_del_fail;
...
if($op=='delall'){
		...
		while(list($key, $value) = each ($_POST['selected'])){
			...
			$del .=$db->del(TB_GOOD," good_area='".$_SESSION['admin_area']."' and good_code='".$_SESSION['admin_school']."' and good_tail='".$value."' ");
//			$db->closedb ();
		}
		if($del){
		$success=_text_report_del_ok;
...
if($op=='cldelall'){
		...
		while(list($key, $value) = each ($_POST['selected'])){
			...
			$del .=$db->del(TB_GOOD," good_area='".$_SESSION['admin_area']."' and good_code='".$_SESSION['admin_school']."' and good_stu='".$value."' and good_tail='".$_GET['good_id']."' ");
//			$db->closedb ();
		}
		if($del){
		$success=_text_report_del_ok;
...
if($op=='studel'){
		...
		$del .=$db->del(TB_GOOD," good_area='".$_SESSION['admin_area']."' and good_code='".$_SESSION['admin_school']."' and good_id='".$_GET['good_id']."' ");
		if($del){
		$success=_text_report_del_ok;
		} else {
		$error_warning=_text_report_del_fail;
...
<?php
//<form action="index.php?name=import&file=good&op=add&action=add&route=" method="post" //enctype="multipart/form-data" id="form" role="form" data-toggle="validator" class="form-horizontal bootstrap-validator-form" >
?>
...
//twitter bootstrap script
 $("button#submitForm").click(function(){
			$.ajax({
			type: "POST",
			url: "modules/import/processgood.php",
...
//                 $("#thanks").html(msg.message),
				 ...
				 $("#error").hide();
				 setTimeout(function() {
  				 window.location='index.php?name=import&file=good&route=<?php echo $route;?>';
				}, 1000);
			} else {
//                $("#error").html(msg.message),
	...
	//		$("#form-content").modal('hide'); 
		...
		<div align="right" ><div class="form-group"><button class="btn bg-green btn-flat" type="submit" id="submitForm" name="submitForm"><i class="fa fa-save"></i>&nbsp;<?php echo _button_save; ?></button>&nbsp;&nbsp;<a href="index.php?name=import&file=good&route=<?php echo $route;?>" class="btn bg-red btn-flat"><i class="fa fa-undo"></i>&nbsp;<?php echo _button_reset; ?></a>
		</div></div>
				<form method="post" enctype="multipart/form-data" id="formAdd" role="formAdd" data-toggle="validator" class="form-horizontal bootstrap-validator-form" >
					    <div class="box box-success" id="loading-example">
							...
							@$res['good'] = $db->select_query("SELECT * FROM ".TB_GOODTAIL." where goodtail_area='".$_SESSION['admin_area']."' and goodtail_code='".$_SESSION['admin_school']."' or goodtail_area='' and goodtail_code='' ORDER BY goodtail_id ");
...
@$res['goods'] = $db->select_query("SELECT * FROM ".TB_GOOD." WHERE good_area='".$_SESSION['admin_area']."' and good_code='".$_SESSION['admin_school']."' and good_id='".$_GET['good_id']."' "); 
...
@$res['good'] = $db->select_query("SELECT *,count(good_stu) as STU FROM ".TB_GOOD." WHERE good_area='".$_SESSION['admin_area']."' and good_code='".$_SESSION['admin_school']."' and good_tail='".$_GET['good_id']."' and good_stu='".$_GET['stu_id']."' group by good_tail"); 
...
?>
     ...
     <button type="button" class="close" data-dismiss="alert">X</button>
      <span><?php echo _text_report_add_fail; ?></span>
		...
		<div align="right" >
		<div class="form-group"><a href="index.php?name=import&file=good&op=add&route=<?php echo $route;?>" class="btn bg-aqua btn-flat"><i class="fa fa-edit"></i>&nbsp;<?php echo _button_add; ?></a><?php if($_SESSION['admin_group']==1){?><a onclick="$('form').submit();" class="btn bg-red btn-flat"><i class="fa fa-trash-o"></i>&nbsp;<?php echo _button_del; ?></a><?php } ?>&nbsp;<a href="index.php?name=import&file=good&route=<?php echo $route;?>" class="btn bg-orange btn-flat"><i class="fa fa-undo"></i>&nbsp;<?php echo _button_detail; ?></a>
		</div>
		</div>
	  ...
	  <?php
		...
		@$res['num'] = $db->select_query("SELECT * FROM ".TB_GOOD." as a, ".TB_STUDENT." as b where good_area='".$_SESSION['admin_area']."' and good_code='".$_SESSION['admin_school']."' and good_tail='".$_GET['good_id']."' and good_stu=stu_id and good_stu='".$_GET['stu_id']."' order by good_id desc "); 
		@$rows['num'] = $db->rows(@$res['num']);
		if(@$rows['num']) {
		?>
      <form action="index.php?name=import&file=good&op=studelall&route=<?php echo $route;?>" method="post" enctype="multipart/form-data" id="form" class="form-inline">
		...
		?>
            <tr>
              <td style="text-align: center;"><?php if($_SESSION['admin_group']==1){ ?><input type="checkbox" name="selected[]" value="<?php echo @$arr['num']['good_id']; ?>" class="selector flat"/><?php } else { echo $i;}?></td>
			  ...
			  <td style="text-align: center;">
				<a href="index.php?name=import&file=good&op=edit&good_id=<?php echo @$arr['num']['good_id']; ?>&route=<?php echo $route;?>" class="btn bg-aqua btn-flat btn-sm"><i class="fa fa-edit "></i></a>
				<?php if($_SESSION['admin_group']==1){ ?>
				<a href="index.php?name=import&file=good&op=studel&good_id=<?php echo @$arr['num']['good_id'];?>&route=<?php echo $route;?>" class="btn bg-red btn-flat btn-sm" data-confirm="<?php echo _text_box_con_delete_text;?>"><i class="fa fa-trash-o "></i></a>
				<?php } ?>
			  </td>
            </tr>
...
@$res['good'] = $db->select_query("SELECT *,count(good_stu) as STU FROM ".TB_GOOD." WHERE good_area='".$_SESSION['admin_area']."' and good_code='".$_SESSION['admin_school']."' and good_tail='".$_GET['good_id']."' group by good_tail"); 
 @$arr['good']= $db->fetch(@$res['good']);
@$res['tail'] = $db->select_query("SELECT * FROM ".TB_GOODTAIL." WHERE goodtail_id='".@$arr['good']['good_tail']."' "); 
@$arr['tail'] =$db->fetch(@$res['tail']);
?>
     ...
     <button type="button" class="close" data-dismiss="alert">X</button>
	  ...
	  <?php
		...
		@$res['num'] = $db->select_query("SELECT *,count(good_stu) as CO FROM ".TB_GOOD." , ".TB_STUDENT." where good_tail='".$_GET['good_id']."'  and good_stu=stu_id and good_area='".$_SESSION['admin_area']."' and good_code='".$_SESSION['admin_school']."' group by good_stu order by CO desc,stu_class,stu_id "); 
		@$rows['num'] = $db->rows(@$res['num']);
		if(@$rows['num']) {
		?>
      <form action="index.php?name=import&file=good&op=cldelall&route=<?php echo $route;?>" method="post" enctype="multipart/form-data" id="form" class="form-inline">
        <table id="example1" class="table table-bordered table-striped responsive" style="width:100%">
          <thead>
            <tr >
              <th width="1" style="text-align: center;"><?php if($_SESSION['admin_group']==1){?><input type="checkbox" id="check" class="selector flat all"><?php } else { ?>#<?php } ?></th>
		...
		?>
            <tr>
			  ...
			  <td style="text-align: center;">
			 <a href="index.php?name=import&file=good&op=studetail&good_id=<?php echo @$arr['num']['good_tail'];?>&stu_id=<?php echo @$arr['num']['stu_id'];?>&route=<?php echo $route;?>" class="btn bg-green btn-flat btn-sm" ><i class="fa fa-search-plus "></i></a>
			 <?php if($_SESSION['admin_group']==1){?>
				<a href="index.php?name=import&file=good&op=cldel&good_id=<?php echo @$arr['num']['good_tail'];?>&stu_id=<?php echo @$arr['num']['stu_id'];?>&route=<?php echo $route;?>" class="btn bg-red btn-flat btn-sm" data-confirm="<?php echo _text_box_con_delete_text;?>"><i class="fa fa-trash-o "></i></a>
				<?php } ?>
			  </td>
            </tr>
...
@$res['count'] = $db->select_query("SELECT * FROM ".TB_GOOD." group by good_id"); 
...
<div class="col-xs-12 connectedSortable">
    ...
    <div align="right" >
    <br>
      <div class="buttons"><a href="index.php?name=import&file=good&op=add&route=<?php echo $route;?>" class="btn bg-aqua btn-flat"><i class="fa fa-edit"></i>&nbsp;<?php echo _button_add; ?></a><?php if($_SESSION['admin_group']==1){?>&nbsp;&nbsp;<a onclick="$('form').submit();" class="btn bg-red btn-flat"><i class="fa fa-trash-o"></i>&nbsp;<?php echo _button_del; ?></a><?php } ?></div>
      <br>
      </div>
    <div class="box box-danger">
	  ...
	  <?php
		...
		@$res['num'] = $db->select_query("SELECT *,count(good_id) as CO FROM ".TB_GOOD.",".TB_STUDENT." where good_area='".$_SESSION['admin_area']."' and good_code='".$_SESSION['admin_school']."' and stu_id=good_stu and stu_suspend='0' group by good_tail order by CO desc"); 
		@$rows['num'] = $db->rows(@$res['num']);
		if(@$rows['num']) {
		?>
      <form action="index.php?name=import&file=good&op=delall&route=<?php echo $route;?>" method="post" enctype="multipart/form-data" id="form" class="form-inline">
        <table id="example1" class="table table-bordered table-striped responsive" style="width:100%">
          <thead>
            <tr >
              <th width="1" style="text-align: center;"><?php if($_SESSION['admin_group']==1){ ?><input type="checkbox" id="check" class="selector flat all"><?php } else { ?>#<?php } ?></th>
		...
		?>
            <tr>
              <td style="text-align: center;"><?php if($_SESSION['admin_group']==1){?><input type="checkbox" name="selected[]" value="<?php echo @$arr['tail']['goodtail_id']; ?>" class="selector flat"/><?php } else { echo $i ;}?></td>
              ...
              <td layout="block" style="text-align: center;"><?php echo @$arr['num']['CO']." ( ".number_format((@$PerC),2)." % )";?></td>
			  <td style="text-align: center;">
			 <a href="index.php?name=import&file=good&op=cldetail&good_id=<?php echo @$arr['tail']['goodtail_id'];?>&route=<?php echo $route;?>" class="btn bg-green btn-flat btn-sm" ><i class="fa fa-search-plus "></i></a>
			 <?php if($_SESSION['admin_group']==1){?>
				<a href="index.php?name=import&file=good&op=del&good_id=<?php echo @$arr['tail']['goodtail_id'];?>&route=<?php echo $route;?>" class="btn bg-red btn-flat btn-sm" data-confirm="<?php echo _text_box_con_delete_text;?>"><i class="fa fa-trash-o "></i></a>
				<?php } ?>
			  </td>
            </tr>
