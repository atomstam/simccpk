...
if($op=='cldel'){
		...
		$del .=$db->del(TB_BAD," bad_area='".$_SESSION['admin_area']."' and bad_code='".$_SESSION['admin_school']."' and bad_tail='".$_GET['bad_id']."' and bad_stu='".$_GET['stu_id']."' ");
		if($del){
		$success=_text_report_del_ok;
		} else {
		$error_warning=_text_report_del_fail;
...
if($op=='cldelall'){
		...
		while(list($key, $value) = each ($_POST['selected'])){
			...
			$del .=$db->del(TB_BAD," bad_area='".$_SESSION['admin_area']."' and bad_code='".$_SESSION['admin_school']."' and bad_stu='".$value."' and bad_tail='".$_GET['bad_id']."' ");
//			$db->closedb ();
		}
		if($del){
		$success=_text_report_del_ok;
...
if($op=='studel'){
		...
		$del .=$db->del(TB_BAD," bad_area='".$_SESSION['admin_area']."' and bad_code='".$_SESSION['admin_school']."' and bad_id='".$_GET['bad_id']."' ");
		if($del){
		$success=_text_report_del_ok;
		} else {
		$error_warning=_text_report_del_fail;
...
//twitter bootstrap script
 $("button#submitForm").click(function(){
			$.ajax({
			type: "POST",
			url: "modules/import/processbad.php",
...
//                 $("#thanks").html(msg.message),
				 ...
				 $("#error").hide();
				 setTimeout(function() {
  				 window.location='index.php?name=import&file=bad&route=<?php echo $route;?>';
				}, 1000);
			} else {
//                $("#error").html(msg.message),
	...
	//		$("#form-content").modal('hide'); 
		...
		<div align="right" ><div class="form-group"><button class="btn bg-green btn-flat" type="submit" id="submitForm" name="submitForm"><i class="fa fa-save"></i>&nbsp;<?php echo _button_save; ?></button>&nbsp;&nbsp;<a href="index.php?name=import&file=bad&route=<?php echo $route;?>" class="btn bg-red btn-flat"><i class="fa fa-undo"></i>&nbsp;<?php echo _button_reset; ?></a>
		</div></div>
				<form method="post" enctype="multipart/form-data" id="formAdd" role="formAdd" data-toggle="validator" class="form-horizontal bootstrap-validator-form" >
					    <div class="box box-success" id="loading-example">
							...
							@$res['stu'] = $db->select_query("SELECT * FROM ".TB_STUDENT." where stu_area='".$_SESSION['admin_area']."' and stu_code='".$_SESSION['admin_school']."' and stu_suspend='0' ORDER BY stu_class,stu_cn,stu_id");
...
@$res['bads'] = $db->select_query("SELECT * FROM ".TB_BAD." WHERE bad_area='".$_SESSION['admin_area']."' and bad_code='".$_SESSION['admin_school']."' and bad_id='".$_GET['bad_id']."' "); 
...
@$res['bad'] = $db->select_query("SELECT *,count(bad_stu) as STU FROM ".TB_BAD." WHERE bad_area='".$_SESSION['admin_area']."' and bad_code='".$_SESSION['admin_school']."' and bad_tail='".$_GET['bad_id']."' and bad_stu='".$_GET['stu_id']."' group by bad_tail"); 
 @$arr['bad']= $db->fetch(@$res['bad']);
@$res['tail'] = $db->select_query("SELECT * FROM ".TB_BADTAIL." WHERE badtail_area='".$_SESSION['admin_area']."' and badtail_code='".$_SESSION['admin_school']."' or badtail_area='' and badtail_code='' and badtail_id='".@$arr['bad']['bad_tail']."' "); 
@$arr['tail'] =$db->fetch(@$res['tail']);
@$res['stu'] = $db->select_query("SELECT * FROM ".TB_STUDENT." WHERE stu_area='".$_SESSION['admin_area']."' and stu_code='".$_SESSION['admin_school']."' and stu_suspend='0' and stu_id='".$_GET['stu_id']."' "); 
@$arr['stu'] =$db->fetch(@$res['stu']);
?>
     ...
     <button type="button" class="close" data-dismiss="alert">X</button>
      <span><?php echo _text_report_add_fail; ?></span>
		...
		<div align="right" >
		<div class="form-group"><a href="index.php?name=import&file=bad&op=add&route=<?php echo $route;?>" class="btn bg-aqua btn-flat"><i class="fa fa-edit"></i>&nbsp;<?php echo _button_add; ?></a><?php if($_SESSION['admin_group']==1){ ?>&nbsp;<a onclick="$('form').submit();" class="btn bg-red btn-flat"><i class="fa fa-trash-o"></i>&nbsp;<?php echo _button_del; ?></a><?php } ?>&nbsp;<a href="index.php?name=import&file=bad&route=<?php echo $route;?>" class="btn bg-orange btn-flat"><i class="fa fa-undo"></i>&nbsp;<?php echo _button_detail; ?></a>
		</div>
		</div>
	  ...
	  <?php
		...
		@$res['num'] = $db->select_query("SELECT * FROM ".TB_BAD." as a, ".TB_STUDENT." as b where bad_tail='".$_GET['bad_id']."' and bad_stu=stu_id and bad_stu='".$_GET['stu_id']."' and bad_area='".$_SESSION['admin_area']."' and bad_code='".$_SESSION['admin_school']."' order by bad_id desc "); 
		@$rows['num'] = $db->rows(@$res['num']);
		if(@$rows['num']) {
		?>
      <form action="index.php?name=import&file=bad&op=studelall&route=<?php echo $route;?>" method="post" enctype="multipart/form-data" id="form" class="form-inline">
		...
		while (@$arr['num'] = $db->fetch(@$res['num'])){
			...
			@$res['per'] = $db->select_query("SELECT * FROM ".TB_PERSON." WHERE per_area='".$_SESSION['admin_area']."' and per_code='".$_SESSION['admin_school']."' and per_ids='".@$arr['num']['bad_dam']."' "); 
			@$arr['per'] =$db->fetch(@$res['per']);
			$DaTT=@$arr['num']['bad_date']."/".@$arr['num']['bad_mouth']."/".@$arr['num']['bad_year'];
			$DATT=formatDateThai($DaTT);
		?>
            <tr>
              <td style="text-align: center;"><?php if($_SESSION['admin_group']==1){ ?><input type="checkbox" name="selected[]" value="<?php echo @$arr['num']['bad_id']; ?>" class="selector flat"/><?php } else { echo $i;} ?></td>
			  ...
			  <td style="text-align: center;">
				<a href="index.php?name=import&file=bad&op=edit&bad_id=<?php echo @$arr['num']['bad_id']; ?>&route=<?php echo $route;?>" class="btn bg-aqua btn-flat btn-sm"><i class="fa fa-edit "></i></a>
				<?php if($_SESSION['admin_group']==1){ ?>
				<a href="index.php?name=import&file=bad&op=studel&bad_id=<?php echo @$arr['num']['bad_id'];?>&route=<?php echo $route;?>" class="btn bg-red btn-flat btn-sm" data-confirm="<?php echo _text_box_con_delete_text;?>"><i class="fa fa-trash-o "></i></a>
				<?php } ?>
			  </td>
            </tr>
...
@$res['bad'] = $db->select_query("SELECT *,count(bad_stu) as STU FROM ".TB_BAD." WHERE bad_area='".$_SESSION['admin_area']."' and bad_code='".$_SESSION['admin_school']."' and bad_tail='".$_GET['bad_id']."' group by bad_tail"); 
 @$arr['bad']= $db->fetch(@$res['bad']);
@$res['tail'] = $db->select_query("SELECT * FROM ".TB_BADTAIL." WHERE badtail_area='".$_SESSION['admin_area']."' and badtail_code='".$_SESSION['admin_school']."' or badtail_area='' and badtail_code='' and badtail_id='".@$arr['bad']['bad_tail']."' "); 
@$arr['tail'] =$db->fetch(@$res['tail']);
?>
     ...
     <button type="button" class="close" data-dismiss="alert">X</button>
	  ...
	  <?php
		...
		@$res['num'] = $db->select_query("SELECT *,count(bad_stu) as CO FROM ".TB_BAD." as a, ".TB_STUDENT." as b where bad_tail='".$_GET['bad_id']."'  and bad_stu=stu_id  and bad_area='".$_SESSION['admin_area']."' and bad_code='".$_SESSION['admin_school']."' group by bad_stu order by CO desc,stu_class,stu_id "); 
		@$rows['num'] = $db->rows(@$res['num']);
		if(@$rows['num']) {
		?>
      <form action="index.php?name=import&file=bad&op=cldelall&route=<?php echo $route;?>" method="post" enctype="multipart/form-data" id="form" class="form-inline">
        <table id="example1" class="table table-bordered table-striped responsive" style="width:100%">
          <thead>
            <tr >
              <th width="1" style="text-align: center;"><?php if($_SESSION['admin_group']==1){ ?><input type="checkbox" id="check" class="selector flat all"><?php } else {?>#<?php } ?></th>
		...
		?>
            <tr>
			  ...
			  <td style="text-align: center;">
			 <a href="index.php?name=import&file=bad&op=studetail&bad_id=<?php echo @$arr['num']['bad_tail'];?>&stu_id=<?php echo @$arr['num']['stu_id'];?>&route=<?php echo $route;?>" class="btn bg-green btn-flat btn-sm" ><i class="fa fa-search-plus "></i></a>
			 <?php if($_SESSION['admin_group']==1){ ?>
				<a href="index.php?name=import&file=bad&op=cldel&bad_id=<?php echo @$arr['num']['bad_tail'];?>&stu_id=<?php echo @$arr['num']['stu_id'];?>&route=<?php echo $route;?>" class="btn bg-red btn-flat btn-sm" data-confirm="<?php echo _text_box_con_delete_text;?>"><i class="fa fa-trash-o "></i></a>
				<?php } ?>
			  </td>
            </tr>
...
@$res['count'] = $db->select_query("SELECT * FROM ".TB_BAD." where bad_area='".$_SESSION['admin_area']."' and bad_code='".$_SESSION['admin_school']."' group by bad_id"); 
...
<div class="col-xs-12 connectedSortable">
    ...
    <div align="right" >
    <br>
      <div class="buttons"><a href="index.php?name=import&file=bad&op=add&route=<?php echo $route;?>" class="btn bg-aqua btn-flat"><i class="fa fa-edit"></i>&nbsp;<?php echo _button_add; ?></a><?php if($_SESSION['admin_group']==1){ ?>&nbsp;<a onclick="$('form').submit();" class="btn bg-red btn-flat"><i class="fa fa-trash-o"></i>&nbsp;<?php echo _button_del; ?></a><?php } ?></div>
      <br>
      </div>
    <div class="box box-danger">
	  ...
	  <?php
		...
		@$res['num'] = $db->select_query("SELECT *,count(bad_id) as CO FROM ".TB_BAD.",".TB_STUDENT." where bad_area='".$_SESSION['admin_area']."' and bad_code='".$_SESSION['admin_school']."' and stu_id=bad_stu and stu_suspend='0' group by bad_tail order by CO desc"); 
		@$rows['num'] = $db->rows(@$res['num']);
		if(@$rows['num']) {
		?>
      <form action="index.php?name=import&file=bad&op=delall&route=<?php echo $route;?>" method="post" enctype="multipart/form-data" id="form" class="form-inline">
        <table id="example1" class="table table-bordered table-striped responsive" style="width:100%">
          <thead>
            <tr >
              <th width="1" style="text-align: center;"><?php if($_SESSION['admin_group']==1){ ?><input type="checkbox" id="check" class="selector flat all"><?php } else {?>#<?php } ?></th>
		...
		$i=1;
		while (@$arr['num'] = $db->fetch(@$res['num'])){
		@$res['tail'] = $db->select_query("SELECT * FROM ".TB_BADTAIL." WHERE badtail_area='".$_SESSION['admin_area']."' and badtail_code='".$_SESSION['admin_school']."' or badtail_area='' and badtail_code='' and badtail_id='".@$arr['num']['bad_tail']."' "); 
		@$arr['tail'] =$db->fetch(@$res['tail']);
		@$res['level'] = $db->select_query("SELECT * FROM ".TB_BADLEVEL." WHERE blevel_id='".@$arr['tail']['badtail_level']."' "); 
		@$arr['level'] =$db->fetch(@$res['level']);
		...
		?>
            <tr>
              <td style="text-align: center;"><?php if($_SESSION['admin_group']==1){ ?><input type="checkbox" name="selected[]" value="<?php echo @$arr['tail']['badtail_id']; ?>" class="selector flat"/><?php } else { echo $i;} ?></td>
              ...
              <td layout="block" style="text-align: center;"><?php echo @$arr['num']['CO']." ( ".number_format((@$PerC),2)." % )";?></td>
			  <td style="text-align: center;">
			 <a href="index.php?name=import&file=bad&op=cldetail&bad_id=<?php echo @$arr['tail']['badtail_id'];?>&route=<?php echo $route;?>" class="btn bg-green btn-flat btn-sm" ><i class="fa fa-search-plus "></i></a>
			 <?php if($_SESSION['admin_group']==1){ ?>
				<a href="index.php?name=import&file=bad&op=del&bad_id=<?php echo @$arr['tail']['badtail_id'];?>&route=<?php echo $route;?>" class="btn bg-red btn-flat btn-sm" data-confirm="<?php echo _text_box_con_delete_text;?>"><i class="fa fa-trash-o "></i></a>
				<?php } ?>
			  </td>
            </tr>
...
<?php
		...
		@$res['num'] = $db->select_query("select stu_id,stu_pic,stu_pid,stu_num,stu_name,stu_sur,class_name,stu_class,sum(badtail_point) as CO  from ".TB_BAD." ,".TB_STUDENT.",".TB_BADTAIL.",".TB_CLASS." where bad_area='".$_SESSION['admin_area']."' and bad_code='".$_SESSION['admin_school']."' and stu_id=bad_stu and bad_tail=badtail_id and class_id=stu_class and stu_suspend='0' group by stu_id order by CO desc"); 
		@$rows['num'] = $db->rows(@$res['num']);
		if(@$rows['num']) {
...
//                '-'+pageTotal9 +' ( -'+ total9 +' <?php echo _text_box_table_pu_score;?>)'
			'-'+ total9 +' <?php echo _text_box_table_pu_score;?>'
            );
        }
								});
