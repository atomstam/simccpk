...
CheckAdmin($_SESSION['admin_login'],$_SESSION['admin_pwd'],$_SESSION['admin_group']);
@$res['user'] = $db->select_query("SELECT * FROM ".TB_ADMIN." WHERE area_code ='".$_SESSION['admin_area']."' and school_code='".$_SESSION['admin_school']."'  and username='".$_SESSION['admin_login']."' "); 
...
<div class="wrapper">
    ...
    <nav class="navbar navbar-static-top">
      ...
      <!-- Navbar Right Menu -->
<?php
@$res['comx'] = $db->select_query("SELECT * FROM ".TB_MESSAGE.",".TB_MESSAGE_CHECK." where ms_to in ('all' ,'".$_SESSION['admin_login']."')  and msc_mss !=ms_id  and msc_user='".$_SESSION['admin_login']."' and ms_area='".$_SESSION['admin_area']."' and ms_school='".$_SESSION['admin_school']."' group by ms_id order by ms_id desc limit 5"); 
...
?>
                    ...
                    <ul class="nav navbar-nav">
                        ...
                        <li class="dropdown messages-menu">
                            ...
                            <ul class="dropdown-menu">
                                ...
                                <li>
									...
									@$res['ms'] = $db->select_query("SELECT * FROM ".TB_MESSAGE." as a,".TB_MESSAGE_CHECK." as b where a.ms_to in ('all' ,'".$_SESSION['admin_login']."') and ms_area='".$_SESSION['admin_area']."' and ms_school='".$_SESSION['admin_school']."' and b.msc_mss !=a.ms_id  group by a.ms_id order by a.ms_id desc limit 5"); 
									while (@$arr['ms'] = $db->fetch(@$res['ms'])){
									@$res['userx'] = $db->select_query("SELECT * FROM ".TB_ADMIN." where username='".@$arr['ms']['ms_posted']."' "); 
									...
									?>
                                        <li><!-- start message -->
                                            <a href="index.php?name=access&file=message&op=detail&route=<?php echo $route;?>&MsID=<?php echo @$arr['ms']['ms_id'];?>">
                                                <div class="pull-left">
                                                    <img src="<?php if(@$arr['userx']['img']){echo WEB_URL_IMG_ADMIN.@$arr['userx']['img'];}else{echo WEB_URL_IMG_ADMIN."no_image.jpg";}?>" class="img-circle" alt="User Image"/>
                                                </div>
                                                <h4>
                        ...
                        <li class="dropdown user user-menu">
                            <a href="#" class="dropdown-toggle" data-toggle="dropdown">
                                ...
                                <span><?php echo @$arr['user']['username']; ?> <i class="caret"></i></span>
                            </a>
                            <ul class="dropdown-menu">
                                <!-- User image -->
                                <li class="user-header bg-light-blue">
                                     <?php if(!empty(@$arr['user']['img'])){?>
                                            <img src="<?php echo WEB_URL_IMG_ADMIN.@$arr['user']['img']; ?>" class="img-circle" alt="User Image" />
                                     <?php } else {?>
			...
			<aside class="main-sidebar">
				...
				<!-- Sidebar user panel -->
					<div class="user-panel">
						...
						@$res['online'] = $db->select_query("SELECT * FROM ".TB_ADMIN_ONLINE." as a , ".TB_ADMIN." as b WHERE b.area_code ='".$_SESSION['admin_area']."' and b.school_code='".$_SESSION['admin_school']."'  and b.username=a.u_user and u_timeout > '".time()."' "); 
			...
			@$res['menu'] = $db->select_query("SELECT * FROM ".TB_ADMIN_MENU." where type='1' and status='1' order by menu_id "); 
			...
			@$res['submenu'] = $db->select_query("SELECT * FROM ".TB_ADMIN_MENU." where main='".$MainMenu['mods']."' and status='1' order by sort "); 
			//@$arr['menu'] = $db->fetch(@$res['menu']);
			?>
			<?php while ($subMenu = $db->fetch(@$res['submenu'])) { ?>
								...
								<?php if(getTotalAdmin($subMenu['mods'],$subMenu['dbs'],$_SESSION['admin_login']) !=0){?>
								<span class="pull-right-container">
								<small class="<?php echo $subMenu['class']; ?>"><?php if($subMenu['mods']=='message'){echo getTotalAdmin($subMenu['mods'],$subMenu['dbs'],$_SESSION['admin_login']);} else {echo getTotalAdmin($subMenu['mods'],$subMenu['dbs'],$_SESSION['admin_login']);}?>
								</small>
								</span>
								<?php } ?>
								</a>
			...
			<?php } else { 
			@$res['menu'] = $db->select_query("SELECT * FROM ".TB_ADMIN_MENU." where type='1' and gr='0' and status='1' order by menu_id "); 
			//@$arr['menu'] = $db->fetch(@$res['menu']);
			?>
			<?php while ($MainMenu = $db->fetch(@$res['menu'])) { ?>
                        <li <?php if($MainMenu['mods']==$name){ echo "class=\"active treeview\"";} else { echo "class=\"treeview\"";}?> >
			...
			@$res['submenu'] = $db->select_query("SELECT * FROM ".TB_ADMIN_MENU." where main='".$MainMenu['mods']."' and gr='0' or gr='".$_SESSION['admin_group']."' and status='1' order by sort "); 
			//@$arr['menu'] = $db->fetch(@$res['menu']);
			?>
			<?php while ($subMenu = $db->fetch(@$res['submenu'])) { ?>
                                <li <?php if($subMenu['MFiles']==$route){ echo "class=\"active\"";}?>><a href="index.php?name=<?php echo $subMenu['mods'];?>&file=<?php echo $subMenu['dbs'];?>&route=<?php echo $subMenu['MFiles']; ?>"><i class="fa fa-angle-double-right"></i> <?php if($file==$subMenu['dbs'] && $subMenu['MFiles']==$route){echo "<font color='#FFFF00'>".$subMenu['name']."</font>"; } else {echo $subMenu['name'];}?>
  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
	      <h4>
		<i class="fa fa-graduation-cap"></i> <?php echo $_SESSION['school_name']." [".$_SESSION['admin_school']."]"; ?>
         </h4>
	<ol class="breadcrumb">
