...
if($op=='del'){
		...
		$del .=$db->del(TB_GD_TRAN," tran_area='".$_SESSION['admin_area']."' and tran_school='".$_SESSION['admin_school']."' and tran_subj='".@$arr['subj']['subj_pin']."' ");

		$del .=$db->del(TB_GD_SUBJ," subj_area='".$_SESSION['admin_area']."' and subj_school='".$_SESSION['admin_school']."' and id='".$_GET['st_id']."' ");
		if($del){
...
if($op=='delall'){
		...
		while(list($key, $value) = each ($_POST['selected'])){
			...
			@$arr['subj'] = $db->fetch(@$res['subj']);
			$del .=$db->del(TB_GD_TRAN," tran_area='".$_SESSION['admin_area']."' and tran_school='".$_SESSION['admin_school']."' and tran_subj='".@$arr['subj']['subj_pin']."' ");
			$del .=$db->del(TB_GD_SUBJ," subj_area='".$_SESSION['admin_area']."' and subj_school='".$_SESSION['admin_school']."' and id='".$value."' ");
//			$db->closedb ();
		}
		if($del){
		$success=_text_report_del_ok;
		...
		<div align="right" ><div class="form-group"><button class="btn bg-green btn-flat" type="submit" id="submitForm" name="submitForm"><i class="fa fa-save"></i>&nbsp;<?php echo _button_save; ?></button>&nbsp;&nbsp;<a href="index.php?name=academic&file=subj&route=<?php echo $route;?>" class="btn bg-red btn-flat"><i class="fa fa-undo"></i>&nbsp;<?php echo _button_reset; ?></a>
				...
				<form method="post" enctype="multipart/form-data" id="formAdd" role="formAdd" data-toggle="validator" class="form-horizontal bootstrap-validator-form" >
					    <div class="box box-success" id="loading-example">
							...
							<?php
							@$res['per'] = $db->select_query("SELECT * FROM ".TB_GD_TEACH." where teach_area='".$_SESSION['admin_area']."' and teach_school='".$_SESSION['admin_school']."' ORDER BY id ");
...
@$res['subj'] = $db->select_query("SELECT *  FROM ".TB_GD_SUBJ." WHERE subj_area='".$_SESSION['admin_area']."' and subj_school='".$_SESSION['admin_school']."' and  id='".$_GET['st_id']."' group by id"); 
@$arr['subj']= $db->fetch(@$res['subj']);
@$res['per'] = $db->select_query("SELECT * FROM ".TB_GD_TEACH." where teach_area='".$_SESSION['admin_area']."' and teach_school='".$_SESSION['admin_school']."' and teach_pin='".@$arr['subj']['subj_teach']."' ");
...
$YY=YEAR-1;
@$res['count'] = $db->select_query("SELECT * FROM ".TB_GD_SUBJ." where subj_area='".$_SESSION['admin_area']."' and subj_school='".$_SESSION['admin_school']."' and subj_year='".$YY."' ORDER BY id  "); 
  ...
  <div id="Tab3Y">
    ...
    <div class="box box-danger">
	  ...
	  <?php
		...
		@$res['subj'] = $db->select_query("SELECT * FROM ".TB_GD_SUBJ." where subj_area='".$_SESSION['admin_area']."' and subj_school='".$_SESSION['admin_school']."' and subj_year='".$YY."' ORDER BY id  ");
