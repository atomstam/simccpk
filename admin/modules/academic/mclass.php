			...
			if (isset($_POST['StuID'][$i])) {
				if (empty($_POST['Ck1'][$i])) {
					if (isset($_POST['Ch1'][$i])) {
						$sql = $db->select_query("SELECT * FROM " . TB_BADTAIL . " WHERE badtail_area='" . $_SESSION['admin_area'] . "' and badtail_code='" . $_SESSION['admin_school'] . "'  and badtail_name='หนีเรียน' ");
						@$result = $db->fetch($sql);
					...
					if ($_POST['Ck1'][$i] == '1') {
						$sql = $db->select_query("SELECT * FROM " . TB_BADTAIL . " WHERE badtail_area='" . $_SESSION['admin_area'] . "' and badtail_code='" . $_SESSION['admin_school'] . "'  and badtail_name='ขาดเรียน' ");
						@$result = $db->fetch($sql);
				...
				<div class="box box-success" id="loading-example">
					...
					<div class="box-body  ">
						...
						<script type="text/javascript">
							$(function() {
								$("select#classlist5").change(function() {
									var datalist4 = $.ajax({ // รับค่าจาก ajax เก็บไว้ที่ตัวแปร datalist2
										url: "modules/academic/classlist4.php", // ไฟล์สำหรับการกำหนดเงื่อนไข
										data: {
											"class_cn": $(this).val(),
						...
						<form method="post" action="index.php?name=academic&file=mclass&op=AddTab4&route=<?php echo $route; ?>" enctype="multipart/form-data" id="formAdd" role="formAdd" data-toggle="validator" class="form-horizontal bootstrap-validator-form">
							...
							<div class="form-group has-feedback">
								...
								<div class="col-sm-2">
									<select class="form-control css-require" id="classlist5" name="Stu_cn" required="required">
