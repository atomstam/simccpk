...
if($op=='del'){
		...
		$del .=$db->del(TB_BAD," bad_area='".$_SESSION['stu_area']."' and bad_code='".$_SESSION['stu_school']."' and bad_tail='".$_GET['bad_id']."' ");
		if($del){
...
if($op=='cldel'){
		...
		$del .=$db->del(TB_BAD," bad_area='".$_SESSION['stu_area']."' and bad_code='".$_SESSION['stu_school']."' and bad_tail='".$_GET['bad_id']."' and bad_stu='".$_GET['stu_id']."' ");
		if($del){
		$success=_text_report_del_ok;
		} else {
		$error_warning=_text_report_del_fail;
...
if($op=='delall'){
		...
		while(list($key, $value) = each ($_POST['selected'])){
			...
			$del .=$db->del(TB_BAD," bad_area='".$_SESSION['stu_area']."' and bad_code='".$_SESSION['stu_school']."' and bad_tail='".$value."' ");
//			$db->closedb ();
		}
		if($del){
		$success=_text_report_del_ok;
...
if($op=='cldelall'){
		...
		while(list($key, $value) = each ($_POST['selected'])){
			...
			$del .=$db->del(TB_BAD," bad_area='".$_SESSION['stu_area']."' and bad_code='".$_SESSION['stu_school']."' and bad_stu='".$value."' and bad_tail='".$_GET['bad_id']."' ");
//			$db->closedb ();
		}
		if($del){
		$success=_text_report_del_ok;
...
if($op=='studel'){
		...
		$del .=$db->del(TB_BAD," bad_area='".$_SESSION['stu_area']."' and bad_code='".$_SESSION['stu_school']."' and bad_id='".$_GET['bad_id']."' ");
		if($del){
		$success=_text_report_del_ok;
		} else {
		$error_warning=_text_report_del_fail;
		...
		<div align="right" ><div class="form-group"><button class="btn bg-green btn-flat" type="submit" id="submitForm" name="submitForm"><i class="fa fa-save"></i>&nbsp;<?php echo _button_save; ?></button>&nbsp;&nbsp;<a href="index.php?name=behavior&file=bad&route=<?php echo $route;?>" class="btn bg-red btn-flat"><i class="fa fa-undo"></i>&nbsp;<?php echo _button_reset; ?></a>
				...
				<form method="post" enctype="multipart/form-data" id="formAdd" role="formAdd" data-toggle="validator" class="form-horizontal bootstrap-validator-form" >
					    <div class="box box-success" id="loading-example">
							...
							@$res['bad'] = $db->select_query("SELECT * FROM ".TB_BADTAIL." where badtail_area='".$_SESSION['stu_area']."' and badtail_code='".$_SESSION['stu_school']."' ORDER BY badtail_id ");
...
@$res['bads'] = $db->select_query("SELECT * FROM ".TB_BAD." WHERE bad_area='".$_SESSION['stu_area']."' and bad_code='".$_SESSION['stu_school']."' and bad_id='".$_GET['bad_id']."' "); 
...
@$res['bad'] = $db->select_query("SELECT *,count(bad_stu) as STU FROM ".TB_BAD." WHERE bad_area='".$_SESSION['stu_area']."' and bad_code='".$_SESSION['stu_school']."' and bad_tail='".$_GET['bad_id']."' and bad_stu='".$_GET['stu_id']."' group by bad_tail"); 
 @$arr['bad']= $db->fetch(@$res['bad']);
@$res['tail'] = $db->select_query("SELECT * FROM ".TB_BADTAIL." WHERE badtail_area='".$_SESSION['stu_area']."' and badtail_code='".$_SESSION['stu_school']."' and badtail_id='".@$_GET['bad_id']."' "); 
@$arr['tail'] =$db->fetch(@$res['tail']);
@$res['stu'] = $db->select_query("SELECT * FROM ".TB_STUDENT." WHERE stu_area='".$_SESSION['stu_area']."' and stu_code='".$_SESSION['stu_school']."' and stu_suspend='0' and stu_id='".$_GET['stu_id']."' "); 
@$arr['stu'] =$db->fetch(@$res['stu']);
?>
     ...
     <button type="button" class="close" data-dismiss="alert">X</button>
	  ...
	  <?php
		...
		@$res['num'] = $db->select_query("SELECT * FROM ".TB_BAD." as a, ".TB_STUDENT." as b where bad_tail='".$_GET['bad_id']."' and bad_stu=stu_id and bad_stu='".$_GET['stu_id']."' and bad_area='".$_SESSION['stu_area']."' and bad_code='".$_SESSION['stu_school']."' order by bad_id desc "); 
      ...
      <form action="index.php?name=behavior&file=bad&op=studelall&route=<?php echo $route;?>" method="post" enctype="multipart/form-data" id="form" class="form-inline">
		...
		while (@$arr['num'] = $db->fetch(@$res['num'])){
			...
			@$res['per'] = $db->select_query("SELECT * FROM ".TB_PERSON." WHERE per_area='".$_SESSION['stu_area']."' and per_code='".$_SESSION['stu_school']."' and per_ids='".@$arr['num']['bad_dam']."' "); 
...
@$res['bad'] = $db->select_query("SELECT *,count(bad_stu) as STU FROM ".TB_BAD.",".TB_STUDENT." where bad_area='".$_SESSION['stu_area']."' and bad_code='".$_SESSION['stu_school']."' and stu_class='".$_SESSION['stu_class']."' and stu_cn='".$_SESSION['stu_cn']."'  and stu_id=bad_stu and stu_suspend='0' and bad_tail='".$_GET['bad_id']."' group by bad_tail"); 
 @$arr['bad']= $db->fetch(@$res['bad']);
...
?>
     ...
     <button type="button" class="close" data-dismiss="alert">X</button>
	  ...
	  <?php
		...
		@$res['num'] = $db->select_query("SELECT *,count(bad_stu) as CO FROM ".TB_BAD." as a, ".TB_STUDENT." as b where a.bad_tail='".$_GET['bad_id']."'  and a.bad_stu=b.stu_id  and bad_area='".$_SESSION['stu_area']."' and b.stu_cn='".$_SESSION['stu_cn']."' and b.stu_class='".$_SESSION['stu_class']."' and b.stu_cn='".$_SESSION['stu_cn']."' group by a.bad_stu order by CO desc,b.stu_class,b.stu_id "); 
		@$rows['num'] = $db->rows(@$res['num']);
		if(@$rows['num']) {
		?>
...
@$res['count'] = $db->select_query("SELECT * FROM ".TB_BAD.",".TB_STUDENT." where bad_area='".$_SESSION['stu_area']."' and bad_code='".$_SESSION['stu_school']."' and stu_id=bad_stu and stu_class='".$_SESSION['stu_class']."' and stu_cn='".$_SESSION['stu_cn']."'  and stu_suspend='0' group by bad_id"); 
...
<div class="col-xs-12 connectedSortable">
    ...
    <div class="box box-danger">
	  ...
	  <?php
		...
		@$res['nums'] = $db->select_query("SELECT *,count(bad_id) as CO FROM ".TB_BAD." as a,".TB_STUDENT." as b where a.bad_area='".$_SESSION['stu_area']."' and a.bad_code='".$_SESSION['stu_school']."' and b.stu_class='".$_SESSION['stu_class']."' and b.stu_cn='".$_SESSION['stu_cn']."'  and b.stu_id=a.bad_stu and b.stu_suspend='0' group by a.bad_tail order by CO desc"); 
      ...
      <form action="index.php?name=behavior&file=bad&op=delall&route=<?php echo $route;?>" method="post" enctype="multipart/form-data" id="form" class="form-inline">
		...
		@$res['tail'] = $db->select_query("SELECT * FROM ".TB_BADTAIL." WHERE badtail_area='".$_SESSION['stu_area']."' and badtail_code='".$_SESSION['stu_school']."' and badtail_id='".@$arr['nums']['bad_tail']."' "); 
		@$arr['tail'] =$db->fetch(@$res['tail']);
		@$res['level'] = $db->select_query("SELECT * FROM ".TB_BADLEVEL." WHERE blevel_id='".@$arr['tail']['badtail_level']."' "); 
		@$arr['level'] =$db->fetch(@$res['level']);
		@$PerC=(100*(@$arr['nums']['CO']))/(@$rows['count']);
		?>
            <tr>
