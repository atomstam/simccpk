...
@$res['bad'] = $db->select_query("SELECT *,count(bad_stu) as STU FROM ".TB_BAD." WHERE bad_area='".$_SESSION['stu_area']."' and bad_code='".$_SESSION['stu_school']."' and bad_stu='".$_SESSION['stu_pwd']."' "); 
 @$arr['bad']= $db->fetch(@$res['bad']);
@$res['stu'] = $db->select_query("SELECT * FROM ".TB_STUDENT." WHERE stu_area='".$_SESSION['stu_area']."' and stu_code='".$_SESSION['stu_school']."' and stu_suspend='0' and stu_id='".$_SESSION['stu_pwd']."' "); 
@$arr['stu'] =$db->fetch(@$res['stu']);
?>
     ...
     <button type="button" class="close" data-dismiss="alert">X</button>
      <span><?php echo _text_report_add_fail; ?></span>
		...
		<div align="right" >
		<div class="form-group"><!--<a href="index.php?name=behavior&file=bad&op=add&route=<?php echo $route;?>" class="btn bg-aqua btn-flat"><i class="fa fa-edit"></i>&nbsp;<?php echo _button_add; ?></a>&nbsp;--><a href="index.php?name=behavior&file=bad&route=<?php echo $route;?>" class="btn bg-orange btn-flat"><i class="fa fa-undo"></i>&nbsp;<?php echo _button_detail; ?></a>
		</div>
		</div>
	  ...
	  <?php
		...
		@$res['num'] = $db->select_query("SELECT * FROM ".TB_BAD." as a, ".TB_STUDENT." as b,".TB_BADTAIL." as c  where  a.bad_stu=b.stu_id and a.bad_stu='".$_SESSION['stu_pwd']."' and a.bad_area='".$_SESSION['stu_area']."' and a.bad_code='".$_SESSION['stu_school']."' and a.bad_tail=c.badtail_id order by a.bad_id desc "); 
