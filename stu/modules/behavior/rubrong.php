		...
		<div align="right" ><div class="form-group"><button class="btn bg-green btn-flat" type="submit" id="submitForm" name="submitForm"><i class="fa fa-save"></i>&nbsp;<?php echo _button_save; ?></button>&nbsp;&nbsp;<a href="index.php?name=behavior&file=rubrong&route=<?php echo $route;?>" class="btn bg-red btn-flat"><i class="fa fa-undo"></i>&nbsp;<?php echo _button_reset; ?></a>
				...
				<form method="post" enctype="multipart/form-data" id="formAdd" role="formAdd" data-toggle="validator" class="form-horizontal bootstrap-validator-form" >
					    <div class="box box-success" id="loading-example">
							...
							<input type="hidden" name="rub_stu[]" value="<?=$_SESSION['stu_pwd'];?>">
							...
							@$res['bt'] = $db->select_query("SELECT * FROM ".TB_RUBRONG." where rb_area='".$_SESSION['stu_area']."' and rb_code='".$_SESSION['stu_school']."' ORDER BY rb_id ");
...
<?php
		...
		@$arr['stu'] =$db->fetch(@$res['stu']);
		@$res['best'] = $db->select_query("SELECT * FROM ".TB_RUBRONGTAIL." where rub_area='".$_SESSION['stu_area']."' and rub_code='".$_SESSION['stu_school']."' and rub_id='".$_GET['rub_id']."'"); 
...
@$res['bad'] = $db->select_query("SELECT *,count(rub_stu) as STU FROM ".TB_RUBRONGTAIL." where rub_area='".$_SESSION['stu_area']."' and rub_code='".$_SESSION['stu_school']."' and rub_tail='".$_GET['rub_id']."' group by rub_tail"); 
 @$arr['bad']= $db->fetch(@$res['bad']);
@$res['tail'] = $db->select_query("SELECT * FROM ".TB_RUBRONG." where rb_area='".$_SESSION['stu_area']."' and rb_code='".$_SESSION['stu_school']."' and rb_id='".$_GET['rub_id']."' "); 
...
?>
     ...
     <button type="button" class="close" data-dismiss="alert">X</button>
	  ...
	  <?php
		...
		@$res['num'] = $db->select_query("SELECT * FROM ".TB_RUBRONGTAIL." as a, ".TB_STUDENT." as b where a.rub_area='".$_SESSION['stu_area']."' and a.rub_code='".$_SESSION['stu_school']."' a.rub_tail='".$_GET['rub_id']."'  and a.rub_stu=b.stu_id order by b.stu_class,b.stu_id "); 
...
<?php
		...
		@$res['stu'] = $db->select_query("SELECT * FROM ".TB_STUDENT." WHERE stu_area='".$_SESSION['stu_area']."' and stu_code='".$_SESSION['stu_school']."' and stu_id='".$_GET['stu_id']."' "); 
		@$arr['stu'] =$db->fetch(@$res['stu']);
		@$res['nut'] = $db->select_query("SELECT * FROM ".TB_RUBRONGTAIL." WHERE rub_area='".$_SESSION['stu_area']."' and rub_code='".$_SESSION['stu_school']."' and rub_id='".$_GET['rub_id']."'"); 
...
<?php
		...
		@$res['numg'] = $db->select_query("select *,sum(goodtail_point) as CO  from ".TB_GOOD." ,".TB_STUDENT.",".TB_GOODTAIL.",".TB_CLASS." where stu_area='".$_SESSION['stu_area']."' and stu_code='".$_SESSION['stu_school']."' and stu_id=good_stu and good_tail=goodtail_id and class_id=stu_class group by stu_id HAVING CO > 0 order by CO desc,stu_class,stu_cn desc  limit 1"); 
		@$rows['numg'] = $db->fetch(@$res['numg']);
		$G=@$rows['numg']['CO'];

		@$res['gtail'] = $db->select_query("SELECT *,sum(goodtail_point) As GP FROM ".TB_GOOD." as a ,".TB_GOODTAIL." as b WHERE a.good_stu='".$_GET['stu_id']."' and a.good_area='".$_SESSION['stu_area']."' and a.good_code='".$_SESSION['stu_school']."' and a.good_tail=b.goodtail_id "); 
		@$arr['gtail'] =$db->fetch(@$res['gtail']);
		@$res['btail'] = $db->select_query("SELECT *,sum(badtail_point) As BP FROM ".TB_BAD." as a ,".TB_BADTAIL." as b WHERE a.bad_stu='".$_GET['stu_id']."' and a.bad_area='".$_SESSION['stu_area']."' and a.bad_code='".$_SESSION['stu_school']."' and a.bad_tail=b.badtail_id "); 
		...
		<div class="form-group"><!--<button class="btn bg-green btn-flat" type="submit" id="submitForm" name="submitForm"><i class="fa fa-save"></i>&nbsp;<?php echo _button_save; ?></button>&nbsp;&nbsp;--><a href="index.php?name=behavior&file=rubrong&route=<?php echo $route;?>" class="btn bg-orange btn-flat"><i class="fa fa-undo"></i>&nbsp;<?php echo _button_detail; ?></a>
				...
				<form method="post" enctype="multipart/form-data" id="formUp" role="formUp" data-toggle="validator" class="form-horizontal bootstrap-validator-form" >
					    <div class="box box-info" id="loading-example">
							...
							<div class="col-sm-2"><input type="text" name="Num" id="Num"  class="form-control css-require " required disabled></div>
							</div>
							<div class="form-group has-feedback" >
							<label class="col-sm-3 control-label" >ปีการศึกษา</label>
							<div class="col-sm-2"><input type="text" name="Year" id="Year"  class="form-control css-require " required disabled></div>
							</div>
						    <div class="form-group has-feedback">
							<label class="col-sm-3 control-label" ><?php echo _text_box_table_stu_exit_status; ?></label>
							<div class="col-sm-4" >
							<input type="radio" name="Stu_fstatus" class="minimal" value="1" checked disabled>&nbsp;<?php echo _text_box_table_stu_exit_status_ok;?>&nbsp;&nbsp;
							<input type="radio" name="Stu_fstatus" class="minimal" value="2" disabled>&nbsp;<?php echo _text_box_table_stu_exit_status_no;?>&nbsp;&nbsp;
							</div>
							</div>
							...
							<div class="col-sm-4" >
							<input type="radio" name="Stu_check" class="minimal" value="1" disabled>&nbsp;ใช้&nbsp;&nbsp;
							<input type="radio" name="Stu_check" class="minimal" value="0" checked disabled>&nbsp;ไม่ใช้&nbsp;&nbsp;
							</div>
							</div>
							<?php } ?>
							<div class="form-group">
...
@$res['nut'] = $db->select_query("SELECT *,count(rb_id) as STU FROM ".TB_RUBRONG." where rb_area='".$_SESSION['stu_area']."' and rb_code='".$_SESSION['stu_school']."' group by rb_stu"); 
 @$arr['nut']= $db->fetch(@$res['nut']);
@$res['stu'] = $db->select_query("SELECT * FROM ".TB_STUDENT." where  stu_area='".$_SESSION['stu_area']."' and stu_code='".$_SESSION['stu_school']."' and stu_id='".$_GET['stu_id']."' "); 
@$arr['stu'] = $db->fetch(@$res['stu']);
...
?>
	  ...
	  <?php
		@$res['num'] = $db->select_query("SELECT * FROM ".TB_RUBRONGTAIL." , ".TB_STUDENT." where  rub_area='".$_SESSION['stu_area']."' and rub_code='".$_SESSION['stu_school']."' and rub_stu=stu_id order by stu_class,stu_id "); 
		@$rows['num'] = $db->rows(@$res['num']);
		if(@$rows['num']) {
		?>
...
@$res['count'] = $db->select_query("SELECT * FROM ".TB_RUBRONGTAIL." where and rub_stu='".$_SESSION['stu_pwd']."' group by rub_id"); 
...
<div class="col-xs-12 connectedSortable">
    ...
    <div align="right" >
    <br>
      <div class="buttons"><a href="index.php?name=behavior&file=rubrong&op=add&route=<?php echo $route;?>" class="btn bg-aqua btn-flat"><i class="fa fa-edit"></i>&nbsp;<?php echo _button_add; ?></a></div>
      <br>
      </div>
    <div class="box box-danger">
	  ...
	  <?php
		...
		@$res['num'] = $db->select_query("SELECT * FROM ".TB_RUBRONGTAIL." , ".TB_STUDENT." where  rub_area='".$_SESSION['stu_area']."' and rub_code='".$_SESSION['stu_school']."' and rub_stu='".$_SESSION['stu_pwd']."' and rub_stu=stu_id order by stu_class,stu_id "); 
		@$rows['num'] = $db->rows(@$res['num']);
		if(@$rows['num']) {
		?>
      <form action="index.php?name=behavior&file=rubrong&op=delall&route=<?php echo $route;?>" method="post" enctype="multipart/form-data" id="form" class="form-inline" >
        <table id="example1" class="table table-bordered table-striped responsive" style="width:100%">
          <thead>
            <tr >
              ...
              <th layout="block" style="text-align:center;" >เหตุผลที่ขอ</th>
		...
		@$res['class'] = $db->select_query("SELECT * FROM ".TB_CLASS." WHERE class_id='".@$arr['num']['stu_class']."' "); 
		@$arr['class'] =$db->fetch(@$res['class']);
		@$res['rub'] = $db->select_query("SELECT * FROM ".TB_RUBRONG." WHERE rb_area='".$_SESSION['stu_area']."' and rb_code='".$_SESSION['stu_school']."' and rb_id='".@$arr['num']['rub_rb']."' "); 
		@$arr['rub'] =$db->fetch(@$res['rub']);
		?>
            <tr>
              <td style="text-align: center;"><?=$i;?></td>
              <td layout="block" style="text-align: left;"><?php echo @$arr['num']['rub_fare'];?></td>
              <td layout="block" style="text-align: left;"><?php echo $arr['rub']['rb_name'] ;?></td>
