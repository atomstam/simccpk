...
@$res['chkclass'] = $db->select_query("SELECT * FROM ".TB_CHK_CHCLASS." WHERE chk_area='".$_SESSION['stu_area']."' and chk_code='".$_SESSION['stu_school']."'  and chk_class='".$_POST['ClassID']."' and chk_cn='".$_POST['Stu_cn']."' and chk_gr='1' and chk_date='".$_POST['DateID']."' "); 
...
if($arr['chkclass']==0){
	...
	for ($i=1; $i < $_POST['rank']; $i++) {
		...
		$sq4=$db->select_query("SELECT * FROM ".TB_BAD." WHERE bad_area='".$_SESSION['stu_area']."' and bad_code='".$_SESSION['stu_school']."' and bad_stu='".$_POST['StuID'][$i]."' and b_date='".$_POST['DateID']."' ");
		@$result4=$db->fetch($sq4);

		if(@$result4['bad_name'] !='ไม่มาโรงเรียน' && @$result4['bad_name'] !='ลาโรงเรียน' ){
		if(@$_POST['Bad_Status'][$i]=='1'){

			$sql=$db->select_query("SELECT * FROM ".TB_BADTAIL." WHERE badtail_area='".$_SESSION['stu_area']."' and badtail_code='".$_SESSION['stu_school']."'  and badtail_name like '%ไม่มาเคารพธงชาติ%' ");
			@$result=$db->fetch($sql);
			$Bad_tail=_text_add_tab1_badtail_name_kad;
			$btailid=@$result['badtail_id'];
			$add .=$db->add_db(TB_BAD,array(
				"bad_area"=>"".$_SESSION['stu_area']."",
				"bad_code"=>"".$_SESSION['stu_school']."",
				...
				"bad_mouth"=>"".$m."",
				"bad_year"=>"".$y."",
				"bad_dam"=>$_SESSION['stu_login'],
				"bad_t"=>"1",
				"b_date"=>"".$_POST['DateID']."",
				"b_Mtime"=>"".$Mtime."",
				"bad_sess"=>$_SESSION['stu_login'],
				"role"=>"2"
			));
			$add .=$db->update_db(TB_BAD,array(
				...
				@$res['stu'] = $db->select_query("SELECT * FROM ".TB_STUDENT." WHERE stu_area='".$_SESSION['stu_area']."' and stu_code='".$_SESSION['stu_school']."'  and stu_id='".$_POST['StuID'][$i]."' "); 
				@$arr['stu'] = $db->fetch(@$res['stu']);
				@$res['B'] = $db->select_query("SELECT * FROM ".TB_BADTAIL." WHERE badtail_area='".$_SESSION['stu_area']."' and badtail_code='".$_SESSION['stu_school']."'  and badtail_id='".$btailid."' "); 
				@$arr['B'] = $db->fetch(@$res['B']);
				@$res['cl'] = $db->select_query("SELECT * FROM ".TB_CLASS_GROUP." WHERE clg_area='".$_SESSION['stu_area']."' and clg_school='".$_SESSION['stu_school']."'  and clg_group='".@$arr['stu']['stu_class']."' and clg_name='".@$arr['stu']['stu_cn']."' "); 
		...
		@$res['chclass'] = $db->select_query("SELECT * FROM ".TB_CHCLASS." WHERE c_area='".$_SESSION['stu_area']."' and c_code='".$_SESSION['stu_school']."' and c_stu='".$_POST['StuID'][$i]."' and c_date='".$_POST['DateID']."' "); 
		@$arr['chclass'] = $db->fetch(@$res['chclass']);
		if(@$arr['chclass']['c_id']){
			$CID=@$arr['chclass']['c_id'];
			if(@$arr['chclass']['c_k4'] ==''){
			$add .=$db->update_db(TB_CHCLASS,array(
				"c_area"=>"".$_SESSION['stu_area']."",
				"c_code"=>"".$_SESSION['stu_school']."",
				"c_stu"=>"".$_POST['StuID'][$i]."",
				"c_class"=>"".$_POST['ClassID']."",
				"c_cn"=>"".$_POST['Stu_cn']."",
				"c_k"=>"".$Bad_tail."",
				"c_date"=>"".$_POST['DateID']."",
				"c_note"=>$_SESSION['stu_login']
			)," c_id=".$CID." ");
			}
		} else {
			...
			$sql=$db->select_query("SELECT * FROM ".TB_BADTAIL." WHERE badtail_area='".$_SESSION['stu_area']."' and badtail_code='".$_SESSION['stu_school']."'   and badtail_name like '%มาสาย%' ");
			@$result=$db->fetch($sql);
			$btailid=@$result['badtail_id'];
			$add .=$db->add_db(TB_BAD,array(
				"bad_area"=>"".$_SESSION['stu_area']."",
		...
		if(!$arr['chkclass']['chk_id']){
			$add .=$db->add_db(TB_CHK_CHCLASS,array(
				"chk_area"=>"".$_SESSION['stu_area']."",
				"chk_code"=>"".$_SESSION['stu_school']."",
				"chk_gr"=>"1",
				"chk_class"=>"".$_POST['ClassID']."",
				"chk_cn"=>"".$_POST['Stu_cn']."",
				"chk_date"=>"".$_POST['DateID']."",
				"chk_datetime"=>"".date("Y-m-d H:i:s")."",
				"chk_note"=>$_SESSION['stu_login']
			));
		} else {
			$add="x";
		}
...
@$res['chkclass'] = $db->select_query("SELECT * FROM ".TB_CHK_CHCLASS." WHERE chk_area='".$_SESSION['stu_area']."' and chk_code='".$_SESSION['stu_school']."'  and chk_class='".$_POST['ClassID']."' and chk_cn='".$_POST['Stu_cn']."' and chk_gr='2' and chk_date='".$_POST['DateID2']."' "); 
@$arr['chkclass'] = $db->rows(@$res['chkclass']);
if($arr['chkclass']==0){
	...
	@$RANK=count($_POST['rank1'])+1;					

	for ($i=1; $i < $_POST['rank1']; $i++) {
		if(isset($_POST['StuID'][$i])){
		$sq4=$db->select_query("SELECT * FROM ".TB_BAD." WHERE bad_area='".$_SESSION['stu_area']."' and bad_code='".$_SESSION['stu_school']."' and bad_stu='".$_POST['StuID'][$i]."' and b_date='".$_POST['DateID2']."' ");
		@$result4=$db->fetch($sq4);
		if(@$result4['bad_name'] !='ไม่มาโรงเรียน' && @$result4['bad_name'] !='ลาโรงเรียน' ){
		if(@$_POST['Bad_Status'][$i]=='1'){
			$sql=$db->select_query("SELECT * FROM ".TB_BADTAIL." WHERE badtail_area='".$_SESSION['stu_area']."' and badtail_code='".$_SESSION['stu_school']."'  and badtail_name like '%ไม่ร่วมกิจกรรมโรงเรียน%' ");
			@$result=$db->fetch($sql);
			if($_SESSION['stu_school'] =='44012028'){ 
		...
		@$res['chclass'] = $db->select_query("SELECT * FROM ".TB_CHCLASS." WHERE c_area='".$_SESSION['stu_area']."' and c_code='".$_SESSION['stu_school']."' and c_stu='".$_POST['StuID'][$i]."' and c_date='".$_POST['DateID2']."' "); 
		@$arr['chclass'] = $db->fetch(@$res['chclass']);
		if(@$arr['chclass']['c_id']){
			$CID=@$arr['chclass']['c_id'];
			if(@$arr['chclass']['c_k4'] ==''){
			...
			if($_SESSION['stu_school'] =='44012028'){ 
				$Bad_tail="มาร่วมกิจกรรม Midset สาย";
				//เพิ่มกิจกรรม Midset
			} else {
				$Bad_tail=_text_add_tab2_badtail_name_sai;
			}
...
if($arr['chkclass']==0){
	...
	@$RANK=count($_POST['rank2'])+1;					

	for ($i=1; $i < $_POST['rank2']; $i++) {
		if(isset($_POST['StuID'][$i])){
		$sq4=$db->select_query("SELECT * FROM ".TB_BAD." WHERE bad_area='".$_SESSION['stu_area']."' and bad_code='".$_SESSION['stu_school']."' and bad_stu='".$_POST['StuID'][$i]."' and b_date='".$_POST['DateID3']."' ");
		@$result4=$db->fetch($sq4);
		if(@$result4['bad_name'] !='ไม่มาโรงเรียน' && @$result4['bad_name'] !='ลาโรงเรียน' ){
		if(@$_POST['Bad_Status'][$i]=='1'){
			$sql=$db->select_query("SELECT * FROM ".TB_BADTAIL." WHERE badtail_area='".$_SESSION['stu_area']."' and badtail_code='".$_SESSION['stu_school']."'  and badtail_name like '%ไม่ร่วมกิจกรรมโรงเรียน%' ");
			...
			$add .=$db->update_db(TB_BAD,array(
				...
				//@$resx = Line_To_Class(@$arr['cl']['clg_group'],@$arr['cl']['clg_name'],$message,$Token);
				//print_r(@$resx);
		@$res['chclass'] = $db->select_query("SELECT * FROM ".TB_CHCLASS." WHERE c_area='".$_SESSION['stu_area']."' and c_code='".$_SESSION['stu_school']."' and c_stu='".$_POST['StuID'][$i]."' and c_date='".$_POST['DateID3']."' "); 
		@$arr['chclass'] = $db->fetch(@$res['chclass']);
		if(@$arr['chclass']['c_id']){
			$CID=@$arr['chclass']['c_id'];
			if(@$arr['chclass']['c_k4'] ==''){
		...
		@$res['chkclass'] = $db->select_query("SELECT * FROM ".TB_CHK_CHCLASS." WHERE chk_area='".$_SESSION['stu_area']."' and chk_code='".$_SESSION['stu_school']."'  and chk_class='".$_POST['ClassID']."' and chk_cn='".$_POST['Stu_cn']."' and chk_gr='3' and chk_date='".$_POST['DateID3']."' "); 
		@$arr['chkclass'] = $db->fetch(@$res['chkclass']);
		if(!$arr['chkclass']['chk_id']){
			$add .=$db->add_db(TB_CHK_CHCLASS,array(
				"chk_area"=>"".$_SESSION['stu_area']."",
    ...
    <div class="btn-pref btn-group btn-group-justified btn-group-lg" role="group" aria-label="...">
        ...
        <div class="btn-group" role="group">
            <button type="button" id="favorites" class="btn btn-default" href="#tab2" data-toggle="tab"><span class="glyphicon glyphicon-th" aria-hidden="true"></span>
                <div class="hidden-xs"><?php if($_SESSION['stu_school'] =='44012028'){ echo "กิจกรรม MindSet";} else {echo _heading_title_M_tab2;}?></div>
            </button>
        </div>
        <div class="btn-group" role="group">
            <button type="button" id="following" class="btn btn-default" href="#tab3" data-toggle="tab"><span class="glyphicon glyphicon-th-list" aria-hidden="true"></span>
                <div class="hidden-xs"><?php if($_SESSION['stu_school'] =='44012028'){ echo "กิจกรรม Rivision";} else {echo _heading_title_M_tab3;}?></div>
            </button>
        </div>
    </div>
	...
	<div class="tab-content">
		<div class="tab-pane fade in active" id="tab1">
				...
				<form method="post" action="index.php?name=behavior&file=bclass&op=AddTab1&route=<?php echo $route;?>" enctype="multipart/form-data" id="formAdd" role="formAdd" data-toggle="validator" class="form-horizontal bootstrap-validator-form" >
						  ...
						  <?php
							...
							@$res['num'] = $db->select_query("SELECT * FROM ".TB_STUDENT." where stu_area='".$_SESSION['stu_area']."' and stu_code='".$_SESSION['stu_school']."' and stu_class='".$_SESSION['stu_class']."' and stu_cn='".$_SESSION['stu_cn']."' and stu_suspend='0' order by id"); 
							...
							<input type="hidden" name="rank"  value="<?php echo $iy;?>">
							<input type="hidden" name="ClassID"  value="<?=$_SESSION['stu_class'];?>">
							<input type="hidden" name="Stu_cn"  value="<?=$_SESSION['stu_cn'];?>">
		...
		<div class="tab-pane fade in" id="tab2">
				...
				<form method="post" action="index.php?name=behavior&file=bclass&op=AddTab2&route=<?php echo $route;?>" enctype="multipart/form-data" id="formAdd" role="formAdd" data-toggle="validator" class="form-horizontal bootstrap-validator-form" >
							<div class="col-xs-12" align="center" >
							<h4 class="box-title"><?php if($_SESSION['stu_school'] =='44012028'){ echo "เพิ่มกิจกรรม MindSet";} else {echo _heading_title_M_tab2;}?></h4>
							</div>
							<div class="col-xs-12" align="right" >
							<div class="form-group"><button class="btn bg-green btn-flat" type="submit" id="submitForm" name="submitForm"><i class="fa fa-save"></i>&nbsp;<?php echo _button_save; ?></button>&nbsp;&nbsp;<a href="index.php?name=behavior&file=bclass&route=<?php echo $route;?>" class="btn bg-red btn-flat"><i class="fa fa-undo"></i>&nbsp;<?php echo _button_reset; ?></a>
							  </div>
						  ...
						  <?php
							...
							<input type="hidden" name="OP"  value="Add">
							<input type="hidden" name="rank1"  value="<?php echo $is;?>">
							<input type="hidden" name="ClassID"  value="<?=$_SESSION['stu_class'];?>">
							<input type="hidden" name="Stu_cn"  value="<?=$_SESSION['stu_cn'];?>">
        ...
        <div class="tab-pane fade in" id="tab3">
				...
				<form method="post" action="index.php?name=behavior&file=bclass&op=AddTab3&route=<?php echo $route;?>" enctype="multipart/form-data" id="formAdd" role="formAdd" data-toggle="validator" class="form-horizontal bootstrap-validator-form" >
							<div class="col-xs-12" align="center" >
							<h4 class="box-title"><?php if($_SESSION['stu_school'] =='44012028'){ echo "เพิ่มกิจกรรม Rivision";} else {echo _heading_title_M_tab3;}?></h4>
							</div>
						  ...
						  <?php
							...
							<input type="hidden" name="OP"  value="Add">
							<input type="hidden" name="rank2"  value="<?php echo $ix;?>">
							<input type="hidden" name="ClassID"  value="<?=$_SESSION['stu_class'];?>">
							<input type="hidden" name="Stu_cn"  value="<?=$_SESSION['stu_cn'];?>">
