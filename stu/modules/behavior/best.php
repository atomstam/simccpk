...
if($op=='del'){
		...
		$del .=$db->del(TB_BESTTAIL," btail_area='".$_SESSION['stu_area']."' and btail_code='".$_SESSION['stu_school']."'  and btail_id='".$_GET['bt_id']."' ");
		if($del){
...
if($op=='cldel'){
		...
		$del .=$db->del(TB_BESTTAIL," btail_area='".$_SESSION['stu_area']."' and btail_code='".$_SESSION['stu_school']."'  and btail_id='".$_GET['bt_id']."' and btail_stu='".$_GET['stu_id']."' ");
		if($del){
		$success=_text_report_del_ok;
		} else {
		$error_warning=_text_report_del_fail;
...
if($op=='delall'){
		...
		while(list($key, $value) = each ($_POST['selected'])){
			...
			$del .=$db->del(TB_BESTTAIL," btail_area='".$_SESSION['stu_area']."' and btail_code='".$_SESSION['stu_school']."'  and btail_id='".$value."' ");
//			$db->closedb ();
		}
		if($del){
		$success=_text_report_del_ok;
...
if($op=='cldelall'){
		...
		while(list($key, $value) = each ($_POST['selected'])){
			...
			$del .=$db->del(TB_BESTTAIL," btail_area='".$_SESSION['stu_area']."' and btail_code='".$_SESSION['stu_school']."'  and btail_stu='".$value."' and btail_id='".$_GET['bt_id']."' ");
//			$db->closedb ();
		}
		if($del){
		$success=_text_report_del_ok;
		...
		<div align="right" ><div class="form-group"><button class="btn bg-green btn-flat" type="submit" id="submitForm" name="submitForm"><i class="fa fa-save"></i>&nbsp;<?php echo _button_save; ?></button>&nbsp;&nbsp;<a href="index.php?name=behavior&file=best&route=<?php echo $route;?>" class="btn bg-red btn-flat"><i class="fa fa-undo"></i>&nbsp;<?php echo _button_reset; ?></a>
				...
				<form method="post" enctype="multipart/form-data" id="formAdd" role="formAdd" data-toggle="validator" class="form-horizontal bootstrap-validator-form" >
					    <div class="box box-success" id="loading-example">
							...
							@$res['stu'] = $db->select_query("SELECT * FROM ".TB_STUDENT." where stu_area='".$_SESSION['stu_area']."' and stu_code='".$_SESSION['stu_school']."' and stu_class='".$_SESSION['stu_class']."' and stu_cn='".$_SESSION['stu_cn']."' ORDER BY stu_class,stu_cn,stu_id");
							while (@$arr['stu'] = $db->fetch(@$res['stu'])){
								@$res['b'] = $db->select_query("SELECT * FROM ".TB_BESTTAIL." WHERE btail_area='".$_SESSION['stu_area']."' and btail_code='".$_SESSION['stu_school']."' and btail_stu='".@$arr['stu']['stu_id']."' ");
...
<?php
		...
		@$res['stu'] = $db->select_query("SELECT * FROM ".TB_STUDENT." WHERE stu_area='".$_SESSION['stu_area']."' and stu_code='".$_SESSION['stu_school']."' and stu_class='".$_SESSION['stu_class']."' and stu_cn='".$_SESSION['stu_cn']."' and stu_id='".$_GET['stu_id']."' "); 
		@$arr['stu'] =$db->fetch(@$res['stu']);
		@$res['best'] = $db->select_query("SELECT * FROM ".TB_BESTTAIL." WHERE btail_area='".$_SESSION['stu_area']."' and btail_code='".$_SESSION['stu_school']."' and btail_stu='".$_GET['stu_id']."'"); 
...
@$res['bad'] = $db->select_query("SELECT *,count(btail_stu) as STU FROM ".TB_BESTTAIL." WHERE btail_area='".$_SESSION['stu_area']."' and btail_code='".$_SESSION['stu_school']."' and stu_class='".$_SESSION['stu_class']."' and stu_cn='".$_SESSION['stu_cn']."' and btail_id='".$_GET['bt_id']."' group by btail_id"); 
...
?>
     ...
     <button type="button" class="close" data-dismiss="alert">X</button>
	  ...
	  <?php
		...
		@$res['num'] = $db->select_query("SELECT *,count(btail_stu) as CO FROM ".TB_BESTTAIL." as a, ".TB_STUDENT." as b where btail_area='".$_SESSION['stu_area']."' and btail_code='".$_SESSION['stu_school']."'  and stu_class='".$_SESSION['stu_class']."' and stu_cn='".$_SESSION['stu_cn']."' and btail_id='".$_GET['bt_id']."'  and btail_stu=stu_id group by btail_stu order by CO desc,stu_class,stu_id "); 
      ...
      <form action="index.php?name=behavior&file=best&op=cldelall&bt_id=<?php echo $_GET['bt_id'];?>&route=<?php echo $route;?>" method="post" enctype="multipart/form-data" id="form" class="form-inline">
		...
		@$res['tail'] = $db->select_query("SELECT * FROM ".TB_BESTTAIL." where btail_area='".$_SESSION['stu_area']."' and btail_code='".$_SESSION['stu_school']."' and btail_id='".@$arr['num']['btail_id']."' "); 
		@$arr['tail'] =$db->fetch(@$res['tail']);
...
@$res['count'] = $db->select_query("SELECT * FROM ".TB_BESTTAIL." where btail_area='".$_SESSION['stu_area']."' and btail_code='".$_SESSION['stu_school']."' group by btail_stu"); 
...
<div class="col-xs-12 connectedSortable">
    ...
    <div align="right" >
    <br>
      <div class="buttons"><a href="index.php?name=behavior&file=best&op=add&route=<?php echo $route;?>" class="btn bg-aqua btn-flat"><i class="fa fa-edit"></i>&nbsp;<?php echo _button_add; ?></a>&nbsp;&nbsp;<a onclick="$('form').submit();" class="btn bg-red btn-flat"><i class="fa fa-trash-o"></i>&nbsp;<?php echo _button_del; ?></a></div>
      <br>
      </div>
    <div class="box box-danger">
      ...
      <form action="index.php?name=behavior&file=best&op=delall&route=<?php echo $route;?>" method="post" enctype="multipart/form-data" id="form" class="form-inline">
		...
		$i=1;
		while (@$arr['num'] = $db->fetch(@$res['num'])){
		@$res['tail'] = $db->select_query("SELECT *,count(btail_id) as CO FROM ".TB_BESTTAIL." where btail_area='".$_SESSION['stu_area']."' and btail_code='".$_SESSION['stu_school']."' and btail_id='".@$arr['num']['bt_id']."' "); 
		@$arr['tail'] =$db->fetch(@$res['tail']);
