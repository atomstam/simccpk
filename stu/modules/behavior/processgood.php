...
if($_POST['OP']=='Add'){
	...
	for($y=0;$y<$Good_tail;$y++){
		@$res['G'] = $db->select_query("SELECT * FROM ".TB_GOODTAIL." WHERE goodtail_area='".$_SESSION['stu_area']."' and goodtail_code='".$_SESSION['stu_school']."'   and goodtail_id='".$_POST['Good_tail'][$y]."' "); 
...
//	$c= array_unique ($Stu_Token);
	...
	for($x=0;$x<$Good_stu;$x++){
		@$res['stu'] = $db->select_query("SELECT * FROM ".TB_STUDENT." left join ".TB_CLASS_GROUP." on clg_group=stu_class and clg_name=stu_cn left join ".TB_CLASS." on class_id=stu_class  WHERE stu_area='".$_SESSION['stu_area']."' and stu_code='".$_SESSION['stu_school']."'  and stu_id='".$_POST['Good_stu'][$x]."' group by stu_id"); 
