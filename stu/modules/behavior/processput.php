...
if($_POST['OP']=='Add'){
	if( $_POST['Best_stu'] !='' && $_POST['Stu_best'] !='' && $_POST['Btail_per'] !='' && $_POST['Pu_Dtime'] && $_POST['Btail_name'] !='' ){
		...
		for($i=0;$i<$Put;$i++){
					...
					$sql_put=$db->select_query("SELECT * FROM ".TB_PUT." WHERE pu_area='".$_SESSION['stu_area']."' and pu_code='".$_SESSION['stu_school']."' and pu_id like '".$_POST['Stu_best']."' ");
					@$result_put=$db->fetch($sql_put);

						$add .=$db->add_db(TB_PUTTAIL,array(
						"pt_area"=>"".$_SESSION['stu_area']."",
						"pt_code"=>"".$_SESSION['stu_school']."",
					...
					$sql=$db->select_query("SELECT * FROM ".TB_GOODTAIL." WHERE goodtail_area='".$_SESSION['stu_area']."' and goodtail_code='".$_SESSION['stu_school']."' and goodtail_name like '%".$result_put['pu_name']."%' ");
					@$result=$db->fetch($sql);
