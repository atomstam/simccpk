...
if($_POST['OP']=='Add'){
	...
	$db->connectdb(DB_NAME,DB_USERNAME,DB_PASSWORD);
		...
		//@$res['aff'] = $db->select_query("SELECT * FROM ".TB_AFFTAIL." WHERE afft_area='".$_SESSION['stu_area']."' and afft_code='".$_SESSION['stu_school']."' and afft_aff='".$_POST['Stu_best']."' "); 
		//@$arr['aff'] =$db->rows(@$res['aff']);
		//if($arr['aff']==0){

		$Mtime=date("H:i:s");

		@$res['tail'] = $db->select_query("SELECT * FROM ".TB_AFFAIRS." WHERE aff_area='".$_SESSION['stu_area']."' and aff_code='".$_SESSION['stu_school']."' and aff_id='".$_POST['Stu_best']."' "); 
	...
	for ($i=0; $i < $RANK; $i++) {
		if(!empty($_POST['StuID'][$i])){
				...
				@$res['stu'] = $db->select_query("SELECT * FROM ".TB_STUDENT." left join ".TB_CLASS_GROUP." on clg_group=stu_class and clg_name=stu_cn left join ".TB_CLASS." on class_id=stu_class WHERE stu_area='".$_SESSION['stu_area']."' and stu_code='".$_SESSION['stu_school']."'  and stu_id='".$_POST['StuID'][$i]."' group by stu_id"); 
				...
				@$res['user'] = $db->select_query("SELECT * FROM ".TB_PERSON." WHERE per_ids='".$_SESSION['stu_login']."' "); 
				@$arr['user'] = $db->fetch(@$res['user']);
				$Tech=@$arr['user']['per_name']." "._text_report_line_message9."".@$arr['user']['tel'];
				$message3 = $Tech."\r\n";

				if($_POST['Ck1'][$i] =='1'){

						$add .=$db->add_db(TB_AFFTAIL,array(
						"afft_area"=>"".$_SESSION['stu_area']."",
						"afft_code"=>"".$_SESSION['stu_school']."",
						"afft_aff"=>"".$_POST['Stu_best']."",
						"afft_stu"=>"".$_POST['StuID'][$i]."",
						"afft_name"=>"".$_POST['Btail_name']."",
						"afft_date"=>"".$_POST['Pu_Dtime']."",
						"afft_per"=>"".$_SESSION['stu_login']."",
						"afft_status"=>"1"
						));

					$sql=$db->select_query("SELECT * FROM ".TB_GOODTAIL." WHERE goodtail_area='".$_SESSION['stu_area']."' and goodtail_code='".$_SESSION['stu_school']."' and goodtail_name like '%ช่วยเหลืองานโรงเรียน%' ");
					@$result=$db->fetch($sql);
					$Good_tail= _text_box_table_tab4_true." ".@$arr['tail']['aff_name'];
					$gtailid=@$result['goodtail_id'];

					$add .=$db->add_db(TB_GOOD,array(
					"good_area"=>"".$_SESSION['stu_area']."",
					"good_code"=>"".$_SESSION['stu_school']."",
					...
					"good_mouth"=>"".$m."",
					"good_year"=>"".$y."",
					"good_dam"=>"".$_SESSION['stu_login']."",
					"good_t"=>"1",
					"g_date"=>"".$_POST['Pu_Dtime']."",
					"g_Mtime"=>"".$Mtime."",
					"good_sess"=>"".$_SESSION['stu_login'].""
					));	
