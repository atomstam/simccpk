...
<?php
		@$res['num'] = $db->select_query("select *,sum(badtail_point) as CO  from ".TB_BAD." as a ,".TB_STUDENT." as b,".TB_BADTAIL." as c where a.bad_area='".$_SESSION['stu_area']."' and a.bad_code='".$_SESSION['stu_school']."' and b.stu_id=a.bad_stu and a.bad_tail=c.badtail_id and a.b_date='".$ToDayDate."' and stu_suspend='0' group by b.stu_id order by CO desc"); 
