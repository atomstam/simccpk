...
CheckStu($_SESSION['stu_login'],$_SESSION['stu_pwd']);
@$res['user'] = $db->select_query("SELECT * FROM ".TB_STUDENT." WHERE stu_area ='".$_SESSION['stu_area']."' and stu_code='".$_SESSION['stu_school']."'  and stu_pid='".$_SESSION['stu_login']."' "); 
...
<div class="wrapper">
    ...
    <nav class="navbar navbar-static-top">
      ...
      <!-- Navbar Right Menu -->
<?php
@$res['comx'] = $db->select_query("SELECT * FROM ".TB_MESSAGE.",".TB_MESSAGE_CHECK." where ms_to in ('all' ,'".$_SESSION['stu_login']."')  and msc_mss !=ms_id  and msc_user='".$_SESSION['stu_login']."' and ms_area='".$_SESSION['stu_area']."' and ms_school='".$_SESSION['stu_school']."' group by ms_id order by ms_id desc limit 5"); 
...
?>
                    ...
                    <ul class="nav navbar-nav">
                        ...
                        <li class="dropdown messages-menu">
                            ...
                            <ul class="dropdown-menu">
                                ...
                                <li>
									...
									@$res['ms'] = $db->select_query("SELECT * FROM ".TB_MESSAGE." as a,".TB_MESSAGE_CHECK." as b where a.ms_to in ('all' ,'".$_SESSION['stu_login']."') and ms_area='".$_SESSION['stu_area']."' and ms_school='".$_SESSION['stu_school']."' and b.msc_mss !=a.ms_id  group by a.ms_id order by a.ms_id desc limit 5"); 
									while (@$arr['ms'] = $db->fetch(@$res['ms'])){
									@$res['userx'] = $db->select_query("SELECT * FROM ".TB_STUDENT." where stu_pid='".@$arr['ms']['ms_posted']."' "); 
									...
									?>
                                        <li><!-- start message -->
                                            <a href="index.php?name=access&file=message&op=detail&route=<?php echo $route;?>&MsID=<?php echo @$arr['ms']['ms_id'];?>">
                                                <div class="pull-left">
                                                    <img src="<?php if(@$arr['userx']['stu_pic']){echo WEB_URL_IMG_STU.@$arr['userx']['stu_pic'];}else{echo WEB_URL_IMG_STU."no_image.jpg";}?>" class="img-circle" alt="User Image"/>
                                                </div>
                                                <h4>
                        ...
                        <li class="dropdown user user-menu">
                            <a href="#" class="dropdown-toggle" data-toggle="dropdown">
                                ...
                                <span><?php echo @$_SESSION['stu_name']; ?> <i class="caret"></i></span>
                            </a>
                            <ul class="dropdown-menu">
                                <!-- User image -->
                                <li class="user-header bg-light-blue">
                                     <?php if(!empty(@$arr['user']['stu_pic'])){?>
                                            <img src="<?php echo WEB_URL_IMG_STU.@$arr['user']['stu_pic']; ?>" class="img-circle" alt="User Image" />
                                     <?php } else {?>
                                            <img src="<?php echo WEB_URL_IMG_STU."no_image.jpg";?>" class="img-circle" alt="User Image"/>
                                      <?php } ?>
                                    <p>
                                        <?php echo @$arr['user']['stu_id']; ?>
                                        <small><?php echo @$_SESSION['stu_name']; ?></small>
                                    </p>
                                </li>
                                <!-- Menu Body -->
                                <li class="user-body">
                                ...
                                <li class="user-footer">
                                    <div class="pull-left">
                                        <a href="index.php?name=config&file=stu&op=detail&stu_id=<?php echo  @$arr['user']['stu_id'];?>&route=config/stu" class="btn btn-default btn-flat">Profile</a>
                                    </div>
			...
			<aside class="main-sidebar">
				...
				<!-- Sidebar user panel -->
						...
						<?php
						@$res['online'] = $db->select_query("SELECT * FROM ".TB_STUDENT_ONLINE." as a , ".TB_STUDENT." as b WHERE b.per_area ='".$_SESSION['stu_area']."' and b.per_code='".$_SESSION['stu_school']."'  and b.stu_pid=a.u_user and u_timeout > '".time()."' "); 
						@$rows['online'] = $db->rows(@$res['online']); 
						?>
                        <div class="pull-left info">
                            <p>Hello, <?php echo @$arr['user']['stu_id'];?></p>
			...
			@$res['menu'] = $db->select_query("SELECT * FROM ".TB_STUDENT_MENU." where type='1' and status='1' order by menu_id "); 
			//@$arr['menu'] = $db->fetch(@$res['menu']);
			?>
			<?php while ($MainMenu = $db->fetch(@$res['menu'])) { ?>
                        <li <?php if($MainMenu['MFiles']==$route){ echo "class=\"active treeview\"";} else { echo "class=\"treeview\"";}?> >
                            <a href="index.php?name=<?php echo $MainMenu['mods'];?>&file=<?php echo $MainMenu['dbs'];?>&route=<?php echo $MainMenu['MFiles']; ?>">
  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
	      <h4>
		<i class="fa fa-graduation-cap"></i> <?php echo $_SESSION['school_name']." [".$_SESSION['stu_school']."]"; ?>
         </h4>
	<ol class="breadcrumb">
