...
<?php
		$db->connectdb(DB_NAME,DB_USERNAME,DB_PASSWORD);
		@$res['nums'] = $db->select_query("select stu_id,stu_pic,stu_pid,stu_num,stu_name,stu_sur,class_name,stu_class,sum(badtail_point) as CO  from ".TB_BAD." ,".TB_STUDENT.",".TB_BADTAIL.",".TB_CLASS." where stu_area='".$_SESSION['stu_area']."' and stu_code='".$_SESSION['stu_school']."'  and stu_id=bad_stu and bad_tail=badtail_id and class_id=stu_class group by stu_id order by CO desc limit 8 "); 
...
<?php
		//$db->connectdb(DB_NAME,DB_USERNAME,DB_PASSWORD);
		@$res['nums'] = $db->select_query("select stu_id,stu_pic,stu_pid,stu_num,stu_name,stu_sur,class_name,stu_class,sum(goodtail_point) as GO  from ".TB_GOOD." ,".TB_STUDENT.",".TB_GOODTAIL.",".TB_CLASS." where stu_area='".$_SESSION['stu_area']."' and stu_code='".$_SESSION['stu_school']."' and stu_id=good_stu and good_tail=goodtail_id and class_id=stu_class group by stu_id order by GO desc limit 8 "); 
		while(@$arr['nums'] = $db->fetch(@$res['nums'])){
?>
                    <li>
                      <img src="<?php if(@$arr['nums']['stu_pic']){echo WEB_URL_IMG_STU.@$arr['nums']['stu_pic'];} else {echo WEB_URL_IMG_STU."no_image.jpg";} ?>" alt="User Image">
