...
if($op=='cldetail' and $action=='' ){
		...
		@$res['numchkclass'] = $db->select_query("SELECT * FROM ".TB_CHK_CHMCLASS." WHERE chkm_area='".$_SESSION['person_area']."' and chkm_code='".$_SESSION['person_school']."' and DATE_FORMAT(chkm_date, '%Y-%m') like '%".$_GET['mo_id']."%'  group by chkm_class,chkm_cn"); 
...
<div class="col-xs-12 connectedSortable">
      ...
      <form  method="post" enctype="multipart/form-data" id="form" class="form-inline">
		...
		$year = $CC[0]+543;
		$date=$CC[2];
		@$res['chkclass'] = $db->select_query("SELECT * FROM ".TB_CHK_CHMCLASS." WHERE chkm_area='".$_SESSION['person_area']."' and chkm_code='".$_SESSION['person_school']."' and DATE_FORMAT(chkm_date, '%Y-%m-%d') like '%".$arr['num']['c_date']."%' group by chkm_class,chkm_cn"); 
...
<?php
		...
		@$res['numchkclass'] = $db->select_query("SELECT * FROM ".TB_CHK_CHMCLASS." WHERE chkm_area='".$_SESSION['person_area']."' and chkm_code='".$_SESSION['person_school']."' and DATE_FORMAT(chkm_date, '%Y-%m-%d') like '%".$_GET['moid']."%' and gr='".$_GET['gr']."' group by chkm_class,chkm_cn"); 
		@$NumRowChkclass = $db->rows(@$res['numchkclass']);
		...
		?>
            <tr>
              ...
              <td layout="block" style="text-align: center;">
				...
				$res['chk'] = $db->select_query("SELECT * FROM ".TB_CHK_CHMCLASS." WHERE chkm_area='".$_SESSION['person_area']."' and chkm_code='".$_SESSION['person_school']."' and chkm_class='".$arr['num']['class_id']."' and chkm_cn='".$arr['num']['clg_name']."' and DATE_FORMAT(chkm_date, '%Y-%m-%d') like '%".$_GET['moid']."%' "); 
...
?>
				...
				<div class="form-group">
				  ...
				  <form  method="post" enctype="multipart/form-data" id="form" class="form-inline">
					...
					$get_month = $DD[1];
					$year = $DD[0]+543;
					@$res['chkclass'] = $db->select_query("SELECT *,count(chkm_id) as CO  FROM ".TB_CHK_CHMCLASS." WHERE chkm_area='".$_SESSION['person_area']."' and chkm_code='".$_SESSION['person_school']."' and DATE_FORMAT(chkm_date, '%Y-%m') like '%".$DD1."%' group by DATE_FORMAT(chkm_date, '%Y-%m') like '%".$DD1."%'"); 
