...
if($_POST['OP']=='Add'){
	...
	$db->connectdb(DB_NAME,DB_USERNAME,DB_PASSWORD);
		...
		//@$res['aff'] = $db->select_query("SELECT * FROM ".TB_AFFTAIL." WHERE afft_area='".$_SESSION['person_area']."' and afft_code='".$_SESSION['person_school']."' and afft_aff='".$_POST['Stu_best']."' "); 
	...
	for ($i=0; $i < $RANK; $i++) {
		if(!empty($_POST['StuID'][$i])){
				...
				@$res['stu'] = $db->select_query("SELECT * FROM ".TB_STUDENT." left join ".TB_CLASS_GROUP." on clg_group=stu_class and clg_name=stu_cn left join ".TB_CLASS." on class_id=stu_class WHERE stu_area='".$_SESSION['person_area']."' and stu_code='".$_SESSION['person_school']."'  and stu_id='".$_POST['StuID'][$i]."' group by stu_id"); 
				...
				if($_POST['Ck1'][$i] =='1'){
						...
						"afft_code"=>"".$_SESSION['person_school']."",
						"afft_aff"=>"".$_POST['Stu_best']."",
						"afft_stu"=>"".$_POST['StuID'][$i]."",
						"afft_name"=>"".$_POST['Btail_name']."",
						"afft_date"=>"".$_POST['Pu_Dtime']."",
						"afft_per"=>"".$_SESSION['person_login']."",
						"afft_status"=>"1"
						));

					$sql=$db->select_query("SELECT * FROM ".TB_GOODTAIL." WHERE goodtail_area='".$_SESSION['person_area']."' and goodtail_code='".$_SESSION['person_school']."' and goodtail_name like '%ช่วยเหลืองานโรงเรียน%' ");
					@$result=$db->fetch($sql);
					...
					"good_mouth"=>"".$m."",
					"good_year"=>"".$y."",
					"good_dam"=>"".$_SESSION['person_login']."",
					"good_t"=>"1",
