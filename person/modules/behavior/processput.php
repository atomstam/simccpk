...
if($_POST['OP']=='Add'){
	if( $_POST['Best_stu'] !='' && $_POST['Stu_best'] !='' && $_POST['Btail_per'] !='' && $_POST['Pu_Dtime'] && $_POST['Btail_name'] !='' ){
		...
		for($i=0;$i<$Put;$i++){
					...
					$sql=$db->select_query("SELECT * FROM ".TB_GOODTAIL." WHERE goodtail_area='".$_SESSION['person_area']."' and goodtail_code='".$_SESSION['person_school']."' and goodtail_name like '%".$result_put['pu_name']."%' ");
					@$result=$db->fetch($sql);
