...
if($_POST['OP']=='Add'){
	...
	for($y=0;$y<$Bad_tail;$y++){
		@$res['G'] = $db->select_query("SELECT * FROM ".TB_BADTAIL." WHERE badtail_area='".$_SESSION['person_area']."' and badtail_code='".$_SESSION['person_school']."'   and badtail_id='".$_POST['Bad_tail'][$y]."' "); 
...
//	$c= array_unique ($Stu_Token);
	...
	for($x=0;$x<$Bad_stu;$x++){
		@$res['stu'] = $db->select_query("SELECT * FROM ".TB_STUDENT." left join ".TB_CLASS_GROUP." on clg_group=stu_class and clg_name=stu_cn left join ".TB_CLASS." on class_id=stu_class  WHERE stu_area='".$_SESSION['person_area']."' and stu_code='".$_SESSION['person_school']."'  and stu_id='".$_POST['Bad_stu'][$x]."' group by stu_id"); 
