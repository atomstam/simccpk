...
if($op=='del'){
		...
		$del .=$db->del(TB_AFFTAIL," afft_area='".$_SESSION['person_area']."' and afft_code='".$_SESSION['person_school']."' and afft_aff='".$_GET['afft_id']."' ");
		if($del){
...
if($op=='delcl'){
		...
		@$res['cl'] = $db->select_query("SELECT * FROM ".TB_CLASS_GROUP.",".TB_STUDENT." where clg_area='".$_SESSION['person_area']."' and clg_school='".$_SESSION['person_school']."' and clg_group='".$_GET['cl_id']."' and clg_name='".$_GET['clg_gr']."' and stu_class=clg_group and stu_cn=clg_name order by clg_id "); 
		while(@$arr['cl'] = $db->fetch(@$res['cl'])){
		$del .=$db->del(TB_AFFTAIL," afft_area='".$_SESSION['person_area']."' and afft_code='".$_SESSION['person_school']."' and afft_stu='".@$arr['cl']['stu_id']."' ");
		}
		if($del){
		$success=_text_report_del_ok;
		} else {
...
if($op=='cldel'){
		...
		$del .=$db->del(TB_AFFTAIL," afft_area='".$_SESSION['person_area']."' and afft_code='".$_SESSION['person_school']."' and afft_id='".$_GET['afft_id']."' and afft_stu='".$_GET['stu_id']."' ");
		if($del){
		$success=_text_report_del_ok;
		} else {
		$error_warning=_text_report_del_fail;
...
if($op=='delall'){
		...
		while(list($key, $value) = each ($_POST['selected'])){
			...
			$del .=$db->del(TB_AFFTAIL," afft_area='".$_SESSION['person_area']."' and afft_code='".$_SESSION['person_school']."' and afft_aff='".$value."' ");
//			$db->closedb ();
		}
		if($del){
		$success=_text_report_del_ok;
...
if($op=='cldelall'){
		...
		while(list($key, $value) = each ($_POST['selected'])){
			...
			$del .=$db->del(TB_AFFTAIL," afft_area='".$_SESSION['person_area']."' and afft_code='".$_SESSION['person_school']."' and afft_stu='".$value."' and afft_id='".$_GET['afft_id']."' ");
//			$db->closedb ();
		}
		if($del){
		$success=_text_report_del_ok;
...
if($op=='studel'){
		...
		$del .=$db->del(TB_AFFTAIL," afft_area='".$_SESSION['person_area']."' and afft_code='".$_SESSION['person_school']."' and afft_id='".$_GET['afft_id']."' ");
		if($del){
		$success=_text_report_del_ok;
		} else {
		$error_warning=_text_report_del_fail;
...
//                $("#error").html(msg.message),
  ...
  <!--  <div class="col-xs-12" align="right" >
	<div class="form-group"><button class="btn bg-green btn-flat" type="submit" id="submitForm" name="submitForm"><i class="fa fa-save"></i>&nbsp;<?php echo _button_save; ?></button>&nbsp;&nbsp;<a href="index.php?name=behavior&file=affairs&route=<?php echo $route;?>" class="btn bg-red btn-flat"><i class="fa fa-undo"></i>&nbsp;<?php echo _button_reset; ?></a>
							...
							@$res['bt'] = $db->select_query("SELECT * FROM ".TB_AFFAIRS." where aff_area='".$_SESSION['person_area']."' and aff_code='".$_SESSION['person_school']."' ORDER BY aff_id ");
						  ...
						  <?php
							...
							<input type="hidden" name="Stu_class4"  value="<?=$_SESSION['person_class'];?>">
							<input type="hidden" name="Stu_cn"  value="<?=$_SESSION['person_class'];?>">
							<br>
							</div>
							</div>
...
<?php
		...
		@$res['best'] = $db->select_query("SELECT * FROM ".TB_AFFTAIL." where afft_area='".$_SESSION['person_area']."' and afft_code='".$_SESSION['person_school']."' and afft_aff='".$_GET['afft_id']."'"); 
...
<?php
		...
		@$res['stu'] = $db->select_query("SELECT * FROM ".TB_STUDENT." where stu_area='".$_SESSION['person_area']."' and stu_code='".$_SESSION['person_school']."' and stu_id='".$_GET['stu_id']."' "); 
		@$arr['stu'] =$db->fetch(@$res['stu']);
		@$res['best'] = $db->select_query("SELECT * FROM ".TB_AFFTAIL." where afft_area='".$_SESSION['person_area']."' and afft_code='".$_SESSION['person_school']."' and afft_aff='".$_GET['afft_id']."' and afft_stu='".$_GET['stu_id']."'  "); 
		@$arr['best']= $db->fetch(@$res['best']);
?>
      <div class="alert alert-success" name="thanks" id="thanks" style="display: none">
      <button type="button" class="close" data-dismiss="alert">X</button>
...
@$res['count'] = $db->select_query("SELECT * FROM ".TB_AFFTAIL." where afft_area='".$_SESSION['person_area']."' and afft_code='".$_SESSION['person_school']."' and afft_aff='".$_GET['afft_id']."' group by afft_stu"); 
@$rows['count'] = $db->rows(@$res['count']);
@$res['tails'] = $db->select_query("SELECT * FROM ".TB_AFFAIRS." where aff_area='".$_SESSION['person_area']."' and aff_code='".$_SESSION['person_school']."' and aff_id='".$_GET['afft_id']."' "); 
...
<div class="col-xs-12 connectedSortable">
    ...
    <div class="box box-danger">
	  ...
	  <?php
		...
		@$res['num'] = $db->select_query("SELECT * FROM ".TB_CLASS_GROUP." where clg_area='".$_SESSION['person_area']."' and clg_school='".$_SESSION['person_school']."' order by clg_id "); 
		@$rows['num'] = $db->rows(@$res['num']);
		if(@$rows['num']) {
		?>
      <form action="index.php?name=behavior&file=affairs&op=delall&route=<?php echo $route;?>" method="post" enctype="multipart/form-data" id="form" class="form-inline">
		...
		@$res['stu'] = $db->select_query("SELECT *,count(DISTINCT IF(afft_status=0,afft_stu,NULL)) as CO,count(DISTINCT IF(afft_status=1,afft_stu,NULL)) as AC FROM ".TB_STUDENT." , ".TB_AFFTAIL." where afft_area='".$_SESSION['person_area']."' and afft_code='".$_SESSION['person_school']."' and stu_class='".@$arr['num']['clg_group']."' and afft_stu=stu_id and stu_cn='".@$arr['num']['clg_name']."' "); 
		...
		@$res['tail'] = $db->select_query("SELECT *,count(afft_stu) as STU FROM ".TB_AFFTAIL." where afft_area='".$_SESSION['person_area']."' and afft_code='".$_SESSION['person_school']."' and afft_stu='".@$arr['stu']['stu_id']."' group by afft_stu"); 
...
@$res['num'] = $db->select_query("SELECT *,count(afft_stu) as CO FROM ".TB_AFFTAIL." as a, ".TB_STUDENT." as b where afft_aff='".$_GET['afft_id']."' and afft_area='".$_SESSION['person_area']."' and afft_code='".$_SESSION['person_school']."' and afft_stu=stu_id and  stu_class='".$_GET['cl_id']."' and stu_cn='".$_GET['clg_gr']."' group by afft_stu order by CO desc,stu_class,stu_id "); 
@$rows['num'] = $db->rows(@$res['num']);

@$res['tail'] = $db->select_query("SELECT * FROM ".TB_AFFAIRS." where aff_area='".$_SESSION['person_area']."' and aff_code='".$_SESSION['person_school']."' and aff_id='".$_GET['afft_id']."' "); 
@$arr['tail'] =$db->fetch(@$res['tail']);
?>
...
@$res['count'] = $db->select_query("SELECT * FROM ".TB_STUDENT." where stu_area='".$_SESSION['person_area']."' and stu_code='".$_SESSION['person_school']."' "); 
@$rows['count'] = $db->rows(@$res['count']);
?>
<div class="row">
<div class="col-xs-12 connectedSortable">
    ...
    <div class="box box-danger">
      ...
      <div class="box-body ">
	  <?php
		@$res['num'] = $db->select_query("SELECT * FROM ".TB_AFFAIRS." WHERE aff_area='".$_SESSION['person_area']."' and aff_code='".$_SESSION['person_school']."' order by aff_id "); 
		@$rows['num'] = $db->rows(@$res['num']);
		if(@$rows['num']) {
		?>
      <form action="index.php?name=behavior&file=affairs&op=delall&route=<?php echo $route;?>" method="post" enctype="multipart/form-data" id="form" class="form-inline">
		...
		@$res['tail'] = $db->select_query("select *,count(DISTINCT IF(afft_status=0,afft_stu,NULL)) as CO,count(DISTINCT IF(afft_status=1,afft_stu,NULL)) as AC FROM ".TB_AFFTAIL." where afft_area='".$_SESSION['person_area']."' and afft_code='".$_SESSION['person_school']."' and afft_aff='".@$arr['num']['aff_id']."' "); 
		@$arr['tail'] =$db->fetch(@$res['tail']);
