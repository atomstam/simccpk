...
if($_POST['Stu_num'] == ''._text_box_table_stu_num_chai.''){
	...
	$Sex=2;
		...
		$add .=$db->add_db(TB_STUDENT,array(
			"stu_id"=>"".$_POST['Stu_id']."",
			"stu_area"=>"".$_SESSION['person_area']."",
			"stu_code"=>"".$_SESSION['person_school']."",
...
if($_POST['Stu_num'] == ''._text_box_table_stu_num_chai.''){
	...
	if(@$arr['bestt']){
		$edit .=$db->update_db(TB_BESTTAIL,array(
			...
			"btail_name"=>"".$_POST['Btail_name']."",
			"btail_per"=>"".$_POST['Btail_per'].""
			)," btail_area='".$_SESSION['person_area']."' and btail_code='".$_SESSION['person_school']."'  and btail_stu=".$_POST['SID']." ");
	} else {
...
if($_POST['OP']=='Import'){
	if( $_SESSION['person_area'] !='' && $_SESSION['person_school'] !='' && $_POST["Icon"] !='' ){
	$db->connectdb(DB_NAME,DB_USERNAME,DB_PASSWORD);
...
//			$Birthday=$YY."-".$MM."-".$DD;
		...
		if($i>1) {
			@$res['stu'] = $db->select_query("SELECT * FROM ".TB_STUDENT." where stu_code='".$_SESSION['person_school']."' and stu_id='".$filesop[5]."' "); 
...
//				"stu_lat"=>"".$_POST['lat_value']."",
//				"stu_long"=>"".$_POST['lon_value']."" 
			)," stu_area='".$_SESSION['person_area']."' and stu_code='".$_SESSION['person_school']."' and stu_id=".$filesop[5]." ");
			}
...
//			$Birthday=$YY."-".$MM."-".$DD;
			...
			@$res['stu'] = $db->select_query("SELECT * FROM ".TB_STUDENT." where stu_code='".$_SESSION['person_school']."' and stu_id='".$data->sheets[$i][cells][$j][6]."' "); 
			@$arr['stu'] = $db->fetch(@$res['stu']);
			if(empty(@$arr['stu']['stu_id'])){
			$import .=$db->add_db(TB_STUDENT,array(
...
//				"stu_lat"=>"".$_POST['lat_value']."",
//				"stu_long"=>"".$_POST['lon_value']."" 
			)," stu_area='".$_SESSION['person_area']."' and stu_code='".$_SESSION['person_school']."' and stu_id=".$data->sheets[$i][cells][$j][6]." ");
			}
		...
		@$res['stux'] = $db->select_query("SELECT * FROM ".TB_STUDENT." WHERE stu_area='".$_SESSION['person_area']."' and stu_code='".$_SESSION['person_school']."' and stu_class='".@$arr['num']['class_id']."' group by stu_cn"); 
		while(@$arr['stux']=$db->fetch(@$res['stux'])){
			$db->add_db(TB_CLASS_GROUP,array(
			"clg_area"=>"".$_SESSION['person_area']."",
			"clg_school"=>"".$_SESSION['person_school']."",
