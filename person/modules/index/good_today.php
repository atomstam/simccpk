...
<?php
		@$res['num'] = $db->select_query("select *,sum(goodtail_point) as GO  from ".TB_GOOD." as a,".TB_STUDENT." as b,".TB_GOODTAIL." as c where a.good_area='".$_SESSION['person_area']."' and a.good_code='".$_SESSION['person_school']."' and b.stu_id=a.good_stu and a.good_tail=c.goodtail_id and a.g_date='".$ToDayDate."' and stu_suspend='0' group by b.stu_id order by GO desc"); 
