...
$return_arr = array();
@$result = $db->select_query("SELECT *,sum(badtail_point) AS CO FROM ".TB_BAD."  ,".TB_BADTAIL.",".TB_STUDENT."  where  bad_tail=badtail_id and stu_id=bad_stu and stu_suspend='0' and bad_area='".$_SESSION['person_area']."' and bad_code='".$_SESSION['person_school']."' group by bad_mouth"); 
 //@$rows = $db->fetch(@$result);
 //for($i=0;$i<13;$i++){
$i=1;
while(@$rows = $db->fetch(@$result)){
	@$resultG = $db->select_query("select *,sum(goodtail_point) as GO  from ".TB_GOOD." ,".TB_GOODTAIL.",".TB_STUDENT."  where  good_tail=goodtail_id and good_mouth='".@$rows['bad_mouth']."' and stu_id=good_stu and stu_suspend='0' and good_area='".$_SESSION['person_area']."' and good_code='".$_SESSION['person_school']."' group by good_mouth"); 
