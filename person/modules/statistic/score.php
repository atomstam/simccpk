...
?>
		...
		<div align="right" >
		<div class="form-group"><a href="index.php?name=statistic&file=score&route=<?php echo $route;?>" class="btn bg-orange btn-flat"><i class="fa fa-undo"></i>&nbsp;<?php echo _button_detail; ?></a>
		</div>
		</div>
...
<script>
$(document).ready(function(){
	$("#Sales-chart").load("modules/statistic/graph_score.php");
});
</script>
      <div class="row">
	  <div id="Sales-chart" ><center><img src="../img/ajax-loader1.gif" border="0"></center></div>
	  </div>
...
<?php
		$db->connectdb(DB_NAME,DB_USERNAME,DB_PASSWORD);
		@$res['num'] = $db->select_query("select stu_id,stu_pic,stu_pid,stu_num,stu_name,stu_sur,class_name,stu_class,sum(goodtail_point) as GO  from ".TB_GOOD." ,".TB_STUDENT.",".TB_GOODTAIL.",".TB_CLASS." where stu_area='".$_SESSION['person_area']."' and stu_code='".$_SESSION['person_school']."' and stu_id=good_stu and good_tail=goodtail_id and class_id=stu_class and stu_suspend ='0' group by stu_id order by GO desc"); 
		@$rows['num'] = $db->rows(@$res['num']);
		if(@$rows['num']) {
	...
	<div class="box-body ">
		...
		?>
            <tr>
              ...
              <td style="text-align: center;">
			 <a href="index.php?name=statistic&file=score&op=gdetail&id=<?php echo @$arr['num']['stu_id'];?>&route=<?php echo $route;?>" class="btn bg-green btn-flat btn-sm" ><i class="fa fa-search-plus "></i></a>
			  </td>
            </tr>
            <?php $is++;} ?>
          </tbody>
...
//                '-'+pageTotal10 +' ( -'+ total10 +' <?php echo _text_box_table_pu_score;?>)'
