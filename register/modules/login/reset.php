...
$message_mail .= "<tr><td><br><a href=".WEB_URL."/index.php?name=admin&file=reset&op=confirm&id=".$user."&session=".$session.">"._RESET_MAIL_SEND_SESSION_RESET."";
...
require_once("../includes/phpmailler/class.phpmailer.php");
 ...
 $mail->Port = 465;
 $mail->SMTPAuth = true;
 $mail->Username = ''._google_email.''; //อีเมล์ของคุณ (Google App)
 $mail->Password = ''._google_email_password.''; //รหัสผ่านอีเมล์ของคุณ (Google App)
 $mail->From = "".WEB_EMAIL.""; // ใครเป็นผู้ส่ง
 $mail->FromName = ''._heading_main_title.''; // ชื่อผู้ส่งสักนิดครับ
...
?>
   <form action="" method="post" enctype="multipart/form-data" id="form" role="form" class="form-horizontal">
					    <div class="box box-info" id="loading-example">
							...
							<label class="col-sm-5 control-label col-sm-pad" ><?php echo _text_input_email; ?></label>
							<div class="col-sm-3" >
							...
							<br>
							<button type="submit" name="submit" class="btn btn-primary"><?php echo _button_add;?></button>&nbsp;&nbsp;<a href="index.php" class="btn bg-red btn-flat btn-sm"><?php echo _button_reset; ?></a>
							<br>
							</div>
							</div>
