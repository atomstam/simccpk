...
require_once("modules/message/function.php");
...
if($op=='delcom'){
		...
		$res['res'] = $db->select_query("SELECT * FROM ".TB_MESSAGE_COM." WHERE mc_id='".$_GET['McID']."' and mc_posted='".$user_login."' "); 
		...
		$del =$db->del(TB_MESSAGE_COM," mc_id='".$_GET['McID']."' and mc_posted='".$user_login."' ");
...
if($op=='del'){
		$db->connectdb(DB_NAME,DB_USERNAME,DB_PASSWORD);
		$res['res'] = $db->select_query("SELECT * FROM ".TB_MESSAGE." WHERE ms_id='".$_GET['MsID']."' and ms_posted='".$user_login."' "); 
		$rows['res'] = @$db->rows($res['res']);
		if($rows['res']){
		$del =$db->del(TB_MESSAGE," ms_id='".$_GET['MsID']."' and ms_posted='".$user_login."' ");
		$del .=$db->del(TB_MESSAGE_CHECK," msc_mss='".$_GET['MsID']."' and ms_posted='".$user_login."' ");
		$del .=$db->del(TB_MESSAGE_COM," mc_ms='".$_GET['MsID']."' and mc_posted='".$user_login."' ");
		} else {
		$error_warning=_text_report_del_null_fail;
		}
...
<form action="index.php?name=message&file=index&op=add&action=add&route=<?php echo $route;?>" method="post" enctype="multipart/form-data" id="form" role="form" class="form-horizontal">
					    ...
					    <div class="box box-success" id="loading-example">
							...
							<?php
							$db->connectdb(DB_NAME,DB_USERNAME,DB_PASSWORD);
							$res['res'] = $db->select_query("SELECT * FROM ".TB_USER." where username !='".$user_login."' order by user_id"); 
...
$db->connectdb(DB_NAME,DB_USERNAME,DB_PASSWORD);
$res['ms'] = $db->select_query("SELECT * FROM ".TB_MESSAGE." where ms_id='".$_GET['MsID']."' and ms_posted='".$user_login."' ");
...
$res['ck']=$db->select_query("SELECT * FROM ".TB_MESSAGE_CHECK." where msc_mss='".$_GET['MsID']."' and msc_user='".$user_login."' and msc_school='".$user_school."' ");
...
if(!$arr['ck']){
		...
		$db->update_db(TB_MESSAGE_CHECK,array(
			"msc_date"=>"".TIMESTAMP.""
		),"  msc_mss='".$_GET['MsID']."' and msc_user='".$user_login."' and msc_school='".$user_school."' ");
}
...
?>
<?php
if($arr['ms']['ms_posted']==$user_login){
?>
							...
							?>
								...
								<li>
                                    <div class="timeline-item">
										...
										<?php if($user_login==$arr['usere']['username']){?>
										...
										<?php } ?>
                                            <a class="btn btn-warning btn-xs">Views : <?php echo $arr['mss']['ms_views'];?></a>
                                ...
                                <li>
                                    ...
                                    <div class="timeline-item">
										...
										<?php if($user_login==$arr['usert']['username']){?>
                                        <div class='timeline-footer'>
...
<div class="col-xs-12 connectedSortable">
    ...
    <div align="right" >
    <br>
      <div class="buttons"><a href="index.php?name=message&file=index&op=add&route=<?php echo $route;?>" class="btn bg-aqua btn-flat"><i class="fa fa-save"></i>&nbsp;<?php echo _button_add; ?></a>&nbsp;&nbsp;</div>
      <br>
      </div>
    <div class="box box-danger">
	  ...
	  <?php
		$db->connectdb(DB_NAME,DB_USERNAME,DB_PASSWORD);
		$res['ms'] = $db->select_query("SELECT * FROM ".TB_MESSAGE." WHERE ms_to in ('all' ,'".$user_login."')  or ms_posted='".$user_login."' order by ms_id desc"); 
        ...
        <table id="example1" class="table table-bordered table-striped responsive" style="width:100%">
          <thead>
            <tr >
              ...
              <th layout="block" style="text-align:center"><?php echo _table_heading_table_message_topic; ?></th>
              <th layout="block" style="text-align:center"><?php echo _table_heading_table_message_views; ?></th>
              <th layout="block" style="text-align:center"><?php echo _table_heading_table_message_comm; ?></th>
              <th layout="block" style="text-align:center"><?php echo _table_heading_table_message_posted; ?></th>
              <th layout="block" style="text-align:center"><?php echo _table_heading_table_message_date; ?></th>
		...
		$res['com'] = $db->select_query("SELECT * FROM ".TB_MESSAGE_CHECK." WHERE msc_user='".$user_login."' and msc_mss='".$arr['ms']['ms_id']."' "); 
		$rows['com'] = @$db->rows($res['com']);
		?>
            <tr>
              <td style="text-align: center;"><?php echo $arr['ms']['ms_id']; ?></td>
              ...
              <td layout="block" style="text-align:center"><?php echo $arr['ms']['ms_views']; ?></td>
              <td layout="block" style="text-align:center"><?php echo $arr['ms']['ms_comm']; ?></td>
              <td layout="block" style="text-align:center"><?php echo $arr['ms']['ms_posted']; ?></td>
              <td layout="block" style="text-align:center"><?php echo ThaiTimeConvert($arr['ms']['ms_date'],'',1);?></td>
              <td style="text-align: center;">
				...
				<?php if($arr['ms']['ms_posted']==$user_login){?>
