...
$added=date("Y-m-d H:i:s");
		...
		)," ct_user='".$_POST['OldName']."' and ct_school='".$user_school."' ");

		$edit .=$db->update_db(TB_ONLINE,array(
...
$db->connectdb(DB_NAME,DB_USERNAME,DB_PASSWORD);
$res['user'] = $db->select_query("SELECT * FROM ".TB_USER." WHERE username='".$user_login."' "); 
$arr['user'] = $db->fetch($res['user']);
$usercode=$arr['user']['user_id'];
$userimg=$arr['user']['img'];
?>
 ...
 //                       $("#showFile").append('<input type=hidden name=icon value='+file.name+'>');
    ...
    <br>
	  ...
	  <?php
		...
		$arr['user'] = $db->fetch($res['user']);
		?>
		<form action="index.php?name=user&file=index&op=edit&userId=<?php echo $arr['user']['user_id'];?>&route=<?php echo $route;?>" method="post" enctype="multipart/form-data" id="form" role="form" class="form-horizontal" >
