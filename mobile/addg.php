...
<div class="row">
   ...
   <div class="box-body  ">
		...
		<div align="right" ><div class="form-group"><a href="addg.php?tuser=<?php echo $tdata;?>" class="btn btn-warning" ><i class="fa fa-edit"></i>&nbsp;เพิ่มข้อมูลพฤติกรรมบวกใหม่</a>
		</div></div>
<?php
$res['gd'] = $db->select_query("SELECT * FROM ".TB_GOOD." where g_date like '%".$DateIn."%' order by good_id desc limit 10 "); 
$row= @$db->rows($res['gd']);
if($row){
...
$res['stu'] = $db->select_query("SELECT * FROM ".TB_STUDENT." where stu_id='".$arr['gd']['good_stu']."' "); 
...
$res['good'] = $db->select_query("SELECT * FROM ".TB_GOODTAIL." where goodtail_id='".$arr['gd']['good_tail']."' ");
$arr['good'] = $db->fetch($res['good']);
$res['per'] = $db->select_query("SELECT * FROM ".TB_PERSON." where  per_ids='".$arr['gd']['good_dam']."' ");
...
echo "ประเภท : <span class='badge bg-purple'>".$arr['good']['goodtail_name']."</span><br>";
echo "รายละเอียด : <span class='badge bg-green'>".$arr['gd']['good_name']."</span><br>";
echo "คะแนน : <span class='badge bg-yellow'>".$arr['good']['goodtail_point']."</span><br>";
echo "ผู้บันทึก : <span class='badge bg-red'>".$arr['per']['per_name']."</span><br>";
echo "วันที่ : <span class='badge bg-green'>".$arr['gd']['g_date']."</span><br>";
echo "</td></tr>";
}
echo "</table>";
} else {
...
//                 $("#thanks").html(msg.message),
				 ...
				 $("#error").hide();
				 setTimeout(function() {
  				 window.location='addg.php?OP=detail&tuser=<?php echo $tdata;?>';
				}, 1000);
			} else {
...
<div id="Respon">
				...
				<form method="post" enctype="multipart/form-data" id="formAdd" role="formAdd" data-toggle="validator" class="form-horizontal bootstrap-validator-form" >
							...
							<div class="form-group has-feedback" >
							<label class="col-sm-3 control-label" >รายการพฤติกรรมบวก : </label>
							<div class="col-sm-6">
							<select class="form-control select3" multiple="multiple" name="Good_tail[]" >
							<?php
							$db->connectdb(DB_NAME,DB_USERNAME,DB_PASSWORD);
							$res['good'] = $db->select_query("SELECT * FROM ".TB_GOODTAIL." ORDER BY goodtail_id ");
							...
							<div class="col-sm-6">
							<input type='text' class="form-control" name="Good_name" class="form-control css-require">
							</div>
							</div>
							<div class="form-group has-feedback">
							...
							<input type='text' class="form-control" name="Good_YMD" class="form-control css-require">
						    ...
						    <div class="form-group has-feedback">
							<label class="col-sm-3 control-label" >การเสริมแรงทางบวก : </label>
							<div class="col-sm-3" >
